...
(function () {
    'use strict';
    angular.module('siemens.simaticit.common')
        .config(['common.widgets.auditTrail.service.configProvider','CONFIG', function (atConfigProvider, CONFIG) {

            atConfigProvider.config.bckService = 'common.services.runtime.backendService';
            if (CONFIG && CONFIG.type === 'eng') {
                atConfigProvider.config.bckService = 'auditTrail.record.service';
            }
        }]);
...
(function () {
    ...
    module.config(['common.services.authentication.configProvider', 'CONFIG', 'APPCONFIG', function (authenticationConfigProvider, CONFIG, APPCONFIG) {
        ...
        if (config.home) {
            authenticationConfigProvider.config.homeRoute = config.home.screen || config.home;
        }
...
(function () {
    ...
    angular.module('siemens.simaticit.common')
        .config(['common.services.runtime.DataConfigProvider','CONFIG','APPCONFIG', function (businessDataConfigProvider, CONFIG, APPCONFIG) {
            ...
            var config = angular.merge({}, APPCONFIG, CONFIG);
...
(function () {
    ...
    StateConfiguration.$inject = ['$stateProvider', '$urlRouterProvider', 'APPCONFIG'];
    function StateConfiguration($stateProvider, $urlRouterProvider, APPCONFIG) {
        /**
         * state description
        ...
        if (APPCONFIG.appConfig && (APPCONFIG.appConfig.type === 'runtime' || APPCONFIG.appConfig.type === 'dev')) {
            home.params.mduiCtx = window.$UIF.String.encode(JSON.stringify({
                stateParams: {},
                tracking: []
            }));
        }
    ...
    function StateListeners($rootScope, $state, $http, $resolve, $stateParams, $location, $translate, common, LOG_TYPES, debug,
        CONFIG, APPCONFIG, swacComponentManager, swacUiModuleManager, UIAuthorization, warningService, commonCtx) {
        ...
        function activate() {
            ...
            warning = warningService.initCustomOverlayOnStateChange();
            setAboutDialogCtx();
        ...
        function isSidepanelState(statename) {
            ...
            var canvasAreaContainer = stateData.views['Canvas@'];
            if (canvasAreaContainer && canvasAreaContainer.hasOwnProperty('templateUrl')) return false;
        ...
        function isInCavasState(stateObj) {
            var stateData = $state.get(stateObj.name);
            if (!stateData) return false;
            var propertyAreaContainer = $state.get(stateObj.name).views['panel-in-canvas@'];
            return propertyAreaContainer;
        }

        function setAboutDialogCtx() {
            if (swacUiModuleManager.enabled) {
                swacUiModuleManager.contextServicePromise.promise.then(function (service) {
                    $http.get("../../system/info.json").then(function (response) {
                        var productName = "Opcenter Execution " + response.data.about.name;
                        var productVersion = " " + response.data.about.version;
                        service.updatePartialCtx('momProductSettings.name', productName);
                        service.updatePartialCtx('momProductSettings.version', productVersion);
                        service.updatePartialCtx('momProductSettings.legalEntity', response.data.about.legalEntity);
                    });
                });
            }
        }

        function onStateChangeStart(event, _toState, _toParams, _fromState, _fromParams, options, _toStateOriginal) {
            // Condition to check if any navigation is requested when UI Component is exposed as swac compoponent
            // if yes, block he navigation
            if (commonCtx.isNavigationBlocked()) {
            ...
            //first step: is startup tasks completed
            if (!applicationContext.isStartupTasksCompleted()) {
                // startup task in progress, a new request to navigate
                // register as pending navigation
                if (_toState.name !== 'home' && _fromState.name === 'home') {
                    applicationContext.setPendingNavigation({
                        stateName: _toState.name,
                        stateParams: _toParams
                    });
                    event.preventDefault();
                    event.skip = true;
                    return;
            ...
            if (isInCavasState(_fromState) && $state.get(_fromState).parent === $state.get(_toState).name) {
                warning.setIsFromIncanvas(true);
            } else {
                warning.setIsFromIncanvas(false);
            }
            ...
            if (_toState.name === 'home' && _fromState.name.indexOf('home.') === 0 && !options.reload) {
                if (isInCavasState(_fromState)) {
                    $rootScope.isInCanvasView = true;
                    return;
                }
                event.preventDefault();
            ...
            function checkDocumentViewer() {
                if (!window.$UIF.Object.safeGet(window, 'PDFViewerApplication')) {
                    return;
                }
            ...
            function navigateToHome() {
                ...
                if (commonCtx.isSwacEnabled()) {
                    stateConfig = { location: false };
                }
                var stateid = Date.now() + '' + window.$UIF.Utility.random();
                $state.go('home', { 'stateid': stateid, 'stateToReturn': _toState.name, 'stateParamsToReturn': JSON.stringify(_toParams) }, stateConfig);
            }
            ...
            function updateContainerState() {
                ...
                if (isInCavasState(_fromState) && warning.getOverlayDisplay() && !warning.getIsViewAction()) {
                    $rootScope.isInCanvasView = true;
                    return;
                }
                else if (isInCavasState(_toState)) {
                    $rootScope.isInCanvasView = true;
                    warning.setOverlayDisplay(true);
                } else {
                    $rootScope.isInCanvasView = false;
                    warning.setOverlayDisplay(false);
                }

                if (!swacUiModuleManager.enabled || sidePanelState) {
                    return;
                }
                swacUiModuleManager.navigationServicePromise.promise.then(function (service) {
                    var params = {};
                    var urlParamNames = _toStateOriginal.url.$$paramNames;
                    for (var index = 0; index <= urlParamNames.length; index++) {
                        var paramName = urlParamNames[index];
                        params[paramName] = _toParams[paramName];
                    }
                    delete params.stateid;
                    service.navigateTo('momSwacSublocation', { screen: _toState.name, parameters: params }, { notify: false });
                });
            }
        ...
        function onStateChangeSuccess(event, _toState, _toParams, _fromState, _fromParams) {
            ...
            if ($state.get(_fromState.name) && $state.get(_fromState.name).views && $state.get(_fromState.name).views['panel-in-canvas@']) {
                setTimeout(function () {
            ...
            if (swacUiModuleManager.enabled) {
                if (data.toState.views && (data.toState.views['Canvas@'] || data.toState.views['FullScreenView'])) {
                    ...
                    if (!((data.fromState.name.indexOf(data.toState.name) !== -1) && data.fromState.views && data.fromState.views['property-area-container@'] &&
                        data.fromState.views['property-area-container@'].templateUrl)) {
                        swacUiModuleManager.contextServicePromise.promise.then(function (service) {
                            service.updatePartialCtx('location.titles', { headerTitle: headerTitle });
                            swacUiModuleManager.eventBusServicePromise.promise.then(function (eventBusSvc) {
                    ...
                    if (!sidePanelState && !isInCavasState(data.toState)) {
                        if (common.historyTracker.length > 2 && backButtonClicked) {
        ...
        function onViewContentLoaded() {
            ...
            if (swacUiModuleManager.enabled && !swacUiModuleManager.swacComponent &&
                ($state.current.name !== undefined && $state.current.name !== ''
                    && $state.current.name !== 'home' && $state.current.name !== 'home.component')
            ) {
