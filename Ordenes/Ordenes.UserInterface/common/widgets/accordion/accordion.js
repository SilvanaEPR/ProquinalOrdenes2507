...
(function () {
    ...
    /**
   * @ngdoc directive
   * @name sitGenericAccordion
   * @module siemens.simaticit.common.widgets.accordion
   * @description
   * A directive which is used as a wrapper over {@link sitGenericAccordionPanel} in order to avoid multiple accordion type with in a single accordion configuration.
   * @usage
   * As an html element:
   * ```
   * <sit-generic-accordion sit-options="genericAccordion.genericAccordionOptions"></sit-generic-accordion>
   * ```
   * @restrict E
   *
   * @param {Object} sitOptions A {@link genericAccordionOptions} object containing details for configuring generic accordion type.
   *
   * @example
   * In a view template, you can use the **sitGenericAccordion** as follows:
   *  ```
   * <sit-generic-accordion sit-options="genericAccordion.genericAccordionOptions">
   *     <sit-generic-accordion-panel sit-options="genericAccordion.panelOptions"><div> This is generic accordion body text.</div></sit-generic-accordion-panel>
   * </sit-generic-accordion>
   *
   * ```
   * To see implementation of sit-generic-accordion-panel, refer {@link sitGenericAccordionPanel}.
   *
   * The following example shows how to configure a sit-generic-accordion widget:
   *
   * In Controller:
   * ```
   *    genericAccordionOptions: {
   *        type: 'STYLED'
   *    }
   * ```
   */
    ...
    /**
   * @ngdoc type
   * @name genericAccordionOptions
   * @module siemens.simaticit.common.widgets.accordion
   * @description A configuration object that contains details for configuring the generic accordion widget, refer {@link sitGenericAccordion}.
   * The object must have the following format:
   * @property {string} type [type = 'STYLED'] The string that represents the type of the generic accordion.
   * There are three types of generic accordion:
   *  * **BASIC** : This type of generic accordion has a transparent background where there is no distance between each accordion. This is the default type of generic accordion.
   *  * **ADVANCED** : This type of generic accordion has a black shaded background where the distance between
   *  each accordion is 8px. This is the required accordion type to create accordion in sidepanel.
   *  * **STYLED** : This type of generic accordion has a width of 340px with a basic border formatting.
   *
   * @example
   * ```
   *    type: 'STYLED'
   ...
   */
    ...
    angular.module('siemens.simaticit.common.widgets.accordion').directive('sitGenericAccordion',
        ['$timeout', 'common.widgets.service',
            function ($timeout, widgetService) {

                var TYPE = widgetService.ENUMS.AccordionType;

                GenericAccordionController.prototype = new widgetService.BaseWidget('Generic Accordion widget', {
                    type: TYPE.BASIC
                });

                GenericAccordionController.$inject = ['$scope'];

                function GenericAccordionController($scope) {
                    this.$init($scope);
                }
                ...
                return {
                    ...
                    controller: GenericAccordionController,
                    controllerAs: 'genericAccordionCtrl',
                    link: function (scope, element, attrs, ctrl) {
                        $timeout(function () {
                            ctrl.$invokeEvent('onLoad', { element: element });
                        }, 0, false);
                    },
                    templateUrl: 'common/widgets/accordion/sit-generic-accordion.html'
                };
            }]);
})();
...
(function () {
    ...
    /**
   * @ngdoc directive
   * @name sitGenericAccordionPanel
   * @module siemens.simaticit.common.widgets.accordion
   * @description
   * A directive which is responsible for creation of accordion.
   * @usage
   * As an html element:
   * ```
   * <sit-generic-accordion-panel sit-options="genericAccordionPanel.genericAccordionPanelOptions"></sit-generic-accordion>
   * ```
   * @restrict E
   *
   * @param {Object} sitOptions A {@link genericAccordionPanelOptions} object containing details for configuring the type of generic accordion.
   *
   * @example
   * In a view template, you can use the **sitGenericAccordionPanel** as follows:
   *  ```
   * <div>
   * <sit-generic-accordion-panel sit-options="genericAccordion.genericAccordionPanelOptions">
   *    <div> This is generic accordion body text.</div>
   * </sit-generic-accordion-panel>
   * </div>
   *
   *
   * ```
   * The following example shows how to configure a sit-generic-accordion-panel widget:
   *
   * In Controller:
   * ```
   *    genericAccordionPanelOptions: {
   *        header: 'Hello world',
   *        state: 'EXPAND'
   *    }
   *
   ...
   */
    ...
    /**
   * @ngdoc type
   * @name genericAccordionPanelOptions
   * @module siemens.simaticit.common.widgets.accordion
   * @description A configuration object that contains details for configuring the generic accordion panel widget, refer {@link sitGenericAccordionPanel}.
   * The object must have the following format:
   * @property {string} header [header = 'Hello World'] The string that contains the user-defined header for the accordion.
   * @property {string} state [state = 'EXPAND'] The string that contains the state of the accordion while loading.
   * There are two states of generic accordion:
   *  * **COLLAPSE** : Here the body of the accordion being loaded is in collapsed form.
   *  * **EXPAND** : Here the body of the accordion being loaded is in expanded form.
   *
   * @example
   * ```
   *    header: 'Hello world',
   ...
   */
    ...
    angular.module('siemens.simaticit.common.widgets.accordion').directive('sitGenericAccordionPanel',
        ['$timeout', 'common.widgets.service',
            function ($timeout, widgetService) {

                var STATE = widgetService.ENUMS.AccordionState;

                GenericAccordionPanelController.prototype = new widgetService.BaseWidget('Generic Accordion Panel widget', {
                    header: '',
                    state: STATE.COLLAPSE
                }, ['onExpand','onCollapse', 'collapse', 'expand']);

                GenericAccordionPanelController.$inject = ['$scope'];

                function GenericAccordionPanelController($scope) {
                    var vm = this;
                    this.$init($scope);

                    (function (vm) {
                        init(vm);
                        vm.$register('collapse', collapse);
                        vm.$register('expand', expand);
                    })(this);
                    ...
                    function toggle() {
                     (vm.options.state === STATE.COLLAPSE)  ? expand() : collapse();
                    }

                    function collapse() {
                        vm.options.state = STATE.COLLAPSE;
                        vm.$invokeEvent('onCollapse', vm);
                    }

                    function expand() {
                        vm.options.state = STATE.EXPAND;
                        vm.$invokeEvent('onExpand', vm);
                    }
                }
                ...
                return {
                    ...
                    controller: GenericAccordionPanelController,
                    controllerAs: 'genericAccordionPanelCtrl',
                    link: function (scope, element, attrs, ctrl) {
                        $timeout(function () {
                            ctrl.$invokeEvent('onLoad', { element: element });
                        }, 0, false);
                    },
                    templateUrl: 'common/widgets/accordion/sit-generic-accordion-panel.html'
                };
            }]);
})();
