...
(function () {
    ...
    */
    ...
    NumericController.$inject = ['$scope', '$element', '$timeout'];
    function NumericController($scope, $element, $timeout) {
        var vm = this;

        if (vm.api) {
            vm.api.getValueAsString = getValueAsString;
        }

        function getValueAsString() {
            if (vm.ngReadonly || vm.readOnly) {
                vm.valueAsString = vm.value;
                return vm.valueAsString;
            }
            var value = $('#sitNumericId', $element)[0].value;
            vm.valueAsString = value;
            return value;
        }

        if (vm.valueAsString) {
            $scope.$watch(function () {
                return vm.ngReadonly || vm.readOnly;
            }, function (newVal, oldVal) {
                if (newVal !== oldVal && newVal === false) {
                    $timeout(function () {
                        $('#sitNumericId', $element)[0].value = vm.valueAsString;
                    });
                }
            });
        }
    ...
    app.directive('sitNumeric', ['common.overlay.overlayService', '$timeout', function (overlayService, $timeout) {
        ...
        return {
            ...
            bindToController: {
                ...
                'value': '=sitValue',
                'valueAsString': '=sitValueAsString',
                'api': '=sitApi',
            ...
            link: function (scope, ele, attr, ctrl, timeout) {
                if (ctrl.valueAsString && !(ctrl.ngReadonly || ctrl.readOnly)) {
                    $timeout(function () {
                        $('#sitNumericId', ele)[0].value = ctrl.valueAsString;
                    });
                }
            },
