...
*/
    ...
    sitInteractiveGraph.$inject = ['$translate', '$compile', '$timeout', 'd3', '$window', '$sanitize'];
    function sitInteractiveGraph($translate, $compile, $timeout, d3, $window, $sanitize) {
        ...
        function GraphController($scope, $element, $compile, fontAwesomIconJson) {
            ...
            function init() {
                ...
                vm.autoFit = autoFit;
                vm.showPanWindow = showPanWindow;
            ...
            function progressBar() {
                $(".graphZoomControlInput").mousemove(function (e) {
                    var calZoomLevel = ($(this).val() - $(this).attr('min')) / ($(this).attr('max') - $(this).attr('min'));
                    var zoomPercent = calZoomLevel * 100;

                    $(this).css('background-image',
                        '-webkit-gradient(linear, left top, right top, ' +
                        'color-stop(' + zoomPercent + '%, #005F87), ' +
                        'color-stop(' + zoomPercent + '%, #e6e4e1)' +
                        ')');
                });
            }
            //#endregion
        ...
        function GraphUtil(scope, $compile, element, controller) {
            ...
            function attachToolTipForEdges(svg) {
                var elements = svg.select('g').selectAll('g.edgeLabel');
                if (elements && elements[0] && elements[0].length > 0) {
            ...
            function calcEllipsis(textStringParam, maxWidth) {
                ...
                myDiv.html($sanitize(textString));
                ...
                if (width >= maxWidth) {
                    ...
                    while (++i < strLength) {
                        currSubStringWidth = myDiv.html($sanitize(textString.substring(0, i)) + '...').width();
            ...
            function handleZoom() {
                var percentAdjustment = 14.28571428;
                ...
                var zoomLevel = scope.zoomLevel / 7;
                var zoomLevelPercent = (zoomLevel * 100) - percentAdjustment;
                element.find('.graphbackground #zoomControlInput').css('background-image', '-webkit-gradient(linear, left top, right top, '
                    + 'color-stop(' + zoomLevelPercent + '%, #005F87), '
                    + 'color-stop(' + zoomLevelPercent + '%, #e6e4e1)' + ')');
            }
        ...
        function link(scope, element, attrs, controller) {
            ...
            $timeout(function () {
                ...
                // Removing JS click because the bootstrap was not updating the range/bar in the DOM.
                ...
                scope.$on('$destroy', function () {
                    ...
                    element.find('div.graphbackground svg[data-internal-type=\'graphsvg\'] g.edgePath').off('mouseenter mouseleave', graphUtil.graphRefresh);
