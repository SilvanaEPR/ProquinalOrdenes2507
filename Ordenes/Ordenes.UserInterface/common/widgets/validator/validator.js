...
(function () {
    ...
    */
    ...
    app.directive('sitFormInputValidator', ['$compile', '$translate', function ($compile, $translate) {
        return {
            ...
            link: function (scope, element, attrs, ctrl) {
                ...
                if (formController.$name === 'textForm' || formController.$name === 'numericForm') {
                    var displayWarning = false;
                    scope.warnMessage = '';
                    formController.showWarning = function (warningMessage) {
                        displayWarning = true;
                        scope.warnMessage = $translate.instant(warningMessage);
                    };
                    formController.hideWarning = function () {
                        displayWarning = false;
                    };
                    formController.getWarningStatus = function () {
                ...
                ngModelController.$parsers.unshift(function (viewValue) {
                    ...
                    if (widgetController && widgetController.executeWidgetValidation) {
                        ngModelUpdated = widgetController.executeWidgetValidation(viewValue, ngModelController) || ngModelController;
                    }
                ...
                var concat = "";
                concat += "<div data-internal-type='formWarning' ng-show='(" + itemFormName + ".$invalid && (" + itemFormName + ".$dirty || " + itemFormName + ".$visited)) || " + itemFormName + ".getWarningStatus()' ";
                concat += "ng-class= \" " + itemFormName +
                    ".$focused ? 'validator-warning-group-addon-focused input-group-addon' : 'validator-warning-group-addon input-group-addon' \"> ";
                concat += "<i ng-class= \" { 'fa fa-warning validator-warning-icon-focused': " + itemFormName + ".$focused, 'fa fa-warning validator-warning-icon' : !" + itemFormName + ".$focused, 'validator-control-warn-icon' : !" + itemFormName + ".$invalid && " + itemFormName + ".getWarningStatus() }\" ";
                ...
                concat += " data-sit-tile-content=\"" + itemFormName +".$invalid ? warningMessage : warnMessage \"";
                concat += " data-sit-tile-type=\" " + itemFormName +".$invalid ? 'warning' : 'info' \"";
                ...
                if (attrs['sitValidation']) {
                    var inputCtrlName = attrs['sitValidation'].split('.')[0];
                    scope.validatorCtrl.validation = scope[inputCtrlName].validation;
                }
                if (attrs['sitChange']) {
                ...
                widgetController = scope[inputCtrl];
                if (widgetController.setNgModelController) {
                    widgetController.setNgModelController(ngModelController);
                }
