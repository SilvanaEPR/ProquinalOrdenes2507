...
(function () {
    ...
    function ExtendedPagerDirective(widgetService, $timeout) {
        ...
        ExtendedPagerController.$inject = ['$scope'];
        function ExtendedPagerController($scope) {
            var vm = this;
            this.$init($scope);
            ...
            function generateDynamicPages() {
                ...
                if (vm.isLeftMoreBtnVisible) {
                    vm.leftMorePages = [];
                    vm.flyoutOptions.left.templatedata = [];
                ...
                if (vm.isRightMoreBtnVisible) {
                    vm.rightMorePages = [];
                    vm.flyoutOptions.right.templatedata = [];
                ...
                compile: function () {
                    return {
                        pre: function (scope, element, attr, ctrl) {
                            ctrl.$invokeEvent('onBeforeLoad', { element: element });
                        },
        ...
        return {
            ...
            link: function (scope, el, attr, ctrl) {
                ...
                var template = '<div class="sit-pagination-flyout popover"><div class="arrow"></div><div class="flyout" role="tooltip">' +
                    '<div class="popover-body"></div></div></div>'
                ...
                $(el).on("shown.bs.popover", function () {
                    ...
                    ctrl.popoverOverElement.css({
                        top: (moreBtnOffset.top - ctrl.popoverOverElement.first().outerHeight() - (buttonMargin / 2)),
                        left: btngrpwidth + buttonWidth
                    })
                    ctrl.popoverOverElement[0].style.setProperty('transform', 'none', 'important');
...
(function () {
   ...
   */
    ...
    function simplePagerDirective(widgetService) {
        ...
        SimplePagerController.$inject = ['$scope'];
        function SimplePagerController($scope) {
            var vm = this;
            this.$init($scope);
