            ...
            (function (widgets) {
                ...
                (function (sidebar) {
                    ...
                    var SidebarMenuItem = /** @class */ (function () {
                        function SidebarMenuItem(id, title, appTitle, header, icon, url, isSwac, sitIconSvgData, category, oldSvgIcon) {
                            this.id = id;
                            this.title = this.getTranslatedTitle(title, appTitle, header);
                        ...
                        SidebarMenuItem.prototype.getTranslatedTitle = function (title, appTitle, header) {
                            if (!header || appTitle !== title) {
                                return title;
                            }
                            return header;
                        };
                    ...
                    var SidebarService = /** @class */ (function () {
                            ...
                            for (var i = 0; i < source.length; i++) {
                                ...
                                if (item.contents && item.contents.length > 0) {
                                    for (var j = 0; j < item.contents.length; j++) {
                                        var subItem = item.contents[j];
                                        if (subItem.id === id) {
                                            homeItem = new SidebarMenuItem(subItem.id, subItem.title, subItem.appTitle, subItem.header, subItem.icon, subItem.url, subItem.isSwac, this.sitIconSvgData, subItem.category, subItem.svgIcon);
                                            itemFound = true;
                                        }
                                        if (subItem.contents && subItem.contents.length > 0 && itemFound === false) {
                                            for (var k = 0; k < subItem.contents.length; k++) {
                                                ...
                                                if (subContent.id === id) {
                                                    homeItem = new SidebarMenuItem(subContent.id, subContent.title, subContent.appTitle, subContent.header, subContent.icon, subContent.url, subContent.isSwac, this.sitIconSvgData, subContent.category, subContent.svgIcon);
                                                    itemFound = true;
                                                }
                                            }
                                        }
                        ...
                        SidebarService.prototype.validateItem = function (menuItem) {
                            if (menuItem.id === undefined) {
                                ...
                                var item = new SidebarMenuItem(menuItem.id, menuItem.title, menuItem.appTitle, menuItem.header, menuItem.icon, menuItem.url, menuItem.isSwac, this.sitIconSvgData, menuItem.category, menuItem.svgIcon);
                        ...
                        SidebarService.prototype.setMenu = function (source) {
                            var defaultSource = [];
                            this.menuItems = [];
                            this.home = this.CONFIG.home.screen || this.CONFIG.home;
                            this.isHomeCardEnabled = this.CONFIG.home.type !== 'custom';
                            this.isCategoryMode = this.CONFIG.home.navigationMode === 'category';
                            ...
                            if (undefined === source) {
                                ...
                                if (this.CONFIG.menu && this.CONFIG.menu.length > 0 && undefined !== this.CONFIG.menu[0].id) {
                                    ...
                                    if (this.isCategoryMode) {
                                        defaultSource.push(this.generateCategoryMenu(JSON.parse(JSON.stringify(this.CONFIG.menu)), this.CONFIG.categories));
                                    }
                                    else {
                                        defaultSource.push(JSON.parse(JSON.stringify(this.CONFIG.menu)));
                                    }
                                    isDefalutSet = false;
                        ...
                        SidebarService.prototype.generateCategoryMenu = function (configMenu, categories) {
                            var _this = this;
                            var categoryMenu = __spreadArray([], categories, true);
                            var groupedItems = {};
                                ...
                                if (contents && contents.length) {
                                    contents.forEach(function (content) {
                                        if (content.hasOwnProperty('category')) {
                                            _this.groupCategoryItems(content, groupedItems);
                                        }
                                    });
                                }
                                else if (menu.hasOwnProperty('category')) {
                                    _this.groupCategoryItems(menu, groupedItems);
                                }
                            });
                            categoryMenu = _.filter(categoryMenu, function (menuItem) {
                                return groupedItems[menuItem.id] && groupedItems[menuItem.id].length > 0;
                            });
                            categoryMenu.forEach(function (menu) {
                                menu.title = menu.id;
                                menu.contents = _this.setCategoryScreenOrder(groupedItems[menu.id], menu.screens);
                            });
                            return categoryMenu;
                        };
                        SidebarService.prototype.groupCategoryItems = function (content, groupedItems) {
                            var category = content.category;
                            if (!groupedItems.hasOwnProperty(category)) {
                                groupedItems[category] = [];
                            }
                            content.display && groupedItems[category].push(content);
                        };
                        SidebarService.prototype.setCategoryScreenOrder = function (groupedScreens, orderedScreenList) {
                            var contents = [];
                            orderedScreenList.forEach(function (screen) {
                                for (var i = 0; i < groupedScreens.length; i++) {
                                    if (screen === groupedScreens[i].id) {
                                        contents.push(groupedScreens[i]);
                                        break;
                                    }
                                }
                            });
