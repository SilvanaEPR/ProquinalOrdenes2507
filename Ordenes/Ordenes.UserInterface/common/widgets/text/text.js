...
(function () {
    ...
    */
    ...
    app.directive('sitInputCompositionSupport', ['$timeout', function ($timeout) {
        return {
            require: 'ngModel',
            link: function (scope, element, attrs, modelCtrl) {
                var compupdateFn = function () {
                    $timeout(function () {
                        modelCtrl.$setViewValue(element.val(), 'compositionupdate');
                    });
                };
                element.on('compositionupdate', compupdateFn);
                scope.$on("$destroy", function () {
                    element.off('compositionupdate', compupdateFn);
                });
            }
        };
    }]);
