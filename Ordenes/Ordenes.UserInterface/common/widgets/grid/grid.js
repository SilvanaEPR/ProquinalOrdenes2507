...
(function () {
    ...
    * @name sitColumnHeaderIndicator
    * @restrict E
    * 
    * @description 
    * The directive is designed to display the svg icon for indicator header in grid
    */
    angular.module('siemens.simaticit.common.widgets.grid')
        .directive('sitColumnHeaderIndicator', function () {
            return {
                restrict: 'E',
                scope: {},
                controller: function () {
                    var vm = this;
                    vm.icon = {
                        path: vm.column.colDef.displayIcon,
                        size: '20px'
                    };
                },
                controllerAs: 'vm',
                ...
                template: '<em sit-mom-icon="vm.icon" sit-class="header-indicator-icon"/>'
            };
        });
})();
...
(function () {
    ...
    * @access internal
    * @module siemens.simaticit.common.widgets.grid
    * @name sitColumnIndicator
    * @restrict E
    * 
    * @description 
    * The diective is designed to manipulate the indicator visibility of grid rows
    */
    angular.module('siemens.simaticit.common.widgets.grid')
        .directive('sitColumnIndicator', function () {
            return {
                restrict: 'E',
                scope: {},
                controller: ['$element', function ($element) {
                    ...
                    vm.getIcon = function () {
                        var icon = '';
                        var display = false;
                        for (var i = 0; i < vm.column.indicators.length; i++) {
                            vm.title = vm.column.indicators[i].tooltip;
                            display = getVisibility(vm.column.indicators[i], vm.row);
                            icon = vm.column.indicators[i].svgIcon;
                    ...
                    function getVisibility(indicator, row) {
                        var isVisible = false;

                        switch (typeof indicator.visible) {
                            case 'function':
                                isVisible = indicator.visible(row);
                                break;
                            case 'boolean':
                                isVisible = indicator.visible;
                                break;
                            case 'string':
                                var funcationParam = 'row';
                                var functioBody = ["return", indicator.visible].join(' ');
                                try {
                                    isVisible = new Function(funcationParam, functioBody).call(row, row); //NOSONAR
                                } catch (e) {
                                    isVisible = false;
                                }
                                break;
                ...
                template: '<img src="{{vm.getIcon()}}" height="100%" width="20" title="{{vm.title}}"/>'
            };
        });
})();
                ...
                controller: function () {
                    this.getValue = function () {
                        ...
                        if (this.fieldSource !== "undefined" && this.fieldSource) {
                            var value = $parse(this.expression)(this.row.entity[this.fieldSource]);
                            return value === "undefined" || value === undefined || value === null ? this.row.entity[this.fieldSource[this.expression]] : value;
                        }
                        var parsedValue = $parse(this.expression)(this.row.entity);
                        return parsedValue !== undefined && parsedValue !== null ? parsedValue : this.row.entity[this.expression];
                    }
                },
...
(function () {
    ...
    GridDirective.$inject = ['$log', '$window', '$timeout', 'common.services.personalization.personalizationService'];
    function GridDirective($log, $window, $timeout, personalizationService) {
        return {
            restrict: 'E',
            scope: {},
            bindToController: {
        ...
        function linkFn(scope, element, attr, ctrl) {
            ...
            function subscribeEvents() {
                ...
                eventListners[1] = scope.$on('ngGridEventRows', onGridRowsChange);
                ...
                eventListners[eventListners.length] = scope.$on("ngGridEventColumnResize", onColumnResize);
                eventListners[eventListners.length] = scope.$on("sit-filter.resetColumnsWidth", resetGridColumnsWidth);
            ...
            function onColumnResize(evt, data) {
                ctrl.emitColumnResized(data);
            }

            function resetGridColumnsWidth() {
                ctrl.resetColumnsWidth();
            }
            ...
            function onGroupsChange(newVal, oldVal) {
                if (newVal !== oldVal) {
                    ...
                    if (newVal.length === 0) {
                        ctrl.serverItemsLoadCount = ctrl.initialDataItemsCount;
                        $(".ngCanvas").removeClass("group");
                    }
                    else {
                        $(".ngCanvas").addClass("group");
                        //subscribing grid rows change on group
                        eventListners[1] = scope.$on('ngGridEventRows', onGridRowsChange);
                    }
                    ctrl.totalServerItemsLoadedCount = 0;
            ...
            /**
            * Called when grid rows changed
            */
            function onGridRowsChange(event, rows) {
                //saving the grouped rows for downloading
                if (ctrl.sitGridOptions.parent !== 'ICV') {
                    eventListners[1]();
                    return;
                }
                if (ctrl.grouping) {
                    ctrl.sitGridOptions.gridGroupedData = rows;
                } else {
                    eventListners[1]();
                    ctrl.sitGridOptions.gridGroupedData = [];
                }
            }
            ...
            var ensureDigest = function (event) {
                ...
                if (scrollPercentage >= 95 && ctrl.totalServerItemsLoadedCount !== ctrl.totalDataItems
                    && !ctrl.grouping && !ctrl.isQuickSearched && !ctrl.isSorted && !ctrl.isFilterPanelOpen) {
            ...
            function onScroll(event) {
                ...
                scrollTimer = setTimeout(ensureDigest.bind(this, event), 50);
            }
    ...
    GridController.$inject = ['$scope', 'common', 'common.widgets.grid.service', 'common.widgets.pager.pageService', '$translate', '$timeout', '$state', 'LOG_TYPES', '$window', '$filter', '$element', '$q', 'common.services.personalization.personalizationService'];
    function GridController($scope, common, gridService, pageService, $translate, $timeout, $state, LOG_TYPES, $window, $filter, element, $q, personalizationService) {
        var vm = this;
        ...
        function activate() {
            ...
            init();
            generateCellTemplates(vm.sitGridOptions.columnDefs);
        ...
        function init() {
            ...
            vm.generateCellTemplates = generateCellTemplates;
        ...
        function initializeEventsEmitMethods() {
            ...
            vm.emitColumnResized = emitColumnResized;
            ...
            vm.resetColumnsWidth = resetColumnsWidth;
        }
        ...
        */
        ...
        function updateGridOptions(firstUpdate) {
            ...
            if (cachedColumnDefinition !== JSON.stringify(ngGridOptions.columnDefs)) {
                ...
                if (vm.sitGridOptions.userPrefId) {
                    //saving the original column width data
                    angular.forEach(ngGridOptions.columnDefs, function (colDef) {
                        colDef.originalColumnWidth = colDef.width;
                    });

                    vm.updatedColumnWidths = personalizationService.getCurrentUserPreference('sitGrid', vm.sitGridOptions.userPrefId + '_Grid');
                    if (vm.updatedColumnWidths) {
                        angular.forEach(ngGridOptions.columnDefs, function (colDef) {
                            if (vm.updatedColumnWidths.hasOwnProperty(colDef.field)) {
                                colDef.width = vm.updatedColumnWidths[colDef.field].width;
                            }
                        });
                    } else {
                        vm.updatedColumnWidths = {};
                    }
                }
        ...
        function resetColumnsWidth() {
            vm.updatedColumnWidths = {};
            personalizationService.setPersonalization('sitGrid', vm.sitGridOptions.userPrefId + '_Grid', vm.updatedColumnWidths);
            vm.gridOptions.gridApi.resetColumnWidths();
        }
        ...
        function generateCellTemplates(colDefs) {
            if (colDefs) {
                for (var index = 0; index < colDefs.length; index++) {
                    if (colDefs[index].dataType === 'boolean' && colDefs[index].showCheckbox === true) {
                        //cellTemplate for check-box when 'showCheckbox' is set to true

                        colDefs[index].cellTemplate = '<div class="sit-checkbox">'
                            + '        <div class="label label-property-grid-control-readonly checkbox-noborder">                           '
                            + '            <ng-form name="checkboxForm">                                                                    '
                            + '                <div class="property-grid-span-group-block">                                                 '
                            + '                    <div class="group-control property-grid-no-border-items">                                '
                            + '                        <div class="check-box checkbox-disabled">                                            '
                            + '                            <label>                                                                          '
                            + '                                <input type="checkbox" ng-checked="row.getProperty(col.field)"/>             '
                            ...
                            + '        </div>                                                                                               '
                            + '    </div>                                                                                                   ';
                    } else if (colDefs[index].indicators && colDefs[index].indicators.length) {
                        //cellTemplate for indicators

                        if (colDefs[index].displayIcon) {
                            colDefs[index].headerCellTemplate = "common/widgets/grid/indicator-header-template.html";
                            colDefs[index].width = '56px';
                            colDefs[index].resizable = false;
                        }
                        colDefs[index].cellTemplate = '<sit-column-indicator class="ngCellText" sit-row="row.entity" sit-column="col.colDef"></sit-column-indicator>'
        ...
        */
        ...
        function refreshGrid(resetPageCount) {
            ...
            (isNoPagerServerDataMode && !vm.grouping ? vm.pageManager.getServerData(vm.serverItemsLoadCount, vm.totalServerItemsLoadedCount) : (vm.grouping || !vm.sitGridOptions.enablePaging) ? vm.pageManager.getAllData() : vm.pageManager.getPageData()).then(
                function (result) {
                    ...
                    vm.sitGridOptions.noData = ($scope.slicedGridData.length) ? false : true;
        ...
        function setOptionsAPIMethods() {
            ...
            vm.sitGridOptions.evaluateExpressionColumns = evaluateExpressionColumns;
        }
        ...
        /**
         * @ngdoc event
         * @name sitGrid#sit-grid.column-resized
         * @eventType emit on sitGrid column header
         * @description
         * Emitted when the current width of a column is changed after the user resizes it.
         * 
         */
        function emitColumnResized(data) {
            $scope.$emit('sit-grid.column-resized');
            //Notify tagList Directive to resize.
            $scope.$broadcast('sit-grid.column-resized');
            //providing an option to save the resized column width
            if (!data) {
                return;
            }
            if (!vm.sitGridOptions.userPrefId) {
            ...
            if (vm.updatedColumnWidths.hasOwnProperty(data.field)) {
                vm.updatedColumnWidths[data.field].width = data.width;
            } else {
                vm.updatedColumnWidths[data.field] = data;
            }
            personalizationService.setPersonalization('sitGrid', vm.sitGridOptions.userPrefId + '_Grid', vm.updatedColumnWidths);
        }
    ...
    *
     ...
     * _(Optional)_ Defines the column header icon to be displayed.
     * The value must be a command svg icon path. The icon is displayed only if **indicators** property is configured.
     * 
     *  Example:
     * ```
     *   displayIcon: "common/icons/cmdInfo24.svg"
     ...
     *  _(Optional)_ A list of objects which contains the configuration to display an visual indication image in a row.
     * Each object of the array must have the following properties:
     *   * **svgIcon**: The name of the indicator svg icon to display as an indicator.
     *   * **visible **: This specifies whether the indicator is visible or not. This property can be set to an expression string or a function that will be evaluated to a respective grid row. The result should be a boolean value.
     *   * **tooltip**: The translated string to be displayed as a tooltip.
     *   
     *    Example:
     *    
     *    Indicator with expression visibility:
     * ```
     *   indicators: [{
     *                   svgIcon: "common/icons/indicatorRedCircle16.svg",
     *                   tooltip: "Archived Books",
     *                   visible: "row.yearPublished < 2000"
     *                }]
     * ```
     *   Indicator with functional visibility:
     * ```
     *   indicators: [{
     *                   svgIcon: "common/icons/indicatorGreenCircle16.svg",
     *                   tooltip: "Active Books",
