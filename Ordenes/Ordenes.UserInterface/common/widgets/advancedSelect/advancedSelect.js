...
(function () {
    ...
    function AdvancedSelectDirective($timeout, $window) {
        return {
            ...
            link: function (scope, element, attr, ctrl) {
                ...
                $timeout(function () {
                    elementHeight = element.find('.multi-select-dropdown').height();
                }, 100, false);
                ...
                if (ctrl.sitAlwaysOnTop) {
                    element.on('hide.bs.dropdown', unsubscribe);
                    element.on('show.bs.dropdown', rearrangeDropdown);
                }

                function closeDropdown() {
                    element.find('div.dropdown').removeClass('show');
                    element.find('#dropdownMenuAdvanced').attr('aria-expanded', false);
                    element.find('[aria-labelledby="dropdownMenuAdvanced"]').removeClass('show');
                }
                ...
                function dynamicPositioning() {
                    var zindex = 100005,
                        eOffset = element.offset(), // grab the new offset position
                        wid = element.find('#dropdownMenuAdvanced').outerWidth(true),
                        topValue = 0,
                        maxDropdownHeight,
                        totalDropItemHeight,
                        modal = $('.modal-backdrop.show');

                    if (dropdownMenu.css('maxHeight')) {
                        maxDropdownHeight = parseInt(dropdownMenu.css('maxHeight').replace(/[^0-9]/g, '')); // NOSONAR
                        totalDropItemHeight = dropdownMenu.outerHeight(true) > maxDropdownHeight ? maxDropdownHeight : dropdownMenu.outerHeight(true);

                        if (totalDropItemHeight + elementHeight + eOffset.top > windowHeight) {
                            topValue = eOffset.top - totalDropItemHeight;
                        } else {
                            topValue = eOffset.top + elementHeight;
                        }
                    }
                ...
                function rearrangeDropdown() {
                    dropdownMenu = element.find('[aria-labelledby="dropdownMenuAdvanced"]');
                    dropdownMenu.addClass('globalDropdown');
                ...
                function subscribe() {
                    element.parents().bind('scroll', onScrollEventHandler);
                    angular.element($window).bind('resize', onResizeEventHandler);
                }

                function unsubscribe() {
                    closeFilter();
                    element.parents().unbind('scroll', onScrollEventHandler);
                    angular.element($window).unbind('resize', onResizeEventHandler);
                }
    ...
    *     <sit-advanced-select
                       ...
                       sit-always-on-top="vm.sitAlwaysOnTop"
    ...
    */
    function AdvancedSelectController($translate, $scope, $element, $timeout) {
        ...
        function getTemplate() {
            ...
            //To support old version of bootstrap
            if ('4.6.1' === window.bootstrap.Tooltip.VERSION) {
                template = 'b4-' + template;
            }
            return 'common/widgets/advancedSelect/' + template + '-selection.html';
        }
