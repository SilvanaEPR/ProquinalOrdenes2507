...
(function () {
    ...
    * ```
    *
    * @param {string} [select] An expression called when tab is activated.
    * @param {string} [deselect] An expression called when tab is deactivated.
    *
    * */
        ...
        return {
            ...
            link: function (scope, elm, attrs, tabsetCtrl, transclude) {
                ...
                scope.$on('$destroy', function () {
                    ...
                    eventListner[eventListner.length - 1].off('touchend');
    ...
    * */
    ...
    function SitTabsetController($scope, $timeout, $element, $translate) {
        ...
        ctrl.select = function (selectedTab) {
            hideDropdown();
            var tabsets = $('.sit-navigation-tabs');
            var tabsetsLength = tabsets.length;
            var tabset = tabsets.first();
            var tabsetBodyContent = tabset.children('.tab-content').first();
            if (tabsetsLength === 1) { tabsetBodyContent.css('display', 'none'); }
            $timeout(function () {
                ...
                if (tabsetsLength === 1) { tabsetBodyContent.css('display', 'block'); }
            });
        };
        ...
        function hideDropdown() {
            ctrl.dropdownMenu && ctrl.dropdownMenu.length && ctrl.dropdownMenu.css({ visibility: "hidden", display: "block" });
        }

        function showDropdown() {
            ctrl.dropdownMenu && ctrl.dropdownMenu.length && ctrl.dropdownMenu.css({ visibility: "visible", display: "block" });
        }
    ...
    function tabset($window, $interval, $timeout) {
        return {
            ...
            link: function (scope, element, attrs, ctrl) {
                ...
                var htmlClickhandler = function () {
                    if (ctrl.dropdownMenu.is(':visible'))
                        ctrl.dropdownMenu.css({ visibility: 'hidden', display: 'block' });
                };

                var navbarNavDropdownClickHandler = function (event) {
                    event.stopPropagation();
                }

                $('html').on('click', htmlClickhandler);
                $('.navbar-nav-dropdown').on('click', navbarNavDropdownClickHandler);

                eventListner[eventListner.length] = htmlClickhandler;
                eventListner[eventListner.length] = navbarNavDropdownClickHandler;

                linkFunction();

                function linkFunction() {
                    var MIN_CONTAINER_WIDTH = 20;
                    var observer,tabElement;
                    scope.vertical = angular.isDefined(attrs.vertical) ? scope.$parent.$eval(attrs.vertical) : false;
                    scope.justified = angular.isDefined(attrs.justified) ? scope.$parent.$eval(attrs.justified) : false;

                    function getTabsetelement() {
                        tabElement = element.find('.navbar-nav-tab').first()[0];
                        if (!tabElement) {
                            window.setTimeout(getTabsetelement);
                            return;
                        }
                        if (element.width() > 0) {
                    ...
                    scope.$on('$destroy', function () {
                        ...
                        $('.navbar-nav-dropdown').off('click', navbarNavDropdownClickHandler);
                        while (eventListner.length) { 
                            eventListner.pop();
                        }
                    });
