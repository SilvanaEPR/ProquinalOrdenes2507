...
(function () {
    ...
    *
    * @example
    * ### Using the sit-dropdown-mode attribute in sit-select widget
    *
    * The following template shows how to display combobox dropdown in {@link sitSelect sit-select} widget:
    *
    *  ```
    *   <sit-property sit-widget="sit-select"
    *                 sit-value="vm.textSelect.value"
    *                 sit-validation="vm.textSelect.validation"
    *                 ng-readonly="vm.textSelect.readOnly"
    *                 sit-change="vm.textSelect.onTextSelectChanged"
    *                 sit-dropdown-mode="comboBox"
    *                 sit-widget-attributes="vm.widgetAttributesTextSelect">Combo Box</sit-property>
    *  ```
    *
    */

    angular.module('siemens.simaticit.common.widgets.propertyGrid').directive('sitProperty', ['$parse', '$timeout', PropertyDirective]);

    function PropertyDirective($parse,$timeout) {
        var sitProperty = {
            scope: {},
            transclude: true,
            ...
            bindToController: {
                ...
                sitAllowReset: '=?',
                sitMultiChange: '=?',
                sitDropdownMode: '@sitDropdownMode'
            },
            link: function (scope, elmnt, attrs, ctrls, transclude) {
                ...
                if (parent.mode === 'view' && typeof vm.readOnly !== 'boolean') {
                    vm.readOnly = true;
                }
    ...
    function PropertyCtrl($filter, propertyGridListService) {
        ...
        function init() {
            ...
            vm.getTrackByValue = getTrackByValue;
            ...
            if (vm.sitWidget === 'sit-key-value' && vm.value && vm.value.length) {
        ...
        function getTrackByValue(item, index) {
            return item.hasOwnProperty('uuid') ? item.uuid : index;
        }
...
(function () {
    ...
    function PropertyGridService(propertyGridListService) {
                        ...
                        if (!isInsideGroup) {
                            ...
                            if (obj.type === 'group') {
                                ...
                                if (!isDivOpened) {
                                    isDivClosed = false;
                                    isDivOpened = true;
                                    concat += '<div class="group-horizontal-mode"><div class="horizontal-mode">'
                                }
                                concat += createPropertyHtml(obj, obj.index, layout, type, isViewMode, isInsideGroup);
                            }
                        }
        ...
        function createPropertyHtml(widgetObject, key, layout, type, isViewMode, isInsideGroup) {
            ...
            if (typeof propertyListValue === 'string' && displayMode === 'list') {
                propertyListValue = propertyListValue.safeReplaceAll('<', '&lt;').safeReplaceAll('>', '&gt;');
            }
            ...
            concat += "<div class='" + (isViewMode && displayMode === 'list' ? "pl-value " : "") +                
                getPropertyValueClass(layout, type, isInsideGroup) + "' title= '" + propertyListValue + "'" +
                (isViewMode && displayMode === 'list' ? " ng-non-bindable" : "") +
                " style='" + (isViewMode && displayMode === 'list' ? "white-space:pre-wrap; word-break:break-word" : "")+"'>";
                ...
                if (widget.widget !== 'sit-entity-picker') {
                    ...
                    switch (typeo) {
                        ...
                        case "string":
                            ...
                            break;
                        case "boolean":
                            if (widget.widget !== 'sit-toggle-button') {
                                type = "sit-checkbox";
                    ...
                    concat += "sit-title='" + dataToBind + ".widgetAttributes.title' ";
                    concat += "ng-readonly='" + read_only + "' ";
                    concat += "ng-disabled='" + disabled + "'";
                    concat += "class='property-grid-input-group' ";
                    concat += ">";
    ...
    function PropertyItemDirective($compile) {
        ...
        function preLinkFn(scope, element, attrs, ctrl) {
            var widgetTag = ' sit-name="{{PropertyItemCtrl.name}}"' + 'sit-id="' + attrs.sitId + '"' +
                    ...
                    ' sit-done-callback="PropertyItemCtrl.sitDoneCallback()" sit-placeholder="PropertyItemCtrl.placeholder" sit-always-on-top="PropertyItemCtrl.sitAlwaysOnTop"'+
                    ...
                    ' ng-readonly="PropertyItemCtrl.ngReadonly" class="' + attrs.class + '"' + ' sit-allow-reset="PropertyItemCtrl.sitAllowReset" sit-dropdown-mode="{{PropertyItemCtrl.sitDropdownMode}}"';
...
(function () {
    ...
    function PropertyGridListService($filter) {
        ...
        function parseValue(property) {
            ...
            function radioList() {
                property.options && property.options.length && (options = property.options); // NOSONAR
                property.widgetAttributes && property.widgetAttributes['sit-options'] &&
                    property.widgetAttributes['sit-options'].length && (options = property.widgetAttributes['sit-options']); // NOSONAR
