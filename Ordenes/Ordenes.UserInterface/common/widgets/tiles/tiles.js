...
(function () {
    ...
    function filterSort($parse, $log) {
        ...
        function compareAlpha(a, b) {
            var strA = a && a.toLowerCase() || '',
                strB = b && b.toLowerCase() || '';
            if (strA === strB) {
                return 0;
            }
            else if (strA < strB) {
         ...
         var fieldInfo,
            ...
            data.sort(function (itemA, itemB) {
                ...
                while (retVal === 0 && index < sortInfo.length) {
                    ...
                    } else if (propA === undefined || propB === undefined) { // we want to allow zero values to be evaluated in the sort function
                        // push bad data to end of list
                        if (propB === undefined && propA === undefined) {
                            retVal = 0;
                        }
        ...
        function getCompareFunc(fieldInfo, data) {
            var compareFunc, item,expression,valueOfField;
            ...
            if (sortService.compareFuncCache[fieldInfo.field]) {
                ...
                expression = $parse('obj[\'' + fieldInfo.field.replace(REGEX_DOT, '\'][\'') + '\']');
                valueOfField = expression({ obj: item });
                if(!valueOfField){
                    var value;
                    var notNullData = _.find(data, function (dataItem) {
                        value = expression({ obj: dataItem });
                        return ( value !== null && value !== undefined);
                    });
                    compareFunc = sortService.guessCompareFunc(expression({ obj: notNullData}));

                } else {
                    compareFunc = sortService.guessCompareFunc(valueOfField);
                }
...
(function () {
    ...
    * ```
    * @property {Object} [descriptionFieldFilter=undefined] _(Optional)_
    * Specifies the data formatting configuration to be applied on the description.
    * The property supports **date** type formatting only.
    * This property can used only when the type of **descriptionField** is **date** and the diplay value needs formatting.
    * 
    * The configuration object is shown below:
    *   * **datatType**: The type of the **descriptionField**. It supports **date** type only.
    *   * **format**: _(Optional)_ The date-time format. If not specified all the values formatted to 'mediumDate'.
    * Example:
    * ```
    *   {
    *       dataType: 'date',
    ...
    * ```
    *
    * @property {Object} [titleFieldFilter=undefined] _(Optional)_
    * Specifies the data formatting configuration to be applied on the title.
    * The property supports **date** type formatting only.
    * This property can used only when the type of **titleField** is **date** and the diplay value needs formatting.
    *
    * The configuration object is shown below:
    *   * **datatType**: The type of the **titleField**. It supports **date** type only.
    *   * **format**: _(Optional)_ The date-time format. If not specified all the values formatted to 'mediumDate'.
    * Example:
    * ```
    *   {
    ...
    function ItemTileController(scope, tileService, $state, $filter, $sanitize, $compile, $timeout, $element, tagsManagementService) {
        ...
        function activate() {
            ...
            if (vm.tileOptions) {
                ...
                if (vm.tileContent && vm.tileContent.svgIcon && typeof (vm.tileContent.svgIcon) === 'string') {
                    newIcon = {
                        path: vm.tileContent.svgIcon,
                        size: '32px'
                    }
                    vm.tileContent.svgIcon = newIcon;
                } else {
                    if (vm.tileContent && vm.tileContent.typeIcon && typeof (vm.tileContent.typeIcon) === 'string') {
                        newIcon = {
                            prefix: 'type',
                            name: vm.tileContent.typeIcon,
                            suffix: '48',
                            size: '32px'
                        }
                        vm.tileContent.typeIcon = newIcon;
                    }
                }
                ...
                if (vm.tileOptions.commands || vm.tileOptions.indicators) {
                    vm.tileContent.updateCommandsIndicators = updateCommandsIndicators;
                }
                configureCommands();
                configureIndicators();
            }
            ...
            var description = vm.tileContent.descriptionField
                ? tileService.getFieldValue(vm.tileContent.descriptionField, vm.tileContent, vm.tileContent.descriptionFieldFilter)
                : vm.tileContent.description;
        ...
        function updateCommandsIndicators() {
            configureCommands();
            configureIndicators();
            scope.$broadcast('tileHeightUpdated');
        }
        ...
        function getDisplayText(textType, index) {
            ...
            if (textType === 'description') {
                var descriptionFiled = vm.tileContent.descriptionField
                    ? vm.tileContent.descriptionField.field || vm.tileContent.descriptionField
                    : null;
                var descriptionFilterInfo = vm.tileContent.descriptionFieldFilter;
                actualText = descriptionFiled
                    ? tileService.getFieldValue(descriptionFiled, vm.tileContent, descriptionFilterInfo)
                    : vm.tileContent.description;
                displayText = actualText;
                if (vm.tileContent.size === 'large') {
                ...
                var titleField = vm.tileContent.titleField
                    ? vm.tileContent.titleField.field || vm.tileContent.titleField
                    : null;
                var titleFilterInfo = vm.tileContent.titleFieldFilter;
                actualText = titleField
                    ? tileService.getFieldValue(titleField, vm.tileContent, titleFilterInfo)
                    : vm.tileContent.title;
                if (vm.tileContent.size === 'large') {
                ...
                if (vm.tileContent.propertyFields && vm.tileContent.propertyFields.length === 0) {
                    ...
                    if (vm.tileContent.size === 'large') {
                        ...
                        displayText = property && property.value ? property.value : ' ';
                    }
                }
...
(function () {
    ...
    */
    angular.module('siemens.simaticit.common.widgets.tiles').directive('sitLastTile', function () {
        return {
            restrict: 'A',
            link: function (scope) {
                if (scope.$last) {
...
(function () {
    ...
    */
    ...
    TileGroupController.$inject = ['$scope', 'common.widgets.tiles.tileService'];
    function TileGroupController($scope, tileService) {
        var vm = this, initialTileLimit;
        activate();
        ...
        function lazyRenderingTileLimitLogic() {
            if (vm.enablePaging) {
                vm.tileLimit = vm.group.tiles.length;
            } else {
                initialTileLimit = tileService.tileLimit;
                vm.tileLimit = tileService.tileLimit;
                $scope.$on("changeLimit", changeLimit);
            }            
        }

        function changeLimit(event, data) {
            vm.tileLimit = initialTileLimit + data;
            $scope.$apply();
        }
    }
})();
   ...
   */
    function TileHeightDirective($timeout) {
        ...
        function link(scope, element) {
            var setHeight, updateHeight;
            setHeight = $timeout(function () {
                var currentElement = element;
                var currentIndicatorSectionHeight = 0;

                var elems = $(element).parents().eq(1).find('div.tile');
                if ($(currentElement).find('div.indicator-section').height() > 0) {
                    currentIndicatorSectionHeight = $(currentElement).find('div.indicator-section').height();
                }
                for (var i = 0; i < elems.length; i++) {
                    if (currentIndicatorSectionHeight > ($(elems[i]).find('div.indicator-section').height() || 0)) {
                        $(elems[i]).find('div.indicator-section').height(currentIndicatorSectionHeight);
                    }
            ...
            scope.$on('tileHeightUpdated', function () {
                updateHeight = $timeout(function () {
                    var commonHeight = 0;
                    var elems = $(element).parents().eq(1).find('div.tile');
                    elems.each(function () {
                        $(this).height('auto');
                    })
                    var indicatorTile = $(element).parents().eq(1).find('div.indicator-icons');
                    if (!indicatorTile.length) {
...
(function () {
    ...
    TileManager.prototype = {
        ...
        updateFacetFields: function (commonData, serverDataOptions) {
            if (window.$UIF.Object.safeGet(serverDataOptions, 'fieldSources')) {                
                window.$UIF.Object.safeGet(serverDataOptions, 'fieldSources.'+commonData.titleFieldSource+'') && (commonData.titleField = commonData.titleFieldSource + '.' + commonData.titleField);
                window.$UIF.Object.safeGet(serverDataOptions, 'fieldSources.' + commonData.descriptionFieldSource + '') && (commonData.descriptionField = commonData.descriptionFieldSource + '.' + commonData.descriptionField);
                commonData.propertyFields.forEach(function (property) {
                    if (property.fieldSource && window.$UIF.Object.safeGet(serverDataOptions, 'fieldSources.' + property.fieldSource + '')) {
                        property.field = property.fieldSource + '.' + property.field;
                    }
                });
            }
        },
        ...
        getFieldValue: function (field, obj, info) {
            ...
            var evalValue = evalFunc(obj);
            var value = evalValue !== undefined ? evalValue : obj[field];

            if (info && info.dataType === 'date' && moment(value).isValid()) {
                return this.filter('date')(new Date(value), info.format || 'mediumDate');
            }
            return value;
        }
    };
...
(function () {
     ...
     * 
     * @property {String} [groupByFieldSource=undefined] _(Optional)_  The unique alias key configured in **fieldSources** of serverDataOptions.
     * This property is configured if the **groupBy** property belongs to the facet type extension. This property is considered only when server-side configuration is used.    
     *
     *
     * @property {String} svgIcon _(Optional)_
     ...
     * collection is filtered using a **begins with** and **contains** match on the specified data field.
     ...
     * * **fieldSources** : (Optional) An object to define the reference of the logical extensions used in **dataEntity**.
    *  This information is used by the widget to process, transform the structure and display the data retrieved from the logical extensions.
    ...
    *      {
    *        unique_alias_name: {
    *                              name: 'fully qualified Reading Modal name of facet',
    *                              type: 'facet'
    *                           }
    *      }
    * ```
    * 
    * The **unique_alias_name** object key holds the unique identifier value for **fieldSources** entry.
    *
    * The **name** property should be assigned with fully qualified reading modal name.
    *
    ...
    * ```
     ...
     *  titleField: 'properties.name'
     * ```
     * @property {String} [titleFieldSource=undefined] **titleFieldSource**: (Optional) The unique alias key configured in **fieldSources** of serverDataOptions.
     ...
     *  descriptionField: 'myCustomData.description',
     * ```
     * @property {String} [descriptionFieldSource=undefined] **descriptionFieldSource**: (Optional) The unique alias key configured in **fieldSources** of serverDataOptions.
     ...
     * ```
     * propertyFields: [
     *     {    field: 'title', displayName: 'TileTitle', fieldSource:'Ftitle' },
     *     {    field: 'description' }
     * ]
     * ```
     * **field** Specifies the field name to be used as the property in the tile.
     * It also supports field names with **dot** notation for displaying text in tiles.
     *
     * **displayName** Specifies the label to be displayed for the property.
     * 
     * **fieldSource** (Optional) The unique alias key configured in **fieldSources** of serverDataOptions.
        ...
        function linkFn(scope, element, attrs, ctrl) {
            ...
            function sortInfoChangeCallback(newVal, oldVal) {
                if (newVal.field !== oldVal.field || newVal.direction !== oldVal.direction) {
                    ...
                    ctrl.getPageManager().setSortInfo(newVal.field, newVal.direction, newVal.fieldSource);
                    ctrl.isSorted = true;
                    ctrl.serverItemsLoadCount = ctrl.initialDataItemsCount;
                    ctrl.totalServerItemsLoadedCount = 0;
            ...
            function onTilesChange(newVal, oldVal) {
                ...
                if (sorting) { sorting = false; }
                    ...
                    ctrl.setPageManager();
                    if (ctrl.filterClauses && ctrl.setFilter) {
                        ctrl.setFilter(ctrl.filterClauses);
                    }
                    ctrl.updateIcons();
                    ctrl.buildSelectedList();
                    deActivateWatchOnCollection();
                    ctrl.getPageManager().setSortInfo(ctrl.options.sortInfo.field, ctrl.options.sortInfo.direction, ctrl.options.sortInfo.fieldSource);
            ...
            eventListners[eventListners.length] = scope.$on('sit-filter-panel-opened', onfilterPanelChange);
            function onfilterPanelChange() {
                ctrl.isFilterPanelOpen = true;
            }

            eventListners[eventListners.length] = scope.$on('sit-dialog-layout-change', onDialogWindowResize);

            eventListners[eventListners.length] = scope.$on('$destroy', onDirectiveDestroy);
            ...
            function onScroll(event) {
                ...
                var loadedTile = document.getElementsByTagName('sit-item-tile')[0];
                if (loadedTile) {
                    var newLimit = Math.ceil(scrollTop / loadedTile.offsetHeight) * Math.ceil(ctrl.tilesContainerWidth / loadedTile.offsetWidth);
                    scope.$broadcast("changeLimit", newLimit);
                }
            }
            function onGroupScroll(event) {
                var scrollTop = event.target.scrollTop;
                var newGroupLimit = Math.ceil(scrollTop / 30);
                ctrl.tileGroupLimit = ctrl.firstGroupLimit + newGroupLimit;
                scope.$apply();
            }
            if (ctrl.getPageManager().isServerData() && !ctrl.options.enablePaging) {
                ...
                element.find('.tile-container').bind('scroll', onGroupScroll);
            }
            ...
            function onDirectiveDestroy() {
                ...
                element.find('div[data-internal-type=tileContainerDiv]').unbind('scroll', onScroll);
                element.find('.tile-container').unbind('scroll', onGroupScroll);
    ...
    function TileViewController($rootScope, $scope, tileService, $state, pageService, $log, $timeout, $q, $translate, $element, $document) {
        var ctrl = this;
        var logger, currentPageSelectedItems, currentPageManagerSettings = {};
        ...
        function activate() {
            ...
            setPageManager();
            ctrl.options.sortInfo.field = ctrl.getPageManager().getSortInfo().field || ctrl.options.sortInfo.field;
            ctrl.options.sortInfo.direction = ctrl.getPageManager().getSortInfo().direction || ctrl.options.sortInfo.direction;
            ctrl.options.sortInfo.fieldSource = ctrl.getPageManager().getSortInfo().fieldSource || ctrl.options.sortInfo.fieldSource;
            ctrl.getPageManager().setSortInfo(ctrl.options.sortInfo.field, ctrl.options.sortInfo.direction, ctrl.options.sortInfo.fieldSource);
            updateIcons();
            updateTileOptions();
            buildSelectedList();
        ...
        function init() {
            ...
            ctrl.firstGroupLimit = 30;
            ctrl.tileGroupLimit = 30;
            ctrl.loadingMessage = $translate.instant('common.busy-message.loading');
            ctrl.noValue = $translate.instant('common.noValue');
            ctrl.displayData = [];
            ctrl.selectedItems = [];
        ...
        function setServerSideTileCount() {
            ...
            ctrl.tilesContainerWidth = ctrl.tilesContainerWidth || $element.parent().width();
            if (ctrl.getPageManager().isServerData() && ctrl.options.enablePaging === false) {
                ...
                tileService.tileLimit = ctrl.serverItemsLoadCount;
                ctrl.firstGroupLimit = Math.ceil(ctrl.tileContainerHeight / 30) + 2;
                ctrl.tileGroupLimit = ctrl.firstGroupLimit;
        ...
        function setFilterClauses() {
            ctrl.filterClauses = ctrl.getPageManager().getFilter();
        }
            ...
            try {
                setDisplayData().then(function () {
                    if (ctrl.options.groupBy && !ctrl.options.isCompactMode) {
                        ...
                        groupBy = ctrl.options.groupByFieldSource ? ctrl.options.groupByFieldSource + '.' + groupBy : groupBy;
                        angular.forEach(ctrl.displayData, function (tile) {
                            //To do:can this be more efficient?
                            var groupName = getGroupByValue(groupBy, tile);
                            var groupByValue = (groupName || groupName === false) ? groupName : ctrl.noValue;
                    ...
                    .finally(
                        function () {
                            ...
                            /* Bug Fix 307392 - Saved filter not loaded
                             *Requesting data with saved filter and requesting initial widget data are perfomed at the same time
                             * Due to this concurrency, one request will be flushed. Its a saved filter request always
                             * A comparision made with pervious filter state and the current filter state.
                             * if mismatch found,data request will be initiated again
                             */
                            ...
                            if (isServerData
                                ...
                                || (currentPageManagerSettings.currentFilter !== stringifyClauses(ctrl.filterClauses))) {
                                ctrl.totalServerItemsLoadedCount = 0;
                                ctrl.isQuickSearched = true;
                                createGroups();
                            }
        ...
        function setDisplayData() {
            ...
            var deferred = $q.defer();
            if (ctrl.filterClauses && ctrl.filterClauses.length) {
                ctrl.getPageManager().setFilter(ctrl.filterClauses);
            }
                ...
                .then(
                    function (result) {
                        if (isNoPagerServerDataMode && (result.data.length < ctrl.serverItemsLoadCount || !result.data.length)) {
                            ctrl.totalDataItems = ctrl.totalServerItemsLoadedCount + result.data.length;
                        ...
                        checkNoData(ctrl.displayData.length);
        ...
        function updateTileCommonData(resetDisplayFields) {
            ...
            var optionsCopy = angular.copy(ctrl.options);
            // define the common values
            var commonData = {
                bgColor: optionsCopy.bgColor,
                bgColorSelected: optionsCopy.bgColorSelected,
                color: optionsCopy.color,
                colorSelected: optionsCopy.colorSelected,
                descriptionField: optionsCopy.descriptionField,
                descriptionFieldFilter: optionsCopy.descriptionFieldFilter,
                descriptionFieldSource: optionsCopy.descriptionFieldSource,
                propertyFields: optionsCopy.propertyFields,
                selectStyle: optionsCopy.selectStyle,
                size: tileService.getTileSize(optionsCopy.tileType, optionsCopy.tileSize, null, 3, 3),
                titleField: optionsCopy.titleField,
                titleFieldFilter: optionsCopy.titleFieldFilter,
                titleFieldSource: optionsCopy.titleFieldSource,
                useCustomColors: optionsCopy.useCustomColors
            };
            if (ctrl.getPageManager().isServerData()) {
                tileService.updateFacetFields(commonData, ctrl.getPageManager().getServerConfiguration());
            }
        ...
        function selectAll(state) {
            ...
            if (ctrl.options.groupBy) {
                angular.forEach(ctrl.groups, function (tileGroup, index) {
                    var tileGroupData = $document.find('sit-tile-group').find(':checkbox')[index];
                    if (tileGroupData) {
                        tileGroupData.checked = state;
                    }
                });
            }
            notifySelectionChanged(null, true);
        ...
        function setFilter(clauses) {
            ...
            if (ctrl.serverItemsLoadCount === 0 && !ctrl.options.enablePaging && ctrl.getPageManager().isServerData()) {
                ctrl.setServerSideTileCount();
            } else {
                ctrl.createGroups();
            }
