...
(function () {
    ...
    * @param {String} [sit-title= undefined] _(Optional)_ Title of the entity picker widget. If undefined, by default title will be 'Select Item'.
    ...
    *                    sit-title="ctrl.entityPickerConfig.title"
    ...
    **/
    ...
    EntityPickerController.$inject = ['$scope', '$translate', '$element', 'common', 'LOG_TYPES', 'LOG_LEVELS', '$filter', '$window', '$timeout', 'common.widgets.dialog.service', '$parse'];
    function EntityPickerController($scope, $translate, $element, common, LOG_TYPES, LOG_LEVELS, $filter, $window, $timeout, dialogService, $parse) {
        ...
        function init() {
            ...
            vm.dialogTitle = vm.title ? $filter('translate')(vm.title) : $filter('translate')('entityPicker.selectItem');
            ...
            vm.uniqueId = vm.attributeToSearch;
                ...
                onClickCallback: function () {
                    ...
                    if (vm.sitoption) {
                        ...
                        $scope.$emit('sit-entity-picker.entity-selected', {
                            ...
                            label: safeVal(vm.selectedObject, vm.selectedAttributeToDisplay)
                        });
                    }
        ...
        function initICVOptions() {
            ...
            if (vm.sitoption) {
                vm.uniqueId = vm.sitoption.uniqueID ? vm.sitoption.uniqueID : vm.uniqueId;
                vm.sitoption.selectionMode = 'single';
                vm.sitoption.viewMode = 'g';
                vm.sitoption.viewOptions = 'g';
                vm.sitoption.downloadOptions = {
                    enabled: false
                };
            ...
            vm.isICVServerSide = false;
            if (vm.sitoption && vm.sitoption.serverDataOptions) {
                vm.isICVServerSide = true;
                vm.icvServerModeStatus = {
                    rowSelectionRequired: false,
                    rowSelected: false
                };
                var originalDataService = vm.sitoption.serverDataOptions.dataService;
                vm.sitoption.serverDataOptions.dataService = {
                    getAll: function (entityName, options) {
                        return originalDataService.getAll(entityName, options).then(function (data) {
                            if (!data.value || !vm.icvServerModeStatus.rowSelectionRequired || vm.icvServerModeStatus.rowSelected) {
                                return data;
                            }
                            var searchResult = search(data.value, vm.icvServerModeStatus.search, vm.uniqueId);
                            if (searchResult.length > 0) {
                                vm.icvServerModeStatus.rowSelected = true;
                                //searchResult[0].selected = true;
                                vm.selectedObject = searchResult[0];
                                $timeout(function () {
                                    vm.sitoption.selectItems([vm.selectedObject], true);
                                });
                            }
                            return data;
                        });
                    ...
                    findAll: function (request) {
                        return originalDataService.findAll(request).then(function (data) {
                            if (!data.value || !vm.icvServerModeStatus.rowSelectionRequired || vm.icvServerModeStatus.rowSelected) {
                                return data;
                            }
                            var searchResult = search(data.value, vm.icvServerModeStatus.search, vm.uniqueId);
        ...
        function showPopup() {
            ...
            if (!vm.icvObject.options.containerID) {
                vm.icvObject.options.containerID = 'globalDialogId-entity-picker-popup';
            }
            if (!vm.value) {
                openDialog(isObjectSelected);
                return;
            }

            var searchObj = typeof vm.value === 'object' ? safeVal(vm.value, vm.uniqueId) : vm.value;
            //incase of local data source
            if (vm.isLocalDatasource) {
                var searchResult = search(vm.datasource, searchObj, vm.uniqueId);
                if (searchResult.length > 0) {
                    searchResult[0].selected = true;
            ...
            //load data when required with ICV as server side data
            if (vm.isICVServerSide) {
                vm.icvServerModeStatus = {
                    rowSelectionRequired: false,
                    rowSelected: false
                };
            ...
            //incase of lazy loading with icv as local data
            vm.datasource('').then(function (result) {
                var searchResult = search(result, searchObj, vm.uniqueId);
                if (searchResult.length > 0) {
                    searchResult[0].selected = true;
                    isObjectSelected = true;
                    vm.selectedObject = searchResult[0];
        ...
        function openDialog(isObjectSelected) {
            var dialogData = {
        ...
        function search(data, viewValue, attributeToSearch) {
            if (!attributeToSearch) {
                return $filter('filter')(data, viewValue);
            }

            var predicate = {};
            $parse(attributeToSearch).assign(predicate, viewValue);
            return $filter('filter')(data, predicate);
        }
        ...
        function safeVal(content, expression) {
            ...
            for (var i = 0; i < expressionParts.length; i++) {
                if (!content.hasOwnProperty(expressionParts[i])) {
                    return null;
                }
                content = content[expressionParts[i]];
            }
            return content;
            */
        }
    ...
    entityPickrerSearchFilter.$inject = ['$filter', '$parse'];
    function entityPickrerSearchFilter($filter, $parse) {
        return search;
