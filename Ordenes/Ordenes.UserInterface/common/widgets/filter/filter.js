...
(function () {
   ...
   * When the UI is first shown, all elements of the **sitFilterFields** list are checked
    ...
    */
    ...
    FilterController.$inject = ['$translate', '$scope', '$timeout', 'common.services.logger.service', 'common.services.filterPersonalization.filterPersonalizationService',
        'common.widgets.globalDialog.service', 'common.widgets.messageOverlay.service', 'common.services.runtime.backendService'];
    function FilterController($translate, $scope, $timeout, logger, filterPersonalizationService, globalDialogService,
        messageOverlayService, backendService) {
        var ctrl = this;
        ...
        function setDefaultValue(clause) {
            ...
            if (!clause.showValueField) {
                ...
                } else if (clause.filterField.selectValues && clause.filterField.selectValues.length && typeof clause.filterField.selectValues !== 'function') {
                    clause.selectValue = clause.filterField.selectValues[0];
                    clause.value = clause.filterField.selectValues[0].id;
        ...
        function reset() {
            ...
            var commandsVisibilityList = [
                { name: 'setDefault', isVisible: false },
                { name: 'unsetDefault', isVisible: false },
                { name: 'delete', isVisible: false }
            ];
            commandsVisibilityList.forEach(function (item) {
                handleCommandButtonVisibility(item.name, item.isVisible);
            });
        }
        ...
        function initialize() {
            ...
            if (ctrl.sitFilterFields) {
                ctrl.sitFilterFields.forEach(function (filterField) {
                    if (filterField.widget === 'sit-select') {
                        filterField.alwaysOnTop = true;
                    }
                    if (filterField.hasOwnProperty("dataSource") && Object.keys(filterField.dataSource).length > 0) {
                        filterField.selectValues = [{ id: '', name: '' }];
                        if (filterField.widget !== 'sit-entity-picker') {
                            updateDataSource(filterField, function (response) {
                                updateFilter(response, filterField);
                                updateSelectValue(filterField);
                            });
                        } else {
        ...
        function updateFilter(response, filterField) {
            // todo:unnecessary hasOwnProperty check.
            //todo: remove else if condition as response from the server is always an array
            if (filterField.widget === 'sit-entity-picker') {

                filterField.selectValues = function (searchString) {
                    var finalQuery = '';
                    var displayProp = filterField.dataSource.displayProperty;
                    var optionString = "$filter=contains(" + displayProp + ",'" + searchString + "')";
                    if (searchString && displayProp && filterField.dataSource.query) {
                        finalQuery = finalQuery + '&' + optionString;
                    } else if (searchString && displayProp) {
                        finalQuery = optionString;
                    } else if (filterField.dataSource.query) {
                        finalQuery = filterField.dataSource.query;
                    }
                    if (filterField.dataSource.hasOwnProperty("selectedEntity")) {
                        // fetch the data from query result
                        return filterField.dataService.findAll({
                    ...
                    } else if (filterField.dataSource.hasOwnProperty("readingFunction")
                        && Object.keys(filterField.dataSource.readingFunction).length > 0) {
                        //fetch data from reading function
                        return backendService.read({
                            appName: filterField.appName,
                            functionName: filterField.dataSource.readingFunction.name,
                            params: filterField.dataSource.readingFunction.params,
                            options: finalQuery
                        }).then(function (response) {
                            return response.value;
                        });
            ...
            } else if (response.hasOwnProperty('value') && Array.isArray(response.value)) {
                filterField.selectValues = [];
            ...
            //TODO: is value column necessary for server request?
            if (filterField.hasOwnProperty('valueColumns') && Array.isArray(filterField.valueColumns)) {
                filterField.selectValueColumns = [];
                if (filterField.type === 'enum') {
                    //no value column support for enum type
        ...
        function updateSelectValue(filterField) {
            //update the clause value for enum after async request succeeded
            ctrl.clauses.forEach(function (clause) {
                if (clause.filterField.field === filterField.field && filterField.selectValues.length) {
                    clause.selectValue = filterField.selectValues[0];
                }
            });
        }

        function updateDataSource(filterField, onLoadSuccess) {
            if (filterField.dataSource.hasOwnProperty("selectedEntity")) {
                // fetch the data from query result
         ...
         * @ngdoc method
         * @module siemens.simaticit.common.widgets.filter
         * @name FilterOptions#refresh
         *
         * @description
         * An API method to re-request the configured server calls to update filter-field data.
         */
        function refresh() {
            //update all server requests
            if (ctrl.sitFilterFields) {
                ctrl.sitFilterFields.forEach(function (filterField) {
                    if (filterField.hasOwnProperty("dataSource")
                        && Object.keys(filterField.dataSource).length > 0 && filterField.widget !== 'sit-entity-picker') {
                        updateDataSource(filterField, function (response) { updateFilter(response, filterField) });
                    }
                });
            }
        }
        ...
        function setAPIMethods() {
            ...
            ctrl.sitFilterOptions.refresh = ctrl.refresh;
        }
        ...
        function handleCommandButtonVisibility(buttonName, isVisible) {
            if (!buttonName) {
                return;
            }
            if (ctrl.commandButtons && ctrl.commandButtons.bar && ctrl.commandButtons.bar.length) {
                for (var i = 0; i < ctrl.commandButtons.bar.length; i++) {
                    if (ctrl.commandButtons.bar[i].name === buttonName) {
                        ctrl.commandButtons.bar[i].visibility = isVisible;
                        break;
                    }
                }
            }
        ...
        function initFilterPersonalization() {
            ctrl.save = save;
            ctrl.saveAs = saveAs;
            ctrl.setFilterAsDefault = setFilterAsDefault;
            ctrl.clear = clear;
            ...
            ctrl.commandButtons = {
                ...
                "bar": [
                    ...
                    },
                    {
                        ...
                        "tooltip": $translate.instant('commandBar.setDefaultTooltip'),
                        'onClickCallback': ctrl.setFilterAsDefault
                    },
                    {
                        "type": "Command",
                        "name": "unsetDefault",
                        ...
                        "tooltip": $translate.instant('commandBar.unsetDefaultTooltip'),
                        'onClickCallback': ctrl.setFilterAsDefault
                    },
                    {
                        "type": "Command",
            ...
            ctrl.saveAsDialogButtons = [
                {
                    ...
                    onClickCallback: function (value) {
                        ...
                        if (index > -1) {
                            ...
                            var commandsVisibilityList = [
                                { name: 'setDefault', isVisible: true },
                                { name: 'unsetDefault', isVisible: false },
        ...
        function clearFilterDataService() {
            if (ctrl.filterData && ctrl.filterData.filterClauses) {
        ...
        function isDefaultFilter(filterName) {
            ...
            if (ctrl.savedFilters && ctrl.savedFilters.length) {
                var filter = ctrl.savedFilters.filter(function (filterData) {
        ...
        function refreshSavedFilterDefaultSelection() {
            ctrl.commandButtons.bar[0].group.forEach(function (element) {
                element.selected = isDefaultFilter(element.name);
            });
        }

        function setFilterAsDefault() {
            var isDefault = (arguments[0] && arguments[0].name === 'setDefault');
            getSavedFilters();
            var commandsVisibilityList = [
                { name: 'setDefault', isVisible: !isDefault },
                { name: 'unsetDefault', isVisible: isDefault }
            ];
            commandsVisibilityList.forEach(function (item) {
                handleCommandButtonVisibility(item.name, item.isVisible);
            });
            var index = _.findIndex(ctrl.savedFilters, function (filterData) {
                return filterData.filterName === ctrl.filterData.filterName;
            });
            if (index > -1) {
                clearFilterDataService();
                filterPersonalizationService.saveDefaultFilter(ctrl.widget, ctrl.userPrefId, ctrl.filterData, isDefault);
                ctrl.commandButtons.bar[0].group[index].data = angular.copy(ctrl.filterData);
            ...
            if (ctrl.savedFilters && ctrl.savedFilters.length) {
                ctrl.savedFilters.forEach(function (filterData) {
                    if (filterData.default) {
                        ctrl.filterData.default = true;
                        var commandsVisibilityList = [
                            { name: 'setDefault', isVisible: false },
                            { name: 'unsetDefault', isVisible: true },
                            { name: 'delete', isVisible: true }
                        ];
                        commandsVisibilityList.forEach(function (item) {
                            handleCommandButtonVisibility(item.name, item.isVisible);
                        });
                        ctrl.filterData.filterName = filterData.filterName;
                        ctrl.filterData.description = filterData.description;
                        ctrl.filterData.filterClauses = angular.copy(filterData.filterClauses);
        ...
        function onFilterChange() {
            ...
            var isDefault = isDefaultFilter(data.filterName);
            refreshSavedFilterDefaultSelection();
            var commandsVisibilityList = [
                { name: 'setDefault', isVisible: !isDefault },
                { name: 'unsetDefault', isVisible: isDefault },
            ...
            ctrl.filterData.default = data.default;
...
(function () {
    ...
    function FilterService($parse, logger) {
         ...
         * @param {Object} [serverDataOptions]
         * A object of serverDataOptions.
         *
         * A object which is having fieldSources property to form OData query to access Facet field.
         *
         ...
         *
         */
        function getODataFilterString(clauses, serverDataOptions) {
            var filterString = "";
            var fieldSource = (serverDataOptions && serverDataOptions.fieldSources) ? serverDataOptions.fieldSources : "";
            var aliasFileds = [];
            ...
            function getFormattedDateTimesClauseString(formattedString, facetName) {
                ...
                if (formattedString) {
                    ...
                    var computedFilterOptions = _.map(filterOptions, function (filter) {
                        ...
                        if (filter !== 'and' && filter !== 'or') {
                            filter = 'Facets/any(f:f/' + facetName +'/' + filter + ')';
                        }
                        return filter;
                    });
            ...
            clauses.forEach(function (clause, idx) {
                ...
                if (clause.filterField.fieldSource) {
                    var facetName = (fieldSource && fieldSource[clause.filterField.fieldSource] && (fieldSource[clause.filterField.fieldSource].type === "facet")) ?
                        fieldSource[clause.filterField.fieldSource].name : "";
                    if (facetName) {
                        if (clause.hasOwnProperty('grpId') && clause.grpClass && clause.grpClass.indexOf('grp-start') !== -1) {
                            groupStart = '(';
                        }
                        if (idx === 0) {
                            if (clause.operator === 'startsWith' || clause.operator === 'endsWith' || clause.operator === 'contains') {
                                filterString = groupStart + 'Facets/any(f: ' + clause.operator + '(f/' + facetName + '/' + getFormattedFacetFieldName(clause.filterField.field) +
                                    ',\'' + clause.value + '\'))';
                            } else if (clause.operator === 'in') {
                                filterString = groupStart + 'Facets/any(f:(f/' + facetName + '/' + getClauseString(clause) + ')';
                            } else if (clause.operator === 'isnull' || clause.operator === '<>') {
                                clauseString = getClauseString(clause).replace(/[()]/g, '');
                                if (clause && clause.filterField.type === "date" && clauseString.match(regexExp)) {
                                    filterString = getFormattedDateTimesClauseString(clauseString, facetName);
                                } else {
                                    filterString = groupStart + 'Facets/all(f:f/' + facetName + '/' + clauseString + ')';
                                }
                            } else {
                                clauseString = getClauseString(clause).replace(/[()]/g, '');
                                if (clause && clause.filterField.type === "date" && clauseString.match(regexExp)) {
                                    ...
                                    filterString = groupStart + "Facets/any(f:f/" + facetName + "/" + clauseString + ")";
                                }
                            }
                        } else {
                            if (clause.operator === 'startsWith' || clause.operator === 'endsWith' || clause.operator === 'contains') {
                                filterString += " " + clause.andOr + " " + groupStart + 'Facets/any(f: ' + clause.operator + '(f/' + facetName + '/' +
                                    getFormattedFacetFieldName(clause.filterField.field) + ',\'' + clause.value + '\'))';
                            } else if (clause.operator === 'in') {
                                filterString += " " + clause.andOr + " " + groupStart + 'Facets/any(f:(f/' + facetName + '/' + getClauseString(clause) + ')';
                            } else if(clause.operator === 'isnull' || clause.operator === '<>'){
                                clauseString = getClauseString(clause).replace(/[()]/g, '');
                                if (clause && clause.filterField.type === "date" && clauseString.match(regexExp)) {
                                    filterString += " " + clause.andOr + " " + groupStart + getFormattedDateTimesClauseString(clauseString, facetName);
                                } else {
                                    filterString += " " + clause.andOr + " " + groupStart + 'Facets/all(f:f/' + facetName + '/' + clauseString + ')';
                                }
                            }
                            else {
                                clauseString = getClauseString(clause).replace(/[()]/g, '');
                                if (clause && clause.filterField.type === "date" && clauseString.match(regexExp)) {
                                    ...
                                    filterString += " " + clause.andOr + " " + groupStart + "Facets/any(f:f/" + facetName + "/" + clauseString + ")";
                                }
                            }
                        }
 ...
 /**
         ...
         *
         * The filtering is limited to perform  *contains* match on a configured data field.
         *
