    ...
    function ManageTagsEditorController($translate, tagsManagementService, $timeout, $q, $scope) {
        ...
        (function tagViewerMethods(vm) {
            ...
            function tagViewerAddAvailableTag(tag) {
                ...
                var index = _.findIndex(vm.availableTags, function (t) {
            ...
            function tagViewerRemoveAssignedTag(tag) {
                ...
                var index = _.findIndex(vm.assignedTags, function (t) {
                    return t.Name === tag.Name;
                });
            ...
            function updateListOfTagsToAddAndToRemove(tag, isAdd) {
                var index = _.findIndex(vm.originalAssignedTags, function (t) {
                    return t.Name === tag.Name;
                });
                if (isAdd) {
                    if (index === -1) {
                        index = _.findIndex(vm.tagsNamesToAdd, function (t) {
                            return t === tag.Name;
                        });
                        if (index === -1) vm.tagsNamesToAdd[vm.tagsNamesToAdd.length] = tag.Name;
                        return;
                    }
                    index = _.findIndex(vm.tagsNamesToRemove, function (t) {
                        return t === tag.Name;
                    });
...
(function () {
    ...
    function TagElementController($translate, $timeout, $element, $rootScope) {
        ...
        function activate() {
            ...
            vm.removeIcon = {
                path: 'common/icons/cmdCloseTab16.svg',
                size: '16px'
            };
...
(function () {
    ...
    function TagMgmtScreenController(tagsManagementService, $state, $stateParams, common, $filter, $scope, $translate, $q, $log, messageOverlayService) {
        ...
        function onGetAssignedAndAvailableTagsSuccess(result) {
            ...
            self.entityIsMarkedToBeCleaned = result[0].value[0].ToBeCleaned;
            self.entityNId = result[0].value[0].NId ? result[0].value[0].NId : result[0].value[0].Id;
            self.sessionTags = result[1].value;
            self.sessionTags.forEach(function (t) {
                if (t.IsSelected) self.availableTags.push(t);
            });
        ...
        function save() {
            ...
            else if (self.entityIsMarkedToBeCleaned) {
                showErrorMessage = true;
                errorMessage = $translate.instant('tagsManager.cantAssignTagToMarkedForCleaning',
                    { EntityType: self.entityinfo.PublicModelEntityShortName, EntityId: self.entityNId });
            }
