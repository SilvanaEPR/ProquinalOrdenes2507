...
(function () {
    ...
    * ```
     (function(){
        angular.module('siemens.simaticit.common.examples').controller('DialogExampleController', ['common.widgets.dialog.service', function (dialogService) {
            ...
            this.dialogVerticalDataTitle = "Sample Dialog";
            this.modalId = 'popup2';
            ...
            this.open = function(){
                if (vm.dialogcentered) {
                    ...
                    dialogService.showDialogModal(vm.modalId, 'exampleBtn');
                }
            }
    *   }]);
    * })();
    ...
    * ```
        ...
        <button class="btn btn-primary" id="exampleBtn" ng-click="dDevCtrl.open()">Open dialog</button>
    ...
    * ```
        (function(){
            angular.module('siemens.simaticit.common.examples').controller('DialogExampleController', ['common.widgets.dialog.service', function (dialogService) {
                ...
                this.dialogVerticalData = { layout: 'Vertical', data: this.templateData, information: 'This popup uses the standard template based on sit-property-grid' };
                this.customTemplateTitle = "Sample customtemplate";
                this.templateUri = 'common/requiredTemplate';
                this.modalId = 'popup2';
                this.clickOk = function () {
                    dialogService.hideDialogModal('popup2');
                };
    ...
    function DialogService($rootScope, LOG_TYPES, $window, swacManager) {
        ...
        */
        ...
        function setDialogDimension(popupElement) {
            var alertContainerHeight = $(popupElement[0]).find('.dialog-alert-container-').height() || 0;
        ...
        function getModalBackdrop(popupEl) {
            //To support old version of bootstrap
            if ('4.6.1' === window.bootstrap.Tooltip.VERSION) {
                return popupEl.data('bs.modal')._backdrop;
            }
            return window.bootstrap.Modal.getInstance(popupEl)._backdrop._element;
        }
        ...
        this.showDialogModal = function (id, posId) {
            isSidePanelExist = document.querySelectorAll('.property-are-parent.ng-hide').length >= 1 ? false : true; //condition to know whether sidepanel present in th DOM
            subscribe(this, id);
            ...
            if (swacManager.enabled) {
                if (isSidePanelExist) {
                    var modalOverlay = parent.document.querySelectorAll('.mom-modal-overlay.mom-modal-overlay-displayed');
                    if (modalOverlay.length) {
                        modalOverlay[0].style.backgroundColor = 'rgba(0, 0, 0, .75)';
                        modalOverlay[0].style.opacity = 0.75;
                    }
                } else {
            ...
            var popUpBackdropContent = getModalBackdrop(popupEl);
            var popUpBackdrop = $(popUpBackdropContent);
        ...
        this.hideDialogModal = function (id) {
            ...
            if (swacManager.enabled) {
                if (isSidePanelExist) {
                    var modalOverlay = parent.document.querySelectorAll('.mom-modal-overlay.mom-modal-overlay-displayed');
                    modalOverlay.length && (modalOverlay[0].style.backgroundColor = 'transparent');
                } else {
