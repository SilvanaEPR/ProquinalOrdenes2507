...
(function () {
    ...
    ComponentController.$inject = ["$q", '$injector', '$compile', '$state', '$stateParams', '$rootScope', '$scope', '$timeout', '$filter',
        'common.base', 'common.services.logger.service', 'common.services.signalManager', 'common.services.ui.authentication'
    ];
    ...
    function ComponentController($q, $injector, $compile, $state, $stateParams, $rootScope, $scope, $timeout, $filter, base, loggerService, signalService, authenticationService) {
        ...
        var processingTaskNumber = 0;
        var vm = this;
        vm.taskChangedCounter = 0;
        ...
        var MAX_RECONNECTION_REQUESTS = 50;
        ...
        function initGridOptions() {
            ...
            if (vm.icvSortInfo) {
                ...
                _icvSortInfo = {
                    ...
                    fields: [{
                        field: 'Name',
                        displayName: $filter("translate")("taskContainer.taskList.name-sort-by")
                    }, {
                        field: 'CreatedOn',
                        displayName: $filter("translate")("taskContainer.taskList.created-sort-by")
                    }, {
                        field: 'LastUpdatedOnLocale',
                        displayName: $filter("translate")("taskContainer.taskList.lastupdated-sort-by")
                    }, {
                        field: 'IndexByStatus',
                        displayName: $filter("translate")("taskContainer.taskList.default-sort-by")
                    }]
                };
            ...
            vm.viewerOptions = {
                ...
                tileConfig: {
                    propertyFields: [{
                        ...
                        displayName: $filter("translate")('taskContainer.tile.taskDefinitionId')
                    }
                        //{ field: 'CreatedOn', displayName: $filter("translate")("taskContainer.tile.created") },
                        , {
                        field: 'LastUpdatedOnLocale',
                        displayName: $filter("translate")("taskContainer.tile.lastUpdated") /*, cellFilter: 'date:\'medium\'' */
                    }
        ...
        function applyFormatDate(data) {
            if (data) {
                for (var k = 0; k < data.length; k++) {
                    data[k].LastUpdatedOnLocale = $filter('date')(new Date(data[k].LastUpdatedOn), 'MMM d, y h:mm:ss a');
                }
            }
        }
        ...
        function applySortBy(data) {
            ...
            for (var k = 0; k < result.length; k++) {
                ...
                result[k].LastUpdatedOnLocale = $filter('date')(new Date(result[k].LastUpdatedOn), 'MMM d, y h:mm:ss a');
            }
        ...
        function onGridSortingChanged(sortInfo) {
            if ((vm.events) && angular.isFunction(vm.events.onGridSortingChanged)) {
                vm.events.onGridSortingChanged({
                    'sortInfo': sortInfo
                });
            }
        }

        function incrementTaskChangedCounter() {
            vm.taskChangedCounter = (vm.taskChangedCounter + 1) % 1000;
        }

        function selectCurrentProcessedTask() {
            vm.viewerData.forEach(function (a) {
                if (a.Id === vm.taskIdSelected) {
                    a.selected = true;
                } else {
        ...
        function onGridItemSelectionChanged(items, item) {
            ...
            //If the processingTaskNumber is != 0 (a value of 0 means no task is currently in processing) then keep the current taskSelected on the ICV
            if (processingTaskNumber !== 0) {
                var clickedTask = 'not set';
                if (item) {
                    clickedTask = item.Id;
                }
                var taskSelectedId = vm.taskSelected ? vm.taskSelected.Id : 'not set';
                logger.logDebug('Selected Task:' + clickedTask + ' but the' + taskSelectedId + 'is still in processing');
                selectCurrentProcessedTask();
                return;
            }

            // Here we don't have any task in progress so let's process the selected task
            if (item && item.selected === true) {
                incrementTaskChangedCounter();
                vm.selectedTask = item;
                vm.taskSelected = item;
                vm.taskIdSelected = vm.selectedTask.Id;

                logger.logDebug('Start processing Task Number: ' + vm.taskChangedCounter + ' Taskid: ' + vm.taskSelected.Id);

                $timeout(function () {
                    if (processingTaskNumber === vm.taskChangedCounter) {
                        processingTaskNumber = 0;
                        logger.logDebug('Timeout for managing OnTaskContainerSelectionChanged triggered (task processed) for Task Number: ' + vm.taskChangedCounter + 'TaskId' + vm.taskSelected.Id);
                    }

                    if ((vm.events) && angular.isFunction(vm.events.onTaskContainerSelectionChanged)) {
                        vm.events.onTaskContainerSelectionChanged({
                            'value': vm.taskSelected
                        });
                    }
                }, 100);

                if (processingTaskNumber != 0) {
                    logger.logDebug('Timeout still not triggered for Task Number: ' + vm.taskChangedCounter + 'TaskId' + vm.taskSelected.Id + 'discard the selection');
                    return;
                }

                processingTaskNumber = vm.taskChangedCounter;

                if (vm.TaskShown !== vm.selectedTask.Id) {
                    logger.logDebug('Task Selected ( ' + vm.selectedTask.Id + ' ) different from Task Shown ( ' + vm.TaskShown + ' ) show the new task in the container');
                    FindAndShowTaskItemContainer(vm.selectedTask);
                } else {
                    logger.logDebug('Task Selected ( ' + vm.selectedTask.Id + ' ) equal to Task shown, keep the current taskSelected on the ICV');
                    selectCurrentProcessedTask();
                }
                updateCommandBar(vm.taskSelected);

            } else {
                //If the user clicked on the same task, then do nothing but keep the task selected
                if (item) {
                    item.selected = true;
                    logger.logDebug('No Task in progress and deselected it');
                }
            }
        }
        ...
        function FindAndShowTaskItemContainer(task) {
            ...
            showCustomComponentInHtml(
                ...
                componentStateParams: {
                    TaskId: vm.TaskShown,
                    TaskDefNId: task.TaskDefinitionNId,
                    TaskStatus: task.Status.StatusNId
                }
            },
        ...
        function setComponentVisible(taskId, componentName, componentSource, componentVisible, taskStatus) {
            ...
            if (itemFounded.length === 0) {
                ...
                if ((itemFree) && (itemFree.length !== 0)) {
                    ...
                    $rootScope.$broadcast('common.widgets.containers.task-visible', {
                        taskId: taskId,
        ...
        function onRefresh(values) {
            ...
            for (var z = 0; z < values.length; z++) {
                ...
                var isEqual = isMatching(gridCurrentData, {
                    value: values[z]
                });
                if (!isEqual) {
                    var indexOfItemInViewerData = vm.viewerData.findIndex(function (currentValue) {
        ...
        function onConnectionLost() {
            ...
            if (reconnectRequestCount === MAX_RECONNECTION_REQUESTS) {
                //    log('maximum reconnection request reached...');
                return;
            }
        ...
        function reconnect() {
            ...
            backendService.reconnectSignals(appName).then(function () {
                ...
                loggerService.log("Error in opening a web socket \n" + angular.toJson(error, true));
                onConnectionLost();
            });
        }
        ...
        function closeConnection(connId) {
            ...
            backendService.unsubscribe(vm.signalConnections[connId], true).then(function () {
                ...
                loggerService.log("unsubscribe successfull");
            }, function () {
                loggerService.log("unsubscribe error");
            });
        }
        ...
        function subscribeToStatusChanged() {
            ...
            backendService.subscribe(connectionObj).then(function (id) {
                vm.signalConnections['StatusChanged'] = id;
            }, function () {
                loggerService.log("StatusChanged Subscription error");
            ...
            if (data.value[i].Id == vm.selectedTask.Id) { // && data.value[i].Status.StatusNId === 'InProgress'
                ...
                if ((vm.selectedTask.TaskDefinitionNId == data.value[i].TaskDefinitionNId) && isLastUpdatedOnNewerThanCurrentLastUpdatedOn(vm.selectedTask, data.value[i])) {
                    ...
                    updateCustomComponentInHtml(component, {
                        componentStateParams: {
                            TaskId: data.value[i].Id,
                            TaskDefNId: data.value[i].TaskDefinitionNId,
                            TaskStatus: data.value[i].Status.StatusNId
                        }
                    },
                        'home.Siemens_SimaticIT_Task_Runtime_TaskList.custom-component', data.value[i]);
                } else {
                    if (vm.selectedTask.Id == data.value[i].Id) {
                        $rootScope.$broadcast('common.widgets.containers.on-status-changed', {
                            task: data.value[i]
                        });
                    }
        ...
        function subscribeToMessageOnTaskReceived() {
            ...
            var connectionObj = {
                appName: appName,
                signalName: 'MessageOnTaskReceived',
                options: '',
                onMessage: onMessageOnTaskReceived,
                onSubscriptionError: onError,
                onConnectionError: onConnectionLost,
                onComplete: onComplete
            }
            backendService.subscribe(connectionObj).then(function (id) {
                vm.signalConnections['MessageOnTaskReceived'] = id;
            }, function () {
                loggerService.log("MessageOnTaskReceived Subscription error");
            });
        }
        ...
        function subscribeToErrorCountChanged() {
            var connectionObj = {
                appName: appName,
                signalName: 'ErrorCountChanged',
                options: '',
                onMessage: onErrorCountChanged,
                onSubscriptionError: onError,
                onConnectionError: onConnectionLost,
                onComplete: onComplete
            }
            backendService.subscribe(connectionObj).then(function (id) {
                vm.signalConnections['ErrorCountChanged'] = id;
            }, function () {
                loggerService.log("ErrorCountChanged Subscription error");
            });
        }
