...
(function () {
   ...
   *    
   *   The command bar can be exposed to maximum two different button set. If you add more commands or reduce the size of the command bar, you may miss out some of the commands.
   *   
   *   @restrict E
   ...
   */
    ...
    function CommandBarController($scope, $element, common, $translate, $window, $timeout, $compile) {
        ...
        vm.otherCommandsText = $translate.instant('commandBar.otherCommands');
        vm.moreCommandsText = $translate.instant('commandBar.moreCommandsAvailable');
        ...
        vm.toggleIcon = function () {
            ...
            if (vm.isVerticalCommandLabelShown) {
                if ($(".arrow-spec p").text() === vm.moreCommandsText) {
                    moreLessText = vm.lessCommandsText;
                } else {
                    moreLessText = vm.moreCommandsText;
                }
            } else {
                moreLessText = '';
            }
            ...
            for (var i = listLength - 1; i >= 0; i--) {
                ...
                if (verticalCommandsHeight > verticalCommandBarHeight) {
                    if (reachedIndex === 0) {
                        ...
                        var moreHeight = getItemHeight(WITH_LABEL_HEIGHT, WITHOUT_LABEL_HEIGHT, SHOPFLOOR_LABEL_HEIGHT);

                        bottomHeight += (moreHeight + diff) - 8;

                        $(".arrow-spec p").text(vm.isVerticalCommandLabelShown ? vm.moreCommandsText : '');
                    }
                    if (reachedIndex !== 1) {
        ...
        function horizontalCutoff() {
            ...
            var maxWidth;
            if (vm.commands.bar.length > 10) {
                maxWidth = vm.__width - maxWidthOffset > 0 ? vm.__width - maxWidthOffset - 150 : 0; // minus margin and 150 buffer for showing more button
            } else {
                maxWidth = vm.__width - maxWidthOffset > 0 ? vm.__width - maxWidthOffset : 0; // minus margin
            }
        ...
        function getCommandWidth(command) {
            ...
            if (type === "MainCommand" || type === "toggle" || type === "Command") {
                ...
                ele = $('<div class="commandBarContainerTool tempWidthCalcDiv">' +
                    ...
                    'class="right-align-align dropdown-toggle MainCommandGroupButton " data-bs-toggle="dropdown" title="maingroup">' +
                    ...
                    'class="right-align-align dropdown-toggle MainCmdGrpdropdown" data-bs-toggle="dropdown" title="maingroup">' +
                    '<span class="caret">' +
                    '</span>' +
                    '</button>' +
    ...
    SitCommandBarDirective.$inject = ['$timeout', '$window', 'common.widgets.commandBar.service','$sanitize'];
    function SitCommandBarDirective($timeout, $window, commandBarService, $sanitize) {
        ...
        function postLink(scope, element, attrs, controller) {
            ...
            scope.$on('$destroy', function () {
                listners.forEach(function (listner) {
                    typeof listner === 'function' && listner();
                });
                if (controller._isGlobalCommandbar && --SitCommandBarDirective.globalCommandBarInstances <= 0) {
                    commandBarService.hide();
                    SitCommandBarDirective.globalCommandBarInstances = 0;
                }
            });

            controller._isGlobalCommandbar = controller.layout === 'vertical' && (element.parents('div[ui-view=Canvas]').length === 1 || element.parents('div[ui-view=panel-in-canvas]').length === 1) && controller.verticalFixedPosition;
            if (controller._isGlobalCommandbar) {
            ...
            function truncateLabel() {
                function ellipsisText(e) {
                    ...
                    var isShopFloorOperation = document.getElementsByClassName('shop-floor operational');
                    var width = isShopFloorOperation.length > 0 ? 76 : 48;

                    if (e.innerHTML.indexOf('...') === -1) {
                        e.innerHTML = $sanitize(getTrunactedLabel(e.innerHTML, font, width));
                    }
                }
            ...
            function getTrunactedLabel(text, font, width) {
                ...
                wordArray.forEach(function (word, index) {
                    ...
                    var textWidth = getTextWidth(word, font);
                    lineWidth += textWidth;
                    if (lineWidth < width) {
                        if (lineCount === 0) {
                            firstlineText.push(word);
                        } else {
                            secondlineText.push(word);
                        }
                    }
                    ...
                    if (lineWidth > width) {
                        if (index > 0) {
                            ...
                            if (lineCount !== 2) {
                                ...
                                secondlineText.push(word);
                                lineWidth += getTextWidth(secondlineText.join(" "), font);
                                if (index < wordArray.length) {
                                    firstlineText.push(getComputedLabel(secondlineText.join(" "), font, width));
                                }
                            }
                            ...
                            if ((wordArray.length - 1) === index) {
                                if (lineWidth > width) {
                                    ...
                                    firstlineText.push(secondlineText);
                                }
                            }
                        } else {
                            firstlineText.push(word);
                ...
                return firstlineText.join(" ");
            }

            function getComputedLabel(label, font, width) {
                var computedLabel = label + '...';
                var textWidth = getTextWidth(label, font);

                while (textWidth >= width) {
                    label = label.slice(0, -1);
                    computedLabel = label.trim() + '...';
                    textWidth = getTextWidth(computedLabel, font);
                }
            ...
            listners[listners.length] = scope.$watch(function () {
                ...
                if (controller.layout === 'vertical') {
                    $timeout(truncateLabel, 250, false);
            ...
            listners[listners.length] = scope.$on('sit-layout-change', function (event, args) {
                if (args.eventSource === 'layout') {
                    $timeout(onResizeElement, 100, false);
                    controller.calculateCutOff();
                }
            });
...
(function () {
    ...
    *   This is used only if both svg-icon and cmd-icon are not specified.
        ...
        function onCommandClick(command, $event) {
            if (!command.disabled) {//execute command callback only if command is not disabled
                if (command.type === 'toggle') {
                    ...
                    if ($event) {
                        var menu_button = $($event.currentTarget).parents('ul.dropdown-menu').prev('.dropdown-toggle');
                        if (menu_button.length > 0) {
                            window.bootstrap.Dropdown.getInstance(menu_button).toggle();
                        }
                    }
                    /*if ($event && $($event.currentTarget).parents('[data-toggle=dropdown]').length > 0) {
                        $($event.currentTarget).parents('[data-toggle=dropdown]').dropdown('toggle');
                    }*/
                    if (vm.showas === 'Menu') {
    ...
    function SitCommandDirective(securityService, functionRightModel) {
        ...
        function postLink(scope, element, attrs, controllers) {
            ...
            function initializeCommandProperties() {
                ...
                if (sitCommandCtrl.layout === 'horizontal' && sitCommandCtrl.name && sitCommandCtrl.name.indexOf('.') > -1 && typeof sitCommandCtrl.name.replaceAll === 'function') {
                    sitCommandCtrl.name = sitCommandCtrl.name.replaceAll('.', ''); 
                }
            ...
            scope.$watch(function () {
                return sitCommandCtrl.badge;
            }, function (newValue, oldValue) {
                if (oldValue === newValue) {
                    return;
                }
                if (null === newValue) {
                    sitCommandCtrl.command.badge = null;
                    return;
                }
                sitCommandCtrl.command.badge = sitCommandCtrl.command.badge || {};
                sitCommandCtrl.command.badge.state = newValue.state || 'information';
                sitCommandCtrl.command.badge.value = newValue.value || 0;
            }, true);
        ...
        return {
            ...
            template:
                ['<div ng-transclude class="command-hide"></div>',
                    ...
                    '<div ng-if="cmdCtrl.command.badge && !cmdCtrl.command.label" class="notification-alert-badge"><p class="notification-alert-badge-text">{{cmdCtrl.command.badge.value}}</p></div>',
                    '</button>',
                    '</div>',
                    ...
                    '<span class="show-as-list-label"><span class="primary-text"> <span > {{cmdCtrl.command.label}} </span> </span> <span ng-if="cmdCtrl.command.description" title="{{cmdCtrl.command.description}}" class="secondary-text"> <span> {{cmdCtrl.command.description}} </span> </span></span>',
                    ...
                    'ng-click="cmdCtrl.commandClicked(cmdCtrl.command, $event)"></em>',
                    '</label>',
                    ' </div>',
                    '</div>',
                    '</div>'].join('\n'),
...
(function () {
    ...
    *   @param {string} showas Define if the group is diplayed as a **Button** , **Menu**  or **List**. 
    *   @param {string} bartype Type of commands included in the command-bar (**Action** or **Tool** commands).
    *
    *   #### Using a configuration object
    *   ```
    ...
    */
    //#endregion
    commandGroupController.$inject = ['$scope', '$state', 'common'];
    function commandGroupController($scope, $state, common) {
        var vm = this;
        vm.logErrorFn = logErrorFn;
    ...
    commandGroup.$inject = ['$window'];
    function commandGroup($window) {
        function preLink(scope, element, attrs, controllers) {
            var sitCommandBarCtrl = controllers[0];
        ...
        function postLink(scope, element, attrs, controllers) {
            ...
            function checkOpenPos(winWidth) {
                if (winWidth) {
                    ...
                    if (sitCommandGroupCtrl.groupButtonElement) {
                        ...
                        if (sitCommandGroupCtrl.layout === 'contextual' && $(sitCommandGroupCtrl.groupButtonElement).parents('.property-area-container').length > 0) {
                            ...
                            if (elementLeftPosition >= winWidth / 2) {
                                sitCommandGroupCtrl.openLeft = true;
                            } else {
                                sitCommandGroupCtrl.openLeft = false;
                            }
            ...
            function rearrangeDropdown() {
                checkOpenPos(win.width());
                if (!scope.$$phase) {
                    scope.$apply();
                }
            }

            sitCommandGroupCtrl.subscribeEvents = function () {
                angular.element($window).bind('resize', rearrangeDropdown);
                $('.dropdown-menu').on("click.bs.dropdown", onClickDropdown);
                element.find('div.btn-group').on('show.bs.dropdown', rearrangeDropdown);
            }

            scope.$on('$destroy', function () {
                $('.dropdown-menu').off("click.bs.dropdown", onClickDropdown);
                angular.element($window).unbind('resize', rearrangeDropdown);
                element.find('div.btn-group').off('show.bs.dropdown', rearrangeDropdown);
            });
        }
        return {
            bindToController: {
            ...
            template: ['<div>',
                ...
                '<div ng-switch-when="Button" class="btn-group" ng-class="{\'MainCommanBtnGrp\': cmdBarGrpBtnCtrl.isMainCommandGroup}" ng-init="cmdBarGrpBtnCtrl.subscribeEvents()">',
                ...
                'data-bs-toggle="{{(cmdBarGrpBtnCtrl.layout === \'vertical\' || cmdBarGrpBtnCtrl.layout === \'contextual\')  ? \'dropdown\' : \'\' }}" data-bs-display="static">',
                ...
                'class="dropdown-toggle MainCmdGrpdropdown" data-bs-toggle="dropdown" data-bs-display="static" title="{{ cmdBarGrpBtnCtrl.group.tooltip || cmdBarGrpBtnCtrl.group.label }}">',
                '<span class="caret">',
                '</span>',
                '</button>',
                '</button>',
                ...
                'data-bs-toggle="dropdown" title="{{ cmdBarGrpBtnCtrl.group.tooltip || cmdBarGrpBtnCtrl.group.label }}" data-bs-display="static">',
    ...
    function SitSearchDirective() {
        ...
        function linkFunction(scope, element, attrs, controllers) {
            ...
            function registerWatch() {
                disableSearchWatch = scope.$watchCollection(function () {
