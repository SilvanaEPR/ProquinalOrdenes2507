...
(function () {
    ...
    function breadcrumbController($interpolate, breadcrumbService, $filter, $sanitize, $element) {
        var vm = this;
        vm.draw = draw;
        ...
        function getTextWidth(characters) {
            ...
            myDiv.html($sanitize(characters));
        ...
        function setContainerDivWidth() {
            vm.containerDivWidth = $element[0].offsetWidth;
        }
    ...
    breadcrumbController.$inject = ['$interpolate', 'common.breadcrumb.breadcrumbService', '$filter', '$sanitize', '$element'];
...
(function () {
    ...
    function breadcrumbService($state, $stateParams, $rootScope, sessionService, swacManager) {
        var PARENT_STATE_REGEX = /^(.+)\.[^.]+$/; //NOSONAR
        var SESSION_VARIABLE = 'sitBreadcrumbCtx';
        var lastViewScope = $rootScope;
        var stateTitles = JSON.parse(sessionService.get(SESSION_VARIABLE)) || {};
        ...
        function addStateInChain(chain, state) {
            if ((state.views['property-area-container@'] && state.views['property-area-container@'].controller) &&
                !(state.views['Canvas@'] || state.views['FullScreenView'])) {
                return;
            }
            if (!state.abstract) {
        ...
        */
        this.setStateTitle = function (state, title) {
            if (title !== stateTitles[state]) {
                ...
                sessionService.set(SESSION_VARIABLE, JSON.stringify(stateTitles));
                $rootScope.$broadcast('breadcrumbService.stateTitleChanged');
            }
        };

        this.getStateTitle = function (state) {
            var stateTitlesInfo = JSON.parse(sessionService.get(SESSION_VARIABLE)) || {};
            return stateTitlesInfo[state];
        };

        /**
        * @ngdoc method
        * @name common.breadcrumb.breadcrumbService#clearSession
        * @description Clears the breadcrumb session context.
        */
        this.clearSession = function () {
            sessionService.remove(SESSION_VARIABLE);
        }

        function subscribeToLogoutEvent() {
            if (swacManager.enabled) {
                swacManager.eventBusServicePromise.promise.then(function (eventBusSvc) {
                    eventBusSvc.register('opExFn.userLoggedOut');
                    eventBusSvc.event.subscribe(function (event) {
                        if (event.data.topic === 'opExFn.userLoggedOut') {
                            sessionService.remove(SESSION_VARIABLE);
                        }
                    });
                });
    ...
    breadcrumbService.$inject = ['$state', '$stateParams', '$rootScope', 'common.services.session', 'common.services.swac.SwacUiModuleManager'];
