...
(function () {
    ...
    angular.module('siemens.simaticit.common.services.modelDriven')
        ...
        .controller(ctrl, ["common.services.modelDriven.dataService", '$q', '$state', '$stateParams', '$translate', 'common.base', 'common.services.modelDriven.service',
            '$scope', '$parse', '$rootScope', 'common.services.modelDriven.contextService', '$filter', '$injector', '$timeout', 'common.services.swac.SwacUiModuleManager',
            'common.mduiBreadcrumb.breadcrumbService', 'common.services.modelDriven.warningService',
            function (multiDataService, $q, $state, $stateParams, $translate, base, md, $scope, $parse, $rootScope, mdContextSrv, $filter, applInjector, $timeout, SwacUiModuleManager, breadcrumbService, warningService) {
                ...
                var actionId = stateId + '_' + actionName, outputParameters = {}, emptyKeepFieldValue = "", fillQueryResult = null,
                    pendingEval = 0, listeners = [], action = null, currentChangedItem = '';
                ...
                var sidePanelAutoClose, sidePanelShowConfirmation;
                ...
                md.loadModule(stateId).then(function (manifest) {
                    ...
                    var screen = manifest.states.filter(function (s) { return s.id === stateId; })[0], applName = screen.appName,
                        dataService = null, contentDataService = null, content = null;
                    ...
                    for (content in screen.contents) {
                        if (screen.contents[content].actions) {
                            if (screen.contents[content].actions.some(initAndSearchAction, { act: actionName, contentName: content })) {
                                ...
                                var contentObject = screen.contents[content];
                                if (contentObject.entityRef && contentObject.entityRef.entity) {
                                    contentDataService = multiDataService[contentObject.entityRef.entity];
                                } else if (contentObject.readingFunctionRef && contentObject.readingFunctionRef.functionName) {
                                    contentDataService = multiDataService[contentObject.readingFunctionRef.functionName];
                                }
                                break;
                            }
                        }
                    ...
                    if (self.displayMode === 'inCanvas') {
                        self.inCanvasColumns = action.behavior.inCanvasProperties ? action.behavior.inCanvasProperties.columns || 3 : 3;
                        self.inCanvasCommandBarData = {
                            'barType': 'Action',
                            'bar': [
                                {
                                    'name': 'Back', 'onClickCallback': self.cancel, 'cmdIcon': 'GoBack',
                                    'type': 'Command', 'label': $translate.instant(action.behavior.cancelTitle) || $translate.instant('common.cancel'),
                                    'tooltip': $translate.instant(action.behavior.cancelTitle) || $translate.instant('common.cancel'), 'visibility': true
                                },
                                {
                                    'name': 'Save', 'onClickCallback': self.save, 'cmdIcon': 'Save', 'disabled': true,
                                    'type': 'Command', 'label': $translate.instant(action.behavior.shortTitle) || $translate.instant('common.save'),
                                    'tooltip': $translate.instant(action.behavior.shortTitle) || $translate.instant('common.save'), 'visibility': true
                                }
                            ]
                        };
                        self.okButtonClick = okButtonClick;
                        self.overlayButtons = [
                            {
                                id: "cancel",
                            ...
                            {
                                id: "ok",
                                displayName: $translate.instant('inCanvas.overlay.continue'),
                                onClickCallback: self.okButtonClick
                            }
                        ];
                        warning.setOverlay($translate.instant('inCanvas.overlay.message'), self.overlayButtons);
                    } else {
                        sidePanelAutoClose = action.behavior.sidePanelProperties && action.behavior.sidePanelProperties.autoClose !== undefined ?
                            action.behavior.sidePanelProperties.autoClose : false;
                        sidePanelShowConfirmation = action.behavior.sidePanelProperties && action.behavior.sidePanelProperties.showConfirmation !== undefined ?
                            action.behavior.sidePanelProperties.showConfirmation : true;
                        self.sidepanelConfig = {
                            ...
                            autoClose: sidePanelAutoClose,
                            showConfirmation: sidePanelShowConfirmation
                        };
                    ...
                    function okButtonClick() {
                        ...
                        if (self.inCanvasbreadcrumbItem != null) {
                            breadCrumbLogic(self.inCanvasbreadcrumbItem);
                        } else {
                            $state.go(warning.getState(), warning.getParam(), { reload: true });
                        }
                    }

                    function breadCrumbLogic(breadcrumbItem) {
                    ...
                    function processInputParams(inputParams, outputParams, extensionName) {
                        ...
                        Object.getOwnPropertyNames(inputParams).forEach(function (paramName) {
                            ...
                            //No param defined to be part of command payload.
                            if (!param) {
                    ...
                    function executeExtendedCommand(extension, inputParams, baseCmdOutputParams) {
                        ...
                        var invokeService = dataService;
                        if (!invokeService || undefined === invokeService[actionId] || undefined === invokeService[actionId][extension.id]) {
                            invokeService = contentDataService;
                        }
                        //Command Call
                        invokeService[actionId][extension.id](inputParams, getAppShortName(extension.commandRef.app) || applName).then(function (outcome) {
                            defer.resolve();
                        });
                    ...
                    function save() {
                        ...
                        var invokeService = dataService;
                        if (!invokeService || undefined === invokeService[actionId]) {
                            invokeService = contentDataService;
                        }
                        //Command Call
                        invokeService[actionId](outputParameters, getAppShortName(action.behavior.commandRef.app) || applName).then(function (outcome) {
                            if (outcome) {
                                var promises = [];
                    ...
                    function addFields(field, currFieldName, realIndex) {
                        if (typeof field.hidden === 'boolean' && !field.hidden) {
                            if (field.disable) { // collect disable expression
                                self.ngDisableItem[currFieldName] = field.disable;
                            }
                            if (field.clear) { // collect clear expression
                                self.ngClear[currFieldName] = field.clear;
                            }

                            if (field.value) { // collect value expression
                                self.calcValue[currFieldName] = field.value;
                            }

                            if (field.reloadOnChange) {
                                self.reloadOnChange[currFieldName] = {}
                                if (typeof field.query === "string") {
                                    self.reloadOnChange[currFieldName] = { query: field.query };
                                }
                            }
                            if (field.group) {
                    ...
                    function addChangeCallback() {
                        self.displayData.forEach(function (item) {
                            if (item.container === 'g' && item.fields && item.fields.length) {
                                item.fields.forEach(function (field) {
                                    field.changeCallback = function (old, last, ngModel, isValueChanged) {
                                        if (old !== last || isValueChanged) { //limit number of evaluation because if fired too frequently
                                            currentChangedItem = field.name;
                                            setTimeout(function () {
                                                evalFieldsExpression2(old, last);
                                            }, 0);
                                        }
                                    }
                                });
                            } else {
                                item.changeCallback = function (old, last, ngModel, isValueChanged) {
                                    if (old !== last || isValueChanged) { //limit number of evaluation because if fired too frequently
                                        currentChangedItem = item.name;
                                        setTimeout(function () {
                                            evalFieldsExpression2(old, last);
                                        }, 0);
                                    }
                    ...
                    function getAllExtensions() {
                        ...
                        self.extensions.forEach(function (extension) {
                            ...
                            if (extension.readingFunctionRef) {
                                dataExtensionService = multiDataService[extension.readingFunctionRef.functionName];
                                applName = getAppShortName(extension.readingFunctionRef.app) || applName;
                                filterExtension = {
                                    params: getEvaluatedReadingFunctionParams(extension.readingFunctionRef.params),
                                    query: mdContextSrv.interpolateQuery(extension.query, null)
                                }
                            } else if (extension.entityRef && extension.entityRef.entity) {
                                dataExtensionService = multiDataService[extension.entityRef.entity];
                                applName = getAppShortName(extension.entityRef.app) || applName;
                                filterExtension = mdContextSrv.interpolateQuery(extension.query, null);
                            } else {
                                extension.fields.forEach(function (field) {
                                    if (field.default) {
                            ...
                            if (dataExtensionService) {
                                extensionPromises.push(getAllExtensionData(filterExtension, dataExtensionService, extension));
                            }
                        });
                        $q.all(extensionPromises).then(function () {
                            defer.resolve();
                        });
                        return defer.promise;
                    }

                    function getAllExtensionData(filterExtension, dataExtensionService, extension) {
                        var defer = $q.defer();
                        var contextInput;
                        dataExtensionService.getAll(filterExtension, applName).then(function (data) {
                            contextInput = { action: {} };
                            extension.fields.forEach(function (field) {
                                var fieldName = field.name;
                                if (field.default) {
                                    contextInput.action[extension.id] = { queryResult: data.value[0] };
                                    evaluateDefault(field, contextInput);
                                } else {
                                    var fieldShortName = fieldName.substring(field.extensionName.length + 1);
                                    self.currentItem[fieldName] = mdContextSrv.parseExpr(fieldShortName, data.value[0]);
                                }
                                if (field.enableMultiSelection) {
                                    setMultiSelectedObject(fieldName);
                                }
                                if (field.useEntityPicker) {
                                    if (self.currentItem[fieldName] && typeof self.currentItem[fieldName] !== 'object') {
                                        var val = self.currentItem[fieldName];
                                        var displayProperty = field.displayProperty;
                                        self.currentItem[fieldName] = {};
                                        self.currentItem[fieldName][displayProperty] = val;
                                        // tracking whether the default value set to handle reloadOnChange with default
                                        self.currentItem[fieldName].isDefaultValueSet = true;
                                    }
                                }
                            });
                            defer.resolve();
                    ...
                    function getData() {
                        ...
                        if (action.behavior.query || checkForReadingFuncParam()) { // a navigation "id" ... convention-> edit mode
                            if (isReadingFunction) {
                                filter = {
                                    query: mdContextSrv.interpolateQuery(action.behavior.query, null),
                                    params: getEvaluatedReadingFunctionParams(action.behavior.readingFunctionRef.params)
                                }
                            } else {
                                filter = mdContextSrv.interpolateQuery(action.behavior.query, null);
                            }
                            if (action.behavior.entityRef && action.behavior.entityRef.app) {
                                applName = getAppShortName(action.behavior.entityRef.app);
                            }
                            if (action.behavior.readingFunctionRef && action.behavior.readingFunctionRef.app) {
                                applName = getAppShortName(action.behavior.readingFunctionRef.app);
                            }
                            ...
                            dataService.getAll(filter, applName).then(function (data) {
                                if (data.value.length > 0) {
                                    ...
                                    //merge with context
                                    contextInput = { action: { queryResult: fillQueryResult[0], item: fillQueryResult[0], items: fillQueryResult, extensions: {} } };
                            ...
                            action.behavior.fields.forEach(function (field) { //also hidden fields
                                if (field.default) {
                                    ...
                                    var tempItem = mdContextSrv.parseExpr(field.name, null);
                                    if (tempItem != undefined)
                                        self.currentItem[field.name] = tempItem;
                                }
                            });
                    ...
                    function checkForReadingFuncParam() {
                        var isNotEmpty = isReadingFunction &&
                            Object.keys(action.behavior.readingFunctionRef.params).length > 0;
                        if (isNotEmpty) {
                            isNotEmpty = Object.values(action.behavior.readingFunctionRef.params).some(function (val) { return val; });
                        }
                        return isNotEmpty;
                    }

                    function selectValueForDropdown(emptyKeepFieldValue) {
                        //try to select the value
                        self.selectFields.forEach(function (f) {
                            var val = self.currentItem[f.name];
                            if (f.options && f.options.length > 0) {
                                var compareVal = val && typeof val === 'object' ? self.currentItem[f.name][f.value] : self.currentItem[f.name];
                                val = f.options.filter(function (i) { return i[f.value] === compareVal; })[0];
                                if (val) {
                        ...
                        if (action.behavior.displayMode !== 'inCanvas') {
                            if (action.behavior.sidePanelProperties) {
                                ...
                                if (sidePanelMode === 'p') {
                                    ...
                                    listeners[listeners.length + 1] = $rootScope.$on('mdui-icv-item-selected', getData);
                                }
                            ...
                            self.inCanvasBreadCrumbOptions = {
                                onClick: function (breadcrumbItem) {
                                    warning.showOverlayOnClick(true);
                                    self.inCanvasbreadcrumbItem = breadcrumbItem;
                                    $state.go(breadcrumbItem.screenId, {}, { reload: true });
                                }
                            };
                        }
                        getData().then(function () {
                            getAllExtensions().then(function () {
                                selectValueForDropdown(emptyKeepFieldValue);
                        ...
                        gridConfigFields.forEach(function (gridConfigField) {
                            if (gridConfigField) {
                                ...
                                if (tempObj[gridConfigField].type === 'Boolean') {
                                    field.cellTemplate =
                                        '<sit-mdtoggle ng-disabled="true" sit-row="row" sit-field="' + gridConfigField + '" ></sit-mdtoggle>';

                                }
                                if (tempObj[gridConfigField].type === 'DateTimeOffset' || tempObj[gridConfigField].type === 'datetime') {
                                    field.cellFilter = 'date:\'medium\'';
                                }
                                if (tempObj[gridConfigField].type === 'number') {
                                    field.cellFilter = 'number';
                                }
                ...
                function interpolateQuery(propertyQuery) {
                    ...
                    if (self.fillQueryResult) {
                        context = {
                            action: { fields: self.currentItem, queryResult: self.fillQueryResult[0], item: self.fillQueryResult[0], items: self.fillQueryResult }
                        };
                    } else {
                    ...
                    if (searchString) {
                        ...
                        if (isReadingFunctionField(prop)) {
                            ...
                            backendSvcMethodParams = {
                                appName: getAppShortName(prop.readingFunctionRef.app),
                                functionName: prop.readingFunctionRef.functionName,
                                params: readingFunctionParams, options: query
                            };
                        } else {
                            backendSvcMethodParams = { appName: getAppShortName(prop.entityRef.app), entityName: prop.entityRef.entity, options: query };
                        }
                ...
                function createReloadOnChangeConfig(prop, field, applName) {
                    ...
                    if (self.reloadOnChange[propName] && dataArtifact) {
                        ...
                        if (prop.readingFunctionRef && prop.readingFunctionRef.params) {
                            params = prop.readingFunctionRef.params;
                            var interpolatedQuery = filter;
                            filter = {
                                query: interpolatedQuery,
                                params: getEvaluatedReadingFunctionParams(prop.readingFunctionRef.params)
                            }
                        }
                        ...
                        if (prop.selectEntityRef && prop.selectEntityRef.app) {
                            selectionEntityApp = prop.selectEntityRef.app;
                        }

                        self.reloadOnChange[propName]["readingFunctionParams"] = params;
                        self.reloadOnChange[propName]["last"] = filter;
                        if (prop.stopReloadOnInit) {
                            self.reloadOnChange[propName]["lastSelectionQuery"] = "";
                        }
                        self.reloadOnChange[propName]["dataArtifact"] = dataArtifact;
                        self.reloadOnChange[propName]["applName"] = applName;
                        self.reloadOnChange[propName]["selectionEntityApp"] = selectionEntityApp;
                ...
                function createFileUploadFieldConfig(prop, field) {
                    ...
                    field.widgetAttributes = {
                        ...
                        'sit-multiple-files': prop.enableMultiFileUpload,
                        'sit-max-limit': prop.maxFileUploads,
                ...
                function getFilterSettingsFieldConfig(prop, field) {
                    ...
                    for (var i = 0; i < filterFields.length; i++) {
                        ...
                        if (filterFields[i].dataSource && filterFields[i].dataSource.selectedEntity) {
                            filterFields[i].appName = filterFields[i].dataSource.selectedEntity.app || screen.appName;
                        }
                        if (filterFields[i].dataSource && filterFields[i].dataSource.readingFunction) {
                            filterFields[i].appName = filterFields[i].dataSource.readingFunction.app || screen.appName;
                        }
                ...
                function getEntityPickerFieldConfig(prop, field) {
                    ...
                    field.widgetAttributes = {
                        "sit-selected-attribute-to-display": prop.displayProperty,
                        "sit-title": prop.entityPickerTitle,
                        "sit-editable": false,
                        "sit-placeholder": $translate.instant(prop.placeholder) || $translate.instant(defaultEntityPickerPlaceHolder),
                        "sit-picker-options": {
                            containerID: 'globalDialogId-entity-picker-popup',
                ...
                function callServerPreSelectQuery(prop, optionalConfig, applName) {
                    ...
                    if (prop.preSelectValue) {
                        if (selectionFilter && prop.selectEntityRef && prop.selectEntityRef.entity) {
                            applName = prop.selectEntityRef.app || applName;
                ...
                function callSelectEntityGetAll(prop, selectionFilter, applName, data) {
                    var promise = null;
                    if (!prop) {
                        return promise;
                    }
                    ...
                    promise = multiDataService[prop.selectEntityRef.entity].getAll(selectionFilter, applName)
                        .then(function (selectQueryData) {
                            ...
                            var baseValueProperty = prop.baseValueProperty ? prop.baseValueProperty : null;
                            var selectValueProperty = prop.selectValueProperty ? prop.selectValueProperty : null;
                            if (baseValueProperty && selectValueProperty) {
                                for (var i in data.value) {
                                    var baseValue = window.$UIF.Object.safeGet(data.value[i], baseValueProperty);
                                    var selectValue = window.$UIF.Object.safeGet(selectQueryData.value[0], selectValueProperty);
                                    if (baseValue === selectValue) {
                                        self.currentItem[propName] = data.value[i];
                ...
                function callSelectEntityGetAll1(prop, selectionFilter, applName) {
                    var promise = null;
                    if (!prop) {
                        return promise;
                    }
                ...
                function callPreSelectQuery(prop, optionalConfig, data) {
                    ...
                    if (selectionFilter && prop.selectEntityRef && prop.selectEntityRef.entity) {
                        applName = prop.selectEntityRef.app || applName;
                        promise = callSelectEntityGetAll(prop, selectionFilter, applName, data);
                    } else {
                ...
                function callSelectQuery(prop, optionalConfig, field) {
                    ...
                    if (prop.entityRef && prop.entityRef.app) {
                        applName = getAppShortName(prop.entityRef.app) || applName;
                    }
                    if (prop.readingFunctionRef && prop.readingFunctionRef.app) {
                        applName = getAppShortName(prop.readingFunctionRef.app) || applName;
                    }
                    var defer = $q.defer();
                    promises.fieldPromise = defer.promise;
                    dataSvc.getAll(filter, applName).then(function (data) {
                        ...
                        var preSelectPromise = callPreSelectQuery(prop, optionalConfig, data);
                        ...
                        preSelectPromise.then(function () {
                            defer.resolve();
                        });
                    });
                    return promises;
                ...
                function getSelectFieldOptionalConfigurations(prop) {
                    var config = {};
                    var filter, selectionFilter, selectionQuery, selectionEntity, preSelectValue, baseValueProperty, selectValueProperty, params, selectionEntityApp;
                    var theDisplayField = "Name", keepField = "Id", distinct = false, dataSvc, dataArtifact, dataAttribute, applName;

                    if (prop.entityRef && prop.entityRef.entity) {
                        dataSvc = multiDataService[prop.entityRef.entity];
                        dataArtifact = prop.entityRef.entity;
                        applName = getAppShortName(prop.entityRef.app);
                    } else if (prop.readingFunctionRef && prop.readingFunctionRef.functionName) {
                        dataArtifact = prop.readingFunctionRef.functionName;
                        self.readingFunctionFields[prop.name] = prop;
                        dataSvc = multiDataService[prop.readingFunctionRef.functionName];
                        applName = getAppShortName(prop.readingFunctionRef.app);
                    }
                    ...
                    config.selectionEntityApp = selectionEntityApp;
                ...
                function createSelectFieldConfig(prop, field, applName) {
                    var filter, selectionFilter, selectionQuery, selectionEntity, preSelectValue, baseValueProperty, selectValueProperty, dataArtifact, params, selectionEntityApp;
                ...
                function createFieldConfig(action, propName, realIndex, applName, extensionName) {
                    ...
                    if (validationObj && validationObj.patternInfo) {
                    ...
                    if (prop && prop.entityRef) {
                        applName = getAppShortName(prop.entityRef.app);
                    }
                    if (prop && prop.readingFunctionRef) {
                        applName = getAppShortName(prop.readingFunctionRef.app);
                    }
                    if (prop.readOnly) {
                        ...
                        switch (true) {
                            ...
                            case ['object', 'queryResult'].indexOf(checkType) > -1:
                                if (prop && prop.entityRef && prop.entityRef.app) {
                                    applName = getAppShortName(prop.entityRef.app);
                                }
                                if (prop && prop.readingFunctionRef && prop.readingFunctionRef.app) {
                                    applName = getAppShortName(prop.readingFunctionRef.app);
                                }
                ...
                */
                ...
                function evalFormValidation() {
                    ...
                    if (self.formDisable) {
                        var ctx = getContext();
                        if (fillQueryResult) {
                            ctx.action["queryResult"] = fillQueryResult[0];
                        ...
                        if (self.displayMode === 'inCanvas') {
                            self.inCanvasCommandBarData.bar[1].disabled = !(self.validInputs && !isFormInvalid);
                        } else {
                ...
                function getEvaluatedReadingFunctionParams(params, ctx) {
                    var evaluatedParams = {};
                    var ctxFields = ctx || { action: { fields: self.currentItem } };
                    if (Object.keys(params).length > 0) {
                        for (var param in params) {
                            if (typeof params[param] === "string") {
                                evaluatedParams[param] = mdContextSrv.parseWithStateCtx(params[param], ctxFields);
                                if ((evaluatedParams[param] === null) || (evaluatedParams[param] === undefined)) {
                ...
                function getNewFilter(elem, ctx) {
                    var newFilter = "";
                    if (fillQueryResult) {
                        ctx['queryResult'] = fillQueryResult[0];
                    ...
                    if (self.reloadOnChange[elem].readingFunctionParams) {
                        var interpolatedQuery = newFilter;
                        newFilter = {
                            query: interpolatedQuery,
                            params: getEvaluatedReadingFunctionParams(self.reloadOnChange[elem].readingFunctionParams, ctx)
                        }
                    }
                ...
                function isValidToReloadOnChange(elem, newFilter, newSelectionFilter) {
                    var isValid = false;
                    if (!elem) {
                        return isValid;
                    }
                    //for reading function params
                    if (typeof newFilter === 'object') {
                        isValid = JSON.stringify(self.reloadOnChange[elem].last) !== JSON.stringify(newFilter) ||
                            self.reloadOnChange[elem].lastSelectionQuery !== newSelectionFilter;
                    } else {
                        //for entity queries
                        isValid = typeof self.reloadOnChange[elem].last === "string" ?
                            (self.reloadOnChange[elem].last !== newFilter || self.reloadOnChange[elem].lastSelectionQuery !== newSelectionFilter) : true;
                    }
                    return isValid;
                }
                var evalReloadOnChange = function (elem, ctx) {
                    ...
                    return function () {
                        ...
                        if (isValidToReloadOnChange(elem, newFilter, newSelectionFilter)) {
                            //reload
                            self.reloadOnChange[elem].last = newFilter;
                            self.reloadOnChange[elem].lastSelectionQuery = newSelectionFilter;
                            if (self.reloadOnChange[elem].pagingMode !== 'server') {
                                            ...
                                            if (self.reloadOnChange[elem].preSelectValue) {
                                                ...
                                                if (newSelectionFilter && self.reloadOnChange[elem].selectionEntity) {
                                                    promise = multiDataService[self.reloadOnChange[elem].selectionEntity]
                                                        .getAll(newSelectionFilter, self.reloadOnChange[elem]["selectionEntityApp"])
                                                        .then(function (selectQueryData) {
                                ...
                                if (self.reloadOnChange[elem].preSelectValue) {
                                    if (newSelectionFilter && self.reloadOnChange[elem].selectionEntity) {
                                        multiDataService[self.reloadOnChange[elem].selectionEntity].getAll(newSelectionFilter, self.reloadOnChange[elem]["selectionEntityApp"])
                                            .then(function (selectQueryData) {
                                                if (selectQueryData.value && selectQueryData.value.length) {
                                                    var selectValue = window.$UIF.Object.safeGet(selectQueryData.value[0], selectValueProperty);
                                                    var optionString = baseValueProperty + ' eq ' + "'" + selectValue + "'";
                                                    var query = getConcatenatedFilterQuery(newFilter, optionString);
                                                    multiDataService[self.reloadOnChange[elem]["dataArtifact"]].getAll(query, self.reloadOnChange[elem]["selectionEntityApp"])
                                                        .then(function (data) {
                ...
                function evalFieldsExpression2(old, newval, isInterNalCall) {
                ...
                listeners[listeners.lenght + 1] = $scope.$on('sit-property-grid.validity-changed', function (event, params) {
                    ...
                    if (self.displayMode === 'inCanvas') {
                        self.inCanvasCommandBarData.bar[1].disabled = !self.validInputs;
                    } else {
...
(function () {
    ...
    function GridCmdController() {
        ...
        vm.fieldSource = vm.options.fieldSource;
...
(function () {
    ...
    /**
      ...
      *
      */
    .directive('sitMdtoggle', ['$parse',function ($parse) {
        return {
            restrict: 'E',
            replace: true,
            scope: false,
            controller: function () {
                this.Value = function (arg) {
                    ...
                    if (this.field && this.row) {
                        ...
                        if (this.col && this.col.field) {
                            fieldName = this.col.field;
                        }
                        var evaluatedValue = $parse(fieldName)(this.row.entity);
                        this.value = evaluatedValue === null ? false : evaluatedValue;
                    }
            ...
            templateUrl: 'common/blueprints/executeCommandTemplate/mdToggleTemplate.html'
        };
    }]);
}());
