...
(function () {
    ...
    function tagsManagementService(commonBase, authenticationService, $q, $http, systemConfigurationService, businessData) {
        ...
        self._appName = null;
        self._entyInfo = {};
        var PERSON_GROUP_SEGREGATION_TAG = "Siemens.SimaticIT.MasterData.PRM_MS.MSModel.DataModel.PersonGroupSegregationTag";
        var PERSON_SEGREGATION_TAG = "Siemens.SimaticIT.MasterData.PRM_MS.MSModel.DataModel.PersonSegregationTag";
        ...
        function getSegregationTagColor(tagName) {
            ...
            executeQuery("$filter=Name eq '" + tagName + "'", 'SegregationTag', 'DataSegregation')
                .then(onGetSegregationTagSuccess)
                .catch(onGetSegregationTagError);

            function onGetSegregationTagError(err) {
                deferred.reject(err);
            }
        ...
        function getTagsStyle(segregationtags) {
            _.each(segregationtags, function (tag, idx) {
                ...
                if (tag.EntityType === PERSON_GROUP_SEGREGATION_TAG || tag.EntityType === PERSON_SEGREGATION_TAG) {
                    getSegregationTagColor(tag.Name).then(onGetSegregationTagColor);
                }
                else {
                    tag.style = getColors(tag);
                }

                function onGetSegregationTagColor(color) {
                    tag.Color = color;
                    tag.style = getColors(tag);
                }
            });
