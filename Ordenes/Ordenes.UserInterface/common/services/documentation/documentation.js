...
(function () {
    ...
    DocumentationCenterService.$inject = ['$q', '$resource', '$http', '$translate', 'common', 'CommandResponse', 'ExecutionError', 'common.services.documentation.config', '$sanitize'];

    function DocumentationCenterService($q, $resource, $http, $translate, common, CommandResponse, ExecutionError, documentationCenterConfigProvider, $sanitize) {
        var vm = this;
        ...
        function setHightlightTag(data) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                if (!isACall(strHtml, 'id')) {

                    strHtml = replaceAttribute(strHtml, 'alt', 'data-alt');
                    strHtml = replaceAttribute(strHtml, 'id', 'alt');

                    //Re-assign to data
                    if (strHtml || strHtml !== '') {
                        strHtml = $sanitize(strHtml);
                        strHtml = replaceAttribute(strHtml, 'alt', 'id', true);
                        data[i].Contents = strHtml;
                    }
                }
                else {
                    if (strHtml || strHtml !== '') {
                        data[i].Contents = $sanitize(strHtml);
                    }
                }
            }
        }

        function replaceAttribute(htmlString, attributeName, newAttributeName, addNoBindable) {
            var noBindable = addNoBindable ? 'ng-non-bindable ' : '';
            var regexString = ''.concat(attributeName, '\\s*=\\s*[\'"]([^\'"]*)[\'"]');
            var regex = new RegExp(regexString, 'g');
            
            var newString = ''.concat(newAttributeName, '="$1"'); 
            var replacedString = htmlString.replace(regex, noBindable + newString);
            return replacedString;
        }

        function isACall(htmlString, attributeName) {
            var regexString=''.concat(attributeName, '\\s*=\\s*[\'"]([^\'"]*)[\'"]'); 
            var regex = new RegExp(regexString, 'g');
            ...
            if (matches && Array.isArray(matches)) {
                matches.forEach(function(match){
                    var attributeValue = match.match(/['"]([^'"]*)['"]/)[1];
                    hasFunctionCall = /\(.*\)/.test(attributeValue);
                });
            }
        ...
        function retrieveJsonHttpGetDoc(path, doRegularPolish, doPagePolish, doHighlightWrap) {
            ...
            getCmd(path).getAll(function (data) {
                if (data.value) {
                    if (doHighlightWrap) {
                ...
                deferred.reject({
                    data: new CommandResponse(false, new ExecutionError(-1, exeError))
                });
            });
