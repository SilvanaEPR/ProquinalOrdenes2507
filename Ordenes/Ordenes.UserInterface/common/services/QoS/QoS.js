                    ...
                    var UIAuthSvc = /** @class */ (function () {
                        ...
                        UIAuthSvc.prototype.initialize = function () {
                            ...
                            if (that.securableScreens.length > 0) {
                                if (that.CONFIG.deploy.mode === 'test' && that.isAdminUser()) {
                        ...
                        UIAuthSvc.prototype.isScreenAccessible = function (screenName, availableScreens) {
                            var that = this;
                            if (_.contains(that.securableScreens, screenName) || (availableScreens && _.contains(availableScreens, screenName))) {
        ...
        function getClaims(category) {
            ...
            if (!isToKeepInCache) {
                ...
                if (deferredQueue[category].length === 1) {
                    var plant = common.plant.getCurrentPlant();
                    var command = plant && plant.NId ? { "Category": category, "PlantId": plant.NId } : { "Category": category };
                        $http.post(_config + 'GetCurrentIdentityClaimsCommand', { "command": command })
