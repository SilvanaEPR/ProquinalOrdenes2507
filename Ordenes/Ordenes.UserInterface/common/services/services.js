...
(function () {
	...
	'siemens.simaticit.common.templates',
        ...
        'siemens.simaticit.common.services.navigation',
        ...
        'siemens.simaticit.common.services.plant',
        'siemens.simaticit.common.services.nervebox',
        'siemens.simaticit.common.services.export',
        ...
        'siemens.simaticit.common.widgets.availableRuntimeApplications',
        ...
        'siemens.simaticit.common.widgets.toggleButton',
        'siemens.simaticit.common.widgets.genericSearch',
        'siemens.simaticit.common.widgets.progressStepper',
        'siemens.simaticit.common.widgets.contextArea',
        'siemens.simaticit.common.widgets.interactiveAccordion',
        'siemens.simaticit.common.widgets.export',
        'siemens.simaticit.common.widgets.advancedNumeric',
        'siemens.simaticit.common.widgets.tree',
        'siemens.simaticit.common.widgets.tooltip',
        'siemens.simaticit.common.widgets.textSelect',
        'ui.codemirror',
        'ui.sortable',
        'nvd3'
    ]);
    ...
    (function (framework) {
        ...
        angular.module('siemens.simaticit.common.services.configUtil')
            .config(['common.services.data.config.endpointsConfigProvider', function (endpointsConfigProvider) {
                endpointsConfigProvider.config.serviceEndpointsUrl = "".concat(window.location.protocol, "//").concat(window.location.host, "/sit-svc/config");
        ...
        */
        ...
        var ConfigDataService = /** @class */ (function () {
            function ConfigDataService($http, $q, endpointsConfig, plantService) {
                var _this = this;
                this.$http = $http;
                this.$q = $q;
                ...
                this.queryServiceEndpoints = function () {
                    ...
                    var plantInfo = _this.plantService.getCurrentPlant();
                    var plantParam = plantInfo && plantInfo.NId ? 'plantid=' + plantInfo.NId + '&&' : '';
                    _this._$http.get(_this.servicesPath + '?' + plantParam + 'timestamp=' + new Date().getTime()).then(function (response) {
                ...
                this.querySignalEndpoints = function () {
                    var deferred = _this.$q.defer();
                    _this._$http.get(_this.signalsPath + '?timestamp=' + new Date().getTime()).then(function (response) {
                        deferred.resolve(response.data);
                    }, function (error) {
                        deferred.reject(error);
                    });
            ...
            ConfigDataService.$inject = ["$http", "$q", "common.services.data.config.endpointsConfig", 'common.services.plant.plantService'];
...
(function () {
    ...
    var ENDPOINTS_OF_PLANT = ['sit-arch/Application', 'sit-svc/Application', 'sit-ui/runtime'];

    var initInjector, $http, $q, $interpolate, configUtilService, plantService, $sanitize;
    var plantInfo, configTextFn, buildNumber;
    var appConfig = {
    ...
    function activate() {
        var content = window.$UIF.Object.safeGet(document.querySelector('meta[name=buildNumber]'), 'content') || 0;
        var version, buildMeta = document.querySelector('meta[name=buildNumber]');
        if (buildMeta && buildMeta.getAttribute('version')) {
            version = buildMeta.getAttribute('version');
        } else {
            version = new Date().getTime();
        }
        buildNumber = content + '' + version;
        angular.module('siemens.simaticit.common').constant('BUILDNUMBER', buildNumber);

        angular.element(window.document).ready(bootstrapFramework);
    }
    ...
    function bootstrapFramework() {
        ...
        loadConfigModule().then(function () {
            preparePlantInfo().then(function () {
                fetchData().then(function () {
                    if (isIEBrowser() && appConfig.type !== 'mdui' && appConfig.type !== 'debug') {
                        noBrowserSupportPopup();
                        return;
                    }
    ...
    function initInjectors() {
        initInjector = angular.injector(['ng', 'ngSanitize', 'siemens.simaticit.common.services.configUtil', 'siemens.simaticit.common.base', 'siemens.simaticit.common.services.plant']);
        $http = initInjector.get('$http');
        $q = initInjector.get('$q');
        $interpolate = initInjector.get('$interpolate');
        $sanitize = initInjector.get('$sanitize');
        configUtilService = initInjector.get('common.services.configUtil');
        plantService = initInjector.get('common.services.plant.plantService');
    }
    ...
    function loadConfigModule() {
        ...
        $http.get(configFileName + '?b=' + buildNumber).then(function (response) {
            var Fn = Function;
    ...
    function preparePlantInfo() {
        var defer = $q.defer();

        //first set the default value to null
        plantInfo = null;

        //No plant information available in non-runtime applications like
        //solution studio, mdui editor, debug application, dev application
        if (appConfig.type !== 'runtime') {
            defer.resolve();
            return defer.promise;
        }

        plantService.preparePlantInfo().then(function (plant) {
            plantInfo = plant;
            defer.resolve();
        });
        return defer.promise;
    ...
    function fetchData() {
        ...
        $http.get(path).then(function (response) {
            ...
            return configUtilService.getServiceEndpoints();
        }).catch(function () {
            endpoints = {};
            return configUtilService.getServiceEndpoints();
        }).then(function (response) {
            angular.extend(endpoints, response);
            createAppConfig(deferred, endpoints);
        }).catch(function () {
            createAppConfig(deferred, endpoints);
        });
        return deferred.promise;
    ...
    function bootstrapApplication() {
        ...
        if (injector) {
            ...
            plantService = injector.get('common.services.plant.plantService');
            plantService.setCurrentPlant(plantInfo);
        }
    }
    ...
    function createEmptyConstant() {
        angular.module('siemens.simaticit.common').constant('CONFIG', {});
    }
    ...
    function getSupportedLocales(injector) {
        var localePath = 'common/scripts/i18n/locales.json';
        var deferred = $q.defer();
        var contextInstance = injector.get('common.ctx');
        if (contextInstance.getAppType() === 'runtime') {
            localePath = 'locales/locales.json';
        }
    ...
    /*
     * generating absolute endpoint path.
     * Pratial path have some drawbacks in dev environment - Project studio UI App debug
     */
    function generateAbsoluteConfig(configData) {
        generate(configData);

        function generate(data) {
            Object.keys(data).forEach(function (item) {
                //incase of complex object call the inner loop again
                ...
                if (data[item].indexOf('/') === 0) {
                    data[item] = window.location.protocol + '//' + window.location.host + data[item];
                }
            });
        }
    }
    ...
    function noBrowserSupportPopup() {
        var path = 'common/resources/common.{{language}}.json';

        //fetch message from resource folder at runtime
        if (appConfig.type === 'runtime') {
            path = 'resources/resource.{{language}}.json';
        }

        //generate path form browser language key
        path = $interpolate(path)(window.navigator);
        ...
        $http.get(path).then(function (response) {
            ...
            //fall back to en-US
            path = $interpolate(path)({ language: 'en-US' });
            $http.get(path).then(function (response) {
                displayPopUp(response);
            }).catch(function () {
                displayPopUp();
    ...
    function displayPopUp(response) {
        var title, text;
        if (window.$UIF.Object.safeGet(response.data, 'noBrowserSupport')) {
            title = response.data.noBrowserSupport.title;
            text = $interpolate('{{textLine1}}<br/>{{textLine2}}<br/><br/>{{textLine3}}<br/>{{textLine4}}')(response.data.noBrowserSupport);
        }
        document.querySelector('#ieUnsupportedPopup .ie-popup-header').innerHTML = $sanitize(title);
        document.querySelector('#ieUnsupportedPopup .ie-popup-header-text').innerHTML = $sanitize(text);
        angular.element(document.querySelector('#ieUnsupportedPopup')).show();
        angular.element(document.querySelector('#initialBusyIndicator')).remove();
    }
})();
            ...
            function Authentication($window, $log, $location, $q, $rootScope, authenticationService, timerService, busyIndicatorService, messageOverlay, CONFIG, logger, $filter, swacManager) {
                ...
                this._startDOMTracking = function () {
                    ...
                    if (!_this._isTracking && _this._DOMTrackingStatus) {
                        $(document).bind('mousemove click mouseup mousedown keydown keypress keyup submit change mouseenter scroll resize dblclick', $.proxy(_this._DOMSubtreeModified, _this));
                ...
                this._stopDOMTracking = function () {
                    ...
                    //}
                    $(document).unbind('mousemove click mouseup mousedown keydown keypress keyup submit change mouseenter scroll resize dblclick', $.proxy(_this._DOMSubtreeModified, _this));
                ...
                */
                this.getIndentity = function () {
                    return _this.authenticationService.getUser();
                };
                /*
                * IMPORTANT:
                * This method was mistakenly published in the reference documentation up to SIMATIC IT UAF v1.2.
        ...
        var AuthenticationService = /** @class */ (function () {
            ...
            */
            ...
            AuthenticationService.prototype.setToken = function (value) {
                ...
                this.deferClaim.notify(this.getUser());
            };
            /*
                * IMPORTANT:
                * This method was mistakenly published in the reference documentation up to SIMATIC IT UAF v1.2.
    ...
    (function (framework) {
        var CommonBase = /** @class */ (function () {
            function CommonBase(commandResponse, executionError, globalization, logger, auth, sidePanelManager, swacComponentService, uiComponentService, securityService, backendService, dataService, archivingDataService, commandService, modulesScreensDataService, signalManager, navigationManagementService, messageOverlayService, globalDialogService, busyIndicatorService, notificationTileGlobalService, commandBarService) {
                this.services = {
                    ...
                    runtime: {
                        ...
                        modulesScreensDataService: modulesScreensDataService
                    },
                    navigation: {
                        navigationManagementService: navigationManagementService
                    }
                };
                this.widgets = {
                ...
                this.utils = {
                    escapeODataValue: function (value) {
            ...
            CommonBase.$inject = [
                ...
                'common.services.runtime.modulesScreensDataService',
                ...
                'common.widgets.contextArea.service'
            ];
            return CommonBase;
        }());
        ...
        /**
            ...
            *          * modulesScreensDataService: {@link service:common.services.runtime.modulesScreensDataService}
            ...
            *      * navigation.service: {@link service:common.services.navigation.navigationManagementService}
            *  * widgets:
            ...
            *      * contextArea.service: {@link service:common.widgets.contextArea.service}
            */
        /**
        * @ngdoc service
        * @name common.base
...
(function (sit) {
    var framework;
    (function (framework) {
        var ContextModal = /** @class */ (function () {
            function ContextModal(APPCONFIG, $q) {
                this.ctx = {
                    configEndpoint: "".concat(window.location.protocol, "//").concat(window.location.host, "/sit-svc/config"),
                    swacEnabled: true,
                    appType: 'runtime',
                    appMode: 'normal',
                    swacType: 'module',
                ...
                if (APPCONFIG.appConfig) {
                    this.ctx.appType = APPCONFIG.appConfig.type;
                    this.ctx.appMode = APPCONFIG.appConfig.mode;
                }
                this.ctx.swacEnabled = this.ctx.appMode === 'swac';
                this.themeServicePromise = $q.defer();
                this.busyServicePromise = $q.defer();
                //this.nerveboxServicePromise = $q.defer();
                this.confirmationServicePromise = $q.defer();
                this.errorServicePromise = $q.defer();
                this.warningServicePromise = $q.defer();
                this.i18NServicePromise = $q.defer();
                this.navigationServicePromise = $q.defer();
                this.notificationServicePromise = $q.defer();
                this.contextServicePromise = $q.defer();
                this.eventBusServicePromise = $q.defer();
                this.displayServicePromise = $q.defer();
            }
            // Generic context methods
            ContextModal.prototype.getCtx = function (property) {
                return this.ctx.hasOwnProperty(property) ? this.ctx[property] : this.ctx;
            };
            ContextModal.prototype.setCtx = function (property, value) {
                this.ctx[property] = value;
            };
            //context specific methods
            ContextModal.prototype.setAppType = function (value) {
                this.ctx.appType = value;
            };
            ContextModal.prototype.getAppType = function () {
                return this.ctx.appType;
            };
            ContextModal.prototype.setAppMode = function (value) {
                this.ctx.appMode = value;
                this.ctx.swacEnabled = this.ctx.appMode === 'swac';
            };
            ContextModal.prototype.getAppMode = function () {
                return this.ctx.appMode;
            };
            ContextModal.prototype.isSwacEnabled = function () {
                return this.ctx.swacEnabled;
            };
            ContextModal.prototype.setSwacType = function (value) {
                this.ctx.swacType = value;
            };
            ContextModal.prototype.getSwacType = function () {
                return this.ctx.swacType;
            };
            ContextModal.prototype.setSwacManager = function (value) {
                this.ctx.swacManager = value;
            };
            ContextModal.prototype.getSwacManager = function () {
                return this.ctx.swacManager;
            };
            ContextModal.prototype.setNavigationBlocked = function (value) {
                this.ctx.navigationBlocked = value;
            };
            ContextModal.prototype.isNavigationBlocked = function () {
                return this.ctx.navigationBlocked;
            };
            return ContextModal;
        }());
        framework.ContextModal = ContextModal;
        var CommonContext = /** @class */ (function () {
            function CommonContext(APPCONFIG) {
                var _this = this;
                this.APPCONFIG = APPCONFIG;
                this.$get = ['$q', function ($q) {
                        return new ContextModal(_this.APPCONFIG, $q);
                    }];
            }
            CommonContext.$inject = ['APPCONFIG'];
            return CommonContext;
        }());
        framework.CommonContext = CommonContext;
        /**
        * @ngdoc provider
        * @name common.ctx
        * @module siemens.simaticit.common
        ...
        *
        * @description
        * The service holds the current executing environment information and the constant values
        */
        angular.module('siemens.simaticit.common').provider('common.ctx', CommonContext);
    })(framework = sit.framework || (sit.framework = {}));
})(sit || (sit = {}));
//# sourceMappingURL=common-ctx-svc.js.map
/// <reference path="model.ts" />
'use strict';
var sit;
(function (sit) {
    ...
    (function (framework) {
        var Common = /** @class */ (function () {
            function Common(globalization, logger, plant, shell, authentication, sidePanelManager, commandBarManager, contextAreaManager, dragDropManager, swacManager, headerService, CONFIG, $state, $window, $rootScope, commandResponse, executionError, messageOverlay, globalDialog, timerService, $translate, $timeout) {
                ...
                this.contextAreaManager = contextAreaManager;
            ...
            Common.prototype.setExecutionError = function (err) {
                ...
                else if (err.status === 500 || err.status === 400 || err.status === 503 || err.status === 403) {
                    try {
            ...
            Common.prototype.safeInterval = function (element, selector, callback, interval, iterationCount) {
                if (!element || !selector || !callback) {
                    throw new Error('SafeTimeout Error: The element context or selector is empty.');
                }
                else {
                    if (typeof (callback) !== 'function') {
                        throw new Error('SafeTimeout Error: callback should be type function.');
                    }
                    else if (typeof (selector) !== 'string') {
                        throw new Error('SafeTimeout Error: selector should be type string.');
                    }
                    else {
                        var counter = 0;
                        var intervalClear = window.setInterval(function () {
                            if (counter > iterationCount) {
                                window.clearInterval(intervalClear);
                                return;
                            }
                            counter++;
                            if (element.is(':visible') && element.find(selector)) {
                                callback.call(this);
                                window.clearInterval(intervalClear);
                            }
                        }, interval);
            ...
            Common.prototype.subscribeApolloContainerEvents = function () {
                ...
                if (this.swacManager.enabled) {
                    this.swacManager.eventBusServicePromise.promise.then(function (eventBusSvc) {
                        ...
                        eventBusSvc.register('showAvailableRuntimeApplications');
                        eventBusSvc.register('showRuntimeHelp');
                        eventBusSvc.register('historyNavigation');
                        eventBusSvc.register('appCtx.update');
                        eventBusSvc.register('mom.swac.navigation.changed');
                        eventBusSvc.register('mom.swac.navigation.queryStringChanged');
                        eventBusSvc.register('mom.swac.navigation.queryStringApplied');
                        eventBusSvc.event.subscribe(function (event) {
                            ...
                            if (event.data.topic === 'showAvailableRuntimeApplications') {
                                _this.logger.logInfo("New context registered", event.data.data);
                                _this.showAvailableRuntimeApplications();
                            }
                            ...
                            if (event.data.topic === 'appCtx.update' && event.data.data.name === 'commandLabels') {
                                _this.logger.logInfo("New context registered", event.data.data);
                                _this.commandBarManager.setCommandLabelVisibility(event.data.data.value);
                            }
                            if (event.data.topic === 'mom.swac.navigation.queryStringChanged') {
                                _this.updateCurrentState(event.data.data.stateName, event.data.data.stateParams);
                            }
                        });
                    });
                }
            };
            Common.prototype.showAvailableRuntimeApplications = function () {
                var _this = this;
                this.globalDialog.set({
                    title: this.$translate.instant('availableRuntimeApplications.title'),
                    templateuri: 'common/widgets/availableRuntimeApplications/available-runtime-applications-template.html',
                    buttons: [{
                            id: 'runtime_application_btn_cancel',
                            displayName: this.$translate.instant('availableRuntimeApplications.close'),
                            onClickCallback: this.globalDialog.hide
                        }]
                });
                this.$timeout(function () {
                    _this.globalDialog.show();
                });
            };
            Common.prototype.navigateBack = function () {
                if (!this.swacManager.enabled) {
                    this.$window.history.back();
                    return;
                }
                if (this.historyTracker.length <= this.index_value) {
                    return;
                }
                this.stateObjectInstane = this.historyTracker[this.historyTracker.length - this.index_value];
                this.stateObjectInstane.navigateBack = true;
                this.$state.go(this.stateObjectInstane.state, this.stateObjectInstane.stateParams, { isBackNavigation: true });
            };
            Common.prototype.updateCurrentState = function (stateName, stateParams) {
                if (!this.swacManager.enabled) {
                    return;
                }
                var historyItem = this.historyTracker[this.historyTracker.length - 1];
                if (historyItem.state !== stateName) {
                    return;
                }
                historyItem.stateParams = angular.extend({}, stateParams);
            };
            ...
            Common.prototype.getApplicationContext = function () {
                return this.applicationContext;
            };
            Common.$inject = [
                'common.services.globalization.globalizationService',
                'common.services.logger.service',
                'common.services.plant.plantService',
         ...
         */
        angular.module('siemens.simaticit.common').service('common', Common);
        ...
        var ApplicationContext = /** @class */ (function () {
            function ApplicationContext() {
                this.functionalRightsLoaded = false;
                this.resourcesLoaded = false;
                this.startupTasksCompleted = false;
                this.pendingNaivgation = null;
            }
            ApplicationContext.prototype.getPendingNavigation = function () {
                return this.pendingNaivgation;
            };
            ApplicationContext.prototype.setPendingNavigation = function (pendingNavigation) {
                this.pendingNaivgation = pendingNavigation;
            };
            ApplicationContext.prototype.isStartupTasksCompleted = function () {
                return this.startupTasksCompleted;
            };
            ApplicationContext.prototype.setStartupTasksCompleted = function (startupTasksCompleted) {
...
(function () {
    ...
    angular.module('siemens.simaticit.common.services.layout')
        .directive('hAdapter', ['$window', 'common.services.component.swacComponentManager',
        'common.widgets.contextArea.service', function ($window, swacComponentManager, contextAreaManager) {
            ...
            return {
                ...
                link: function (scope, elem, attrs) {
                    ...
                    scope.onResize = function () {
                        ...
                        var height = 0;
                        var contextAreaHeight = 0;
                        if (attrs.hAdapter !== '') {
                            var elements = attrs.hAdapter.split(',');
                            for (var i = 0; i < elements.length; i++) {
                                if ($(elements[i]).length > 0) {
                                    height += $(elements[i])[0].clientHeight;
                                    if (elements[i] === '.canvas-sit-context-area')
                                        contextAreaHeight = $(elements[i])[0].clientHeight;
                                }
                            }
                        }
                        ...
                        if (!contextAreaManager.isContextAreaVisible) {
                            elem.windowHeight = elem.windowHeight + contextAreaHeight;
                        }

                        if (elem.attr('data-internal-type') === "property-area-container") {
                            elem[0] && (elem[0].style.height = elem.windowHeight + "px");
                        } else {
                    ...
                    scope.onResize();
                    eventListners[eventListners.length] = scope.$on('context-area-height-changed', onContextArea);
                    function onContextArea() {
                        scope.onResize();
                    }
        ...
        */
        var CARD_CONFIGURATION = {
            ...
            'Interoperability': {
                ...
                'icon': 'svg cmdInteroperability24'
            },
            'Manufacturing': {
                'id': 'Manufacturing',
            ...
            'System': {
                ...
                'icon': 'svg cmdSysConfiguration24'
            },
            'ShopfloorExecution': {
            ...
            'ProductionCoordination': {
                ...
                'icon': 'svg cmdProductionCoordination24'
            },
            'ProductandProductionConfiguration': {
                ...
                'icon': 'svg cmdProductionConfiguration24'
            },
            'WorkOrder': {
                'id': 'WorkOrder',
            ...
            'Process': {
                ...
                'icon': 'svg cmdMaterialAvailability24'
            }
        };
        var HomeCardConfiguration = /** @class */ (function () {
            function HomeCardConfiguration(CONFIG, UIAuthorization) {
                ...
                this.getHomeConfiguration = function () {
                    return this.homeConfig;
                };
                if (CONFIG.type !== 'rt' || CONFIG.home.type === 'custom') {
                    return;
                }
                this.homeType = CONFIG.home.type;
                this.navigationMode = CONFIG.home.navigationMode;
                this.generateOrderedScreen(CONFIG);
                this.getAuthorizedScreens(UIAuthorization);
                this.initHomeConfig(CONFIG);
                this.removeUnusedScreenEntries();
            }
            HomeCardConfiguration.prototype.generateOrderedScreen = function (CONFIG) {
                var _this = this;
                if (this.navigationMode !== 'category') {
                    return;
                }
                CONFIG.categories.forEach(function (category) {
                    if (category.display) {
                        _this.homeConfig[category.id] = category.screens;
                    }
                });
            };
            HomeCardConfiguration.prototype.getAuthorizedScreens = function (UIAuthorization) {
                this.authorizedScreens = __spreadArray([], UIAuthorization.getAuthorizedScreens(), true);
            };
            ...
            HomeCardConfiguration.prototype.getScreenTitleKey = function (content) {
                if (!content.header || content.appTitle !== content.title) {
                    return null;
                }
                return content.header;
            };
            HomeCardConfiguration.prototype.formHomeConfig = function (content) {
                var category = content.category;
                var config = {
                    'title': content.title,
                    'titleKey': this.getScreenTitleKey(content),
                    'state': content.id,
                    'icon': this.parseIcon(content.icon)
                };
                if (!this.homeConfig.hasOwnProperty(category)) {
                    this.homeConfig[category] = [];
                }
                if (this.navigationMode === 'category') {
                    this.setCategoryScreeOrder(category, config, content);
                }
                else {
                    this.homeConfig[category].push(config);
                }
            };
            HomeCardConfiguration.prototype.parseIcon = function (icon) {
                if (icon && icon.substr(0, 4) === 'svg ') {
                ...
                else if ((icon.substr(icon.length - 4, icon.length).toLowerCase() === '.svg')) {
                    return {
                        path: icon.trim(),
                        size: '64px'
                    };
            ...
            HomeCardConfiguration.prototype.setCategoryScreeOrder = function (category, config, content) {
                var i = 0, screenList = this.homeConfig[category], screenLength = screenList.length, screenUpdated = false;
                for (i = 0; i < screenLength; i++) {
                    var item = screenList[i];
                    if (typeof item === 'string' && item === content.id) {
                        this.homeConfig[category].splice(i, 1, config);
                        screenUpdated = true;
                        break;
                    }
                }
            ...
            HomeCardConfiguration.prototype.removeUnusedScreenEntries = function () {
                var _this = this;
                if (this.navigationMode !== 'category') {
                    return;
                }
                var keys = Object.keys(this.homeConfig);
                keys.forEach(function (key) {
                    var index = _this.homeConfig[key].length;
                    while (index--) {
                        if (typeof _this.homeConfig[key][index] === 'string') {
                            _this.homeConfig[key].splice(index, 1);
                        }
                    }
                    _this.homeConfig[key].length === 0 && delete _this.homeConfig[key];
                });
            };
            HomeCardConfiguration.prototype.getHomeType = function () {
                return this.homeType;
            };
            HomeCardConfiguration.prototype.getCardInfo = function () {
                return CARD_CONFIGURATION;
            };
            HomeCardConfiguration.prototype.getConfiguredHomeCards = function () {
                if (this.navigationMode === 'category') {
                    return Object.keys(this.homeConfig);
                }
                return Object.keys(this.homeConfig).sort();
            };
            HomeCardConfiguration.$inject = ['CONFIG', 'common.services.security.ui.authorization', '$translate', '$state'];
...
(function () {
    ...
    function sidePanelManager(swacManager, $window, $timeout) {
        var _that = this;
         ...
         * ```
         */
        function apolloClick(event) {
            if (event.data.topic === 'apolloClicked') {
                $window.document.body.querySelector('div[data-internal-type="property-area-container-modal"]').click();
            }
        }
            ...
            $timeout(function () {
                ...
                if (sidePanelInstances === 1 && swacManager.enabled) {
                    if (type.mode === 'e' && promise) {
                        var panelAutoClose = angular.element('body').attr('sit-side-panel-auto-close');
                        if (panelAutoClose === 'false') {
                        ...
                        var registerApolloClick = promise.register('apolloClicked');
                        promise.event.subscribe(apolloClick);
                    }
                }
            });
        }
        ...
        function close(type) {
            ...
            if (swacManager.enabled && promise) {
                promise.event.unsubscribe(apolloClick);
    ...
    sidePanelManager.$inject = ['common.services.swac.SwacUiModuleManager', '$window', '$timeout'];



    function globalMsgOverlayManager($rootScope, overlayService, swacUiModuleManager, $sanitize) {
            ...
            if (swacUiModuleManager.enabled) {
                if ($rootScope.globalOverlayData.buttons.length > 1 || (options !== undefined && options.type !== undefined && options.type === "confirmation")) {
                    swacUiModuleManager.confirmationServicePromise.promise.then(function (service) {
                        ...
                        service.show($sanitize($rootScope.globalOverlayData.title), $sanitize($rootScope.globalOverlayData.text), {
    ...
    angular.module('siemens.simaticit.common.services.layout').service('common.widgets.messageOverlay.service', ['$rootScope', 'common.overlay.overlayService',
        'common.services.swac.SwacUiModuleManager', '$sanitize', globalMsgOverlayManager
    ]);
    ...
    /**
    * @ngdoc service
    * @name common.services.contextArea.service
    * @module siemens.simaticit.common.services.layout
    *
    * @description
    * This service is responsible for managing the Context Area widget.
    * @example
    * The following example shows the object configuration:
    * ```
    * vm.config = {
    ...
    */
    ...
    function contextAreaManager($rootScope) {
        var self = this;
        var setContextAreaVisibility;
        var updateContextArea;
         ...
         * @ngdoc method
         * @access internal
         * @name common.services.contextArea.service#register
         * @param {function} setContextAreaVisibilityCallback Function that shows or hides the context area (shell controller).
         * @param {function} updateContextAreaCallback  Function that updates the values of the context area (shell controller).
         *
         * @description
         * Stores two functions (from the shell controller) that manages the **contextArea**.
         * ```
         * common.services.contextArea.service.register(setContextAreaVisibilityCallback, updateContextAreaCallback);
         * ```
         */

        function register(setContextAreaVisibilityCallback, updateContextAreaCallback) {
            setContextAreaVisibility = setContextAreaVisibilityCallback;
            updateContextArea = updateContextAreaCallback;
        }


        /**
       * @ngdoc method
       * @name common.services.contextArea.service#show
       * @module siemens.simaticit.common.services.layout
       * @description
       * The Api is used to show the context area.
       * @param { Object} configObject
       * ```
       * common.services.contextArea.service.show(configObject);
       * ```
       * The object should have the following format:
       * * **propertyList**: The Array of object to be displayed in the Context Area in the list format. Each object must have following properties:
       *   * **label**: The label to be displayed for a property.
       *   * **value**: The corresponding value to be displayed for a property.
       *
       * * **templateUrl**: The Url of the custom template to be displayed in the Context Area.
       * * **templateData**: The object data that needs to be binded with custom template
       ...
       */
        ...
        function show(data) {
            self.isContextAreaVisible = true;
            setContextAreaVisibility(true, data);
        }

        /**
       * @ngdoc method
       * @name common.services.contextArea.service#hide
       * @module siemens.simaticit.common.services.layout
       * @description
       * The Api is used to hide the context area.
       *```
       *  common.services.contextArea.service.hide();
       *```
       * * **Note:** * If you need to hide the context area in other pages, call the service in **$destroy** function*.
       */
        function hide() {
            self.isContextAreaVisible = false;
            setContextAreaVisibility(false);
        }

        /**
       * @ngdoc method
       * @name common.services.contextArea.service#update
       * @module siemens.simaticit.common.services.layout
       * @description
       * The Api is used to update the values in the context area.
       * ```
       *  common.services.contextArea.service.update(configObject);
       * ```
       */
        function update(data) {
            updateContextArea(data);
        }
    }
    angular.module('siemens.simaticit.common.services.layout').service('common.widgets.contextArea.service', ['$rootScope', contextAreaManager]);
...
(function () {
    ...
    angular.module('siemens.simaticit.common.services.layout').provider('common.services.layout.shell', function () {
        ...
        function setDefaultLanguage(logger, globalization, authentication, languages) {
            ...
            if (umcLang) {
                ...
                var shortUmcLang = codes[0];
                if (codes.length === 2) {
                    umcLang = codes[0].toLowerCase() + '-' + codes[1].toUpperCase();
                }

                if (languages && languages.indexOf(umcLang) > -1) {
                    localeKey = umcLang;
                } else if (languages){
                    languages.find(function (language) {
                        if (language.startsWith(shortUmcLang)) {
                            localeKey = language;
                            return true;
                        }
                        return false;
                ...
                return {
                    ...
                    setGlobalization: function (RESOURCE, languages) {
                        ...
                        setLocalResourcePath(RESOURCE, $q, $location, $window, logger, globalization).then(function () {
                            setDefaultLanguage(logger, globalization, authentication, languages).then(function () {
                    ...
                    setDefaultLanguage: function (languages) {
                        return setDefaultLanguage(logger, globalization, authentication, languages);
                    },
    ...
    (function (framework) {
        ...
        *
        ...
        var SignalManagerService = /** @class */ (function () {
            function SignalManagerService($window, $log, $q, config, authentication, plantService, ExecutionError) {
                this.$window = $window;
                this.$log = $log;
                this.$q = $q;
                this.config = config;
            ...
            */
            ...
            SignalManagerService.prototype.disconnectAppWithSignals = function (appName) {
                var deferred = this.$q.defer();
                if (appName) {
                    var connection;
                    var i = 0, length = this.connections.length, count = 0;
                    for (i; i < length; i++) {
                        if (this.connections[i] && this.connections[i].signalApp === appName) {
                            connection = this.connections[i];
                            connection.signalManager.close();
                            this.removeConnection(connection.id, connection.signalApp);
                            i--;
            ...
            SignalManagerService.prototype.getConfigUrlForCurrentPlant = function () {
                var plant = this.plantService.getCurrentPlant();
                if (plant && plant.NId) {
                    return '/sit-svc/config?plantid=' + plant.NId;
                }
                return '/sit-svc/config';
            };
            SignalManagerService.$inject = ['$window', '$log', '$q', 'common.services.signalManager.AddressConfig', 'common.services.ui.authentication', 'common.services.plant.plantService', 'ExecutionError'];
        ...
        var SignalConnection = /** @class */ (function () {
            function SignalConnection(signalEndPointMetadataUrl, authentication, signalName, onConnectionError, appName, settings, parent) {
                ...
                */
                ...
                this.onError = function (reason) {
                    if (_this.state() === 3 || _this.state() === 5) {
                        _this.lastError = new _this.ExecutionError('-1', 'Connection Error');
                        if (_this.onConnectionError instanceof Function) {
            ...
            */
            ...
            SignalConnection.prototype.connect = function (autoSubscribe, appName) {
                ...
                var configUrl = this.parent.getConfigUrlForCurrentPlant();
                this.signalManager.init(token.claimToSend, configUrl, appName).then(function () {
                    ...
                    //  this.onConnectionError(this, error);
                    deferred.reject(error);
                });
                return deferred.promise;
            };
            SignalConnection.prototype.establishConnection = function (autoSubscribe, signalName, onConnectionError) {
                ...
                this.onConnectionError = onConnectionError;
                this.name = signalName;
                this.signalManager.open(signalName, this.onNext, this.onError, this.onCompleted, autoSubscribe, this.options).then(function () {
                    if (_this.activeTimeoutCount < 1) {
                        return;
                    }
            ...
            SignalConnection.prototype.openSocket = function (autoSubscribe, appName, errorCallbackObject) {
                ...
                var configUrl = this.parent.getConfigUrlForCurrentPlant();
                this.signalManager.init(token.claimToSend, configUrl, appName, errorCallbackObject).then(function () {
                    if (_this.activeTimeoutCount < 1) {
                        _this.signalManager.close();
                        return;
                    }
            ...
            * ```
            ...
            function SignalManagerDevController($scope,signalManagerService) {
                ...
                signalManagerService.createConnection("EvtMgrApp", "mySignal", onConnectionError).then(function (signalconn) {
                    ...
                    //Subscribe to signalConnection with type equal to "Test"
   ...
   */
    ...
    angular.module('siemens.simaticit.common.base').service('common.base.unityInterceptorService',
        ['$q', 'common.services.logger.service', 'common.base.unityInterceptorConfig', 'LOG_TYPES',
            'common.services.ui.authentication', 'common.services.authentication.config', 'common.services.swac.SwacUiModuleManager','BUILDNUMBER', interceptor]);

    function interceptor($q, logger, unityInterceptorConfig, LOG_TYPES, authentication, authenticationConfig, swacManager, BUILDNUMBER) {
        ...
        function request(config) {
            ...
            if (config.url && config.url.indexOf('.json') > -1) {
                config.url += '?b=' + BUILDNUMBER;
            }
