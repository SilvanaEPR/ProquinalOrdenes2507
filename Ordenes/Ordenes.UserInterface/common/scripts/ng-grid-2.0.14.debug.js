...
/* SIMATIC IT Unified Architecture Foundation V4.0 | Copyright (C) Siemens AG 2020. All Rights Reserved. */
...
(function (window, $) {
    ...
    angular.module('ngGrid.services').factory('$domUtilityService', ['$utilityService', '$window', function ($utils, $window) {
        ...
        domUtilityService.BuildStyles = function ($scope, grid, digest) {
            ...
            if (isScrollBarVisible && trw === grid.elementDims.rootMaxW) {
                ...
                ngRowWidth = trw < grid.elementDims.rootMaxW ? grid.elementDims.rootMaxW : trw;
            }
    ...
    var ngColumn = function (config, $scope, grid, domUtilityService, $templateCache, $utils) {
        ...
        if ($utils.isNullOrUndefined(colDef.visible)) {
            self.visible = $utils.isNullOrUndefined(colDef.visible);
        } else if (typeof colDef.visible === 'boolean') {
        ...
        self.gripClick = function () {
            ...
            if (clicks === 1) {
                ...
                // © Siemens AG, 2023
                //Save and Update column width when double clicked to resize
                self.resizedColData = {
                    width: self.width,
                    origWidth: self.origWidth,
                    field: self.field
                };
                $scope.$emit('ngGridEventColumnResize', self.resizedColData);
                clicks = 0; //after action performed, reset counter
            }
        };
        ...
        self.gripOnMouseUp = function (event) {
            ...
            $scope.$broadcast('ngGridEventColumnResize');
            // © Siemens AG, 2023
            //Save and Update column width after resize
            self.resizedColData = {
                width: self.width,
                origWidth: self.origWidth,
                field: self.field
    ...
    var ngEventProvider = function (grid, $scope, domUtilityService, $timeout) {
        ...
        self.assignEvents = function () {
            ...
            $scope.$on('$destroy', $scope.$watch('renderedColumns', function () {
                $timeout(self.setDraggables, 0, false);
            }));
        };
        ...
        self.assignGridEventHandlers = function () {
            ...
            var windowResize = function () {
                ...
                windowThrottle = setTimeout(function () {
                    ...
                    var viewPort = $scope.viewportDimHeight();
                    if (viewPort) {
                        // salt - render two rows more for smooth scrolling
                        $scope._limit = Math.ceil(($scope.viewportDimHeight() + ($scope.rowHeight * 2 + 10)) / grid.config.rowHeight);
                    }
                }, 100);
            };
            ...
            var resizeObserver = new ResizeObserver(function () {
                setTimeout(function () {
                    ...
                    if (viewPort) {
                        // salt - render two rows more for smooth scrolling
                        $scope._limit = Math.ceil(($scope.viewportDimHeight() + ($scope.rowHeight * 2 + 10)) / $scope.rowHeight);
                    }
                }, 100);
            });

            if (grid.$element && grid.$element[0]) {
                resizeObserver.observe(grid.$element[0]);
            }
    ...
    var ngGrid = function ($scope, options, sortService, domUtilityService, $filter, $templateCache, $utils, $timeout, $parse, $http, $q, dragDropManager) {
        ...
        self.configureColumnWidths = function () {
            ...
            if (asterisksArray.length > 0) {
                ...
                angular.forEach(asterisksArray, function (colDef, i) {
                    // Get the ngColumn that matches the current column from columnDefs
                    var ngColumn = $scope.columns[indexMap[colDef.index]];
                    // is the ngColumn has width provided by the user?
                    // no, then calculate the width based on the asteriskVal
                    if (ngColumn.width === undefined) {
                        ngColumn.width = asteriskVal * colDef.width.length;
                    }
                    else {
                        // is the ngColumn.width is * ? 
                        // yes, calculate 
                        // no, is the saved value and ngColumn value are same from second column?
                        // yes, calculate again.
                        if (ngColumn.width === '*' || (i > 0 && self.width === ngColumn.width)) {
                            ngColumn.width = asteriskVal * colDef.width.length;
                        }
                    }
        ...
        $scope.adjustScrollTop = function (scrollTop, force) {
            ...
            var newRange;
            self.config.virtualizationThreshold = options.virtualizationThreshold;
    ...
    var ngRowFactory = function (grid, $scope, domUtilityService, $templateCache, $utils) {
        ...
        self.renderedChange = function () {
            ...
            if (grid.config.groups.length > 0 && grid.rowFactory.aggCache.length > 0 && !grid.isScrolled) {
                var isCollapsed = _.findIndex(grid.rowFactory.aggCache, {
                    collapsed: false
                })
                if (isCollapsed !== -1) {
        ...
        self.getGrouping = function (groups) {
            ...
            for (var x = 0; x < rows.length; x++) {
                ...
                for (var y = 0; y < groups.length; y++) {
                    ...
                    // © Siemens AG, 2015
                    val = (val === '' || val === null || val === undefined) ? $scope.i18n.ngNoValue : val.toString();
                    if (!ptr[val]) {
                        ptr[val] = {};
                    }
                    if (!ptr[NG_FIELD]) {
    ...
    var ngSelectionProvider = function (grid, $scope, $parse, $utils) {
        ...
        self.toggleSelectAll = function (checkAll, bypass, selectFiltered) {
            ...
            if (bypass || grid.config.beforeSelectionChange(rows, checkAll)) {
                if ((!selectFiltered || (!$scope.enablePaging && !checkAll)) && self.selectedItems.length > 0) {
            ...
            compile: function () {
                return {
                    pre: function ($scope, iElement, iAttrs) {
                        ...
                        // © Siemens AG, 2014
                        //logic to reset the resized column widths
                        options.gridApi = {
                            resetColumnWidths: resetColWidths
                        };
                        function resetColWidths() {
                            angular.forEach($scope.columns, function(col) {
                                if (col.colDef.hasOwnProperty('originalColumnWidth')) {
                                    col.width = col.colDef.originalColumnWidth;
                                    col.colDef.width = col.colDef.originalColumnWidth;
                                }
                            });
                            domUtilityService.RebuildGrid($scope, grid);
                        }
                        ...
                        return grid.init().then(function () {
                            ...
                            if (typeof options.data === "string") {
                                var dataWatcher = function (a) {
                                    ...
                                    grid.data = $.extend([], a);
                                    var pageSelection = $scope.page.allSelected;
                                    ...
                                    /*Auto selectAll for infinite scroll*/
                                    if (pageSelection && !$scope.enablePaging) {
                                        if (grid.data.length) {
                                            $scope.selectionProvider.toggleSelectAll(pageSelection, false, !!grid.data.length);
                                        } else {
                                            $scope.selectionProvider.toggleSelectAll(false);
                                        }
                                    }
                ...
                return {
                    pre: function ($scope, iElement) {
                        ...
                        if ($scope.row.isAggRow) {
                            ...
                            compiledObject = $compile(html)($scope);
                        } else {
                            compiledObject = $compile($templateCache.get($scope.gridId + 'rowTemplate.html'))($scope);
                        }
                        iElement.append(compiledObject);
    ...
    ngGridDirectives.directive('ngViewport', ['$animate', '$timeout', function ($animate, $timeout) {
        ...
        return function ($scope, elm) {
            ...
            var viewPort = $scope.viewportDimHeight();
            if (viewPort) {
                // 70 is the salt to render one extra row when scrolled
                $scope._limit = Math.ceil(($scope.viewportDimHeight() + 70) / $scope.rowHeight);
                $scope._begin = 0;
            } else {
                // if the view port height is not avaialable, rendering 30 rows initially
                $scope._limit = 30;
                $scope._begin = 0;
            }
            ...
            function scroll(evt) {
                ...
                var begin = Math.ceil(scrollValue / (rowHeight + 1)) - 1;
                if (begin < 0) begin = 0;
                $scope._begin = begin;
                ...
                if ($scope.forceSyncScrolling) {
                    ...
                    scrollTimer = setTimeout(ensureDigest);
                }
                if ($scope._prevScollLeft !== undefined && scrollLeft !== $scope._prevScollLeft) {
                ...
                $scope._prevScollLeft = scrollLeft;
                $scope._prevScollTop = scrollTop;
    ...
    angular.module('ngGrid').run(['$templateCache', function ($templateCache) {
        ...
        $templateCache.put('aggregateTemplate.html',
            ...
            "\n" +
            "<div ng-style=\"::{height: rowHeight}\" class=\"uyAggregateText\" ng-if=\"multiSelect\">\r" +
            "\n" +
            "<div id=\"group-check-container\" style=\"display:table-cell; vertical-align:middle; height:inherit;\">\r" +
            "<div style=\"line-height:1em\">" +
            "<div class=\"sit-checkbox\">" +
            "<div class=\"validator-control checkbox-noborder\" style=\"background: transparent; line-height:12px\">" +
            "<div class=\"group-control\">" +
            "<div class=\"check-box checkbox-enabled\" style=\"padding-top:2px; display:block; position:static\">" +
            " <label>" +
            " <input type=\"checkbox\" class=\"dirty-disabled\" style=\"margin: 0 10px 0 2px; cursor: pointer\" ng-click=\"selectAggregate(row.getAggregateSelectArg(true));\" ng-checked=\"row.getCheckState();\"/>" +
            ...
            "</div>\r" +
            "\n" +
            "    <div ng-if=\"row.showSvgIcon()\" style=\"display:inline-block; float:left;\"><div id=\"group-arrow-container\" sit-mom-icon=\"{'path':  'common/icons/miscRightArrow16.svg','size':'16px'}\" ng-class=\"'uyAggArrow'\" ng-click=\"row.toggleExpand()\" ng-style=\"::{height: rowHeight}\"></div></div>\r" +
            "    <div ng-if=\"!row.showSvgIcon()\" style=\"display:inline-block; float:left;\"><div id=\"group-arrow-container\" sit-mom-icon=\"{'path':  'common/icons/miscDownArrow16.svg','size':'16px'}\" ng-class=\"'uyAggArrow'\" ng-click=\"row.toggleExpand()\" ng-style=\"::{height: rowHeight}\"></div></div>\r" +
            "\n" +
            "<div ng-style=\"::{height: rowHeight}\" class=\"uyAggregateText uyAggregateTextOverflow\">\r" +
            "\n" +
            "<div id=\"group-text-container\" title=\"{{::row.label CUSTOM_FILTERS}}({{row.totalChildren()}}{{AggItemsLabel}})\" ng-click=\"row.toggleExpand()\"  class=\"uyAggText\">{{row.label CUSTOM_FILTERS}} ({{row.totalChildren()}}{{AggItemsLabel}})</div>\r" +
            "\n" +
        ...
        $templateCache.put('checkboxHeaderTemplate.html',
            "<div class=\"sit-checkbox\" style=\"position:absolute; height:24px; width:24px\">" +
            "<div class=\"validator-control checkbox-noborder\" style=\"background: transparent; min-height:24px; height:24px; padding:0; padding-left: 4px;\">" +
            "<div class=\"group-control\">" +
            "<div class=\"check-box checkbox-enabled\" style=\"padding-top:2px;display:block; position:static\">" +
            "<label>" +
            "<input class=\"ngSelectionHeader dirty-disabled\" type=\"checkbox\" ng-if=\"multiSelect\" style=\" top:2px\"" +
            " ng-model=\"page.allSelected\" ng-change=\"toggleSelectAll(page.allSelected, true)\"/>" +
            "<span class=\"property-label-ellipsis\">" +
            "<span class=\"check\"></span></span>" +
            "</label>" +
            "</div>" +
        ...
        $templateCache.put('checkboxCellTemplate.html',
            ...
            " <div class=\"group-control\">" +
            "<div class=\"check-box checkbox-enabled\" style=\"padding-top:2px;display:block; position:static\">" +
             "<label ng-click=\"$event.preventDefault()\">" +
            " <input type=\"checkbox\" class=\"dirty-disabled\" data-checked=\"{{::row.selected}}\" ng-model=\"row.selected\">" +
            "<span class=\"property-label-ellipsis\">" +
            "<span class=\"check\"></span></span>" +
            "</label>" +
            "</div>" +
        ...
        $templateCache.put('footerTemplate.html',
            "<div ng-if=\"showFooter\" class=\"ngFooterPanel\" ng-class=\"{'ui-widget-content': jqueryUITheme, 'ui-corner-bottom': jqueryUITheme}\" style=\"width:100%\" ng-style=\"footerStyle()\">\r" +
            "\n" +
            ...
            "\n" +
            // © Siemens AG, 2016
            "            <span class=\"ngLabel\">{{'pager.total-items' | translate}}: {{maxRows()}}</span><span ng-if=\"filterText.length > 0\" class=\"ngLabel\">({{'pager.filtered-items' | translate}}: {{totalFilteredItemsLength()}})</span>\r" +
            "\n" +
            "        </div>\r" +
            "\n" +
            ...
            "\n" +
            "                <span class=\"ngGridMaxPagesNumber\" ng-if=\"maxPages() > 0\">/ {{maxPages()}}</span>\r" +
            "\n" +
            "            </div>" +
            "\n" +
        ...
        $templateCache.put('gridTemplate.html',
            ...
            "\n" +
            "    <div class=\"ngGroupPanel\" ng-if=\"showGroupPanel()\" style=\"width:100%\" ng-style=\"::groupPanelStyle()\">\r" +
            "\n" +
            ...
            "        <div id=\"data-row-container\" ng-style=\"::rowStyle(row)\" ng-repeat=\"row in renderedRows | limitTo : _limit : _begin\" ng-click=\"row.toggleSelected($event)\" ng-class=\"row.alternatingRowClass()\" ng-row></div>\r" +
            "\n" +
            "    </div>\r" +
            "\n" +
            "</div>\r" +
        ...
        $templateCache.put('headerCellTemplate.html',
            // © Siemens AG, 2014
            "<div class=\"ngHeaderSortColumn {{::col.headerClass}}\" style=\"cursor: pointer\" ng-class=\"{ 'ngSorted': !col.noSortVisible() }\" ng-click=\"col.sort($event)\">\r" +
            "\n" +
            "    <div id=\"header-sort-button\" ng-attr-title=\"{{::col.displayName}}\" class=\"colt{{::col.index}} ngHeaderText\">{{::col.displayName}}</div>\r" +
            "\n" +
            "    <div class=\"ngSortButtonDown\" ng-if=\"col.showSortButtonDown()\">\r" +
            "\n" +
            "<svg icon-name='miscUpArrow16' version='1.1' id='Livello_2' xmlns='http://www.w3.org/2000/svg' xmlns: xlink='http://www.w3.org/1999/xlink' x='0px' y='0px' viewBox='0 0 16 16' enable-background='new 0 0 16 16' xml: space='preserve' class='up-icon'>\r" +
            "\n" +
            "<polygon id='polygon2-9' class='sort-icon' fill='#CDCDCD' points='8,6 12,10 4,10 '>\r" +
            "\n" +
            "</polygon >\r" +
            "\n" +
            "</svg>\r" +
            ...
            "</div >\r" +
            "\n" +
            "    <div class=\"ngSortButtonUp\" ng-if=\"col.showSortButtonUp()\">\r" +
            "\n" +
            "<svg icon-name='miscDownArrow16' version='1.1' id='Livello_2' xmlns='http://www.w3.org/2000/svg' xmlns: xlink='http://www.w3.org/1999/xlink' x='0px' y='0px' viewBox='0 0 16 16' enable-background='new 0 0 16 16' xml: space='preserve' class='down-icon'>\r" +
            "\n" +
            "<polygon class='sort-icon' fill='#CDCDCD' points='4,6 8,10 12,6 '>\r" +
            "\n" +
            "</polygon >\r" +
            "\n" +
            "</svg>\r" +
            ...
            "</div>\r" +
            "\n" +
            "<div id=\"header-resize-button\" ng-if=\"col.resizable\" class=\"ngHeaderGrip\" ng-click=\"col.gripClick($event)\" ng-mousedown=\"col.gripOnMouseDown($event)\"><span></span></div>\r" +
            "\n"
        );
        ...
        $templateCache.put('menuTemplate.html',
            ...
            "\n" +
            "\t\t\t\t<a title=\"Group By\" ng-class=\"::col.groupedByClass()\" ng-if=\"col.groupable && col.visible\" ng-click=\"groupBy(col)\"></a>\r" +
            "\n" +
            "\t\t\t\t<span class=\"ngGroupingNumber\" ng-if=\"col.groupIndex > 0\">{{::col.groupIndex}}</span>          \r" +
            "\n" +
            "            </li>\r" +
            "\n" +
            "        </ul>\r" +
        ...
        $templateCache.put('rowTemplate.html',
            '<div ng-repeat="col in renderedColumns" ng-class="::col.colIndex()" class="ngCell {{::col.cellClass}}"> <div ng-cell ></div> </div>'
        );
