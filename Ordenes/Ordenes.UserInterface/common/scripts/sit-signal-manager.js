...
SIT.SignalsManager = function ($q) {
    ...
    function _send(verb, address, headers) {
        ...
        try {
            ...
            xhr.withCredentials = _isCrossDomainRequest(address);
            ...
            xhr.onerror = function () {
                deferred.reject('error on connection.');
            };
            xhr.ontimeout = function () {
                deferred.reject('timeout on connection.');
            };
            xhr.send(null);
        }
        catch (err) {
    ...
    function setAuthCookie(token, configEndpoint) {
        ...
        getAuthorizationEndpoint(configEndpoint).then(function (authEndpoint) {
            authEndpoint = _constructURL(configEndpoint, authEndpoint);
    ...
    function _constructURL(baseURL, specificAddress) {
        try {
            //Raises a TypeError exception if baseURL is not a valid URL
            var baseURLObj = new URL(baseURL);
            var specificURLObj = new URL(specificAddress, baseURLObj.origin)
            return specificURLObj.href;
        } catch (e) {
            return '';
        }
    }

    function _constructWebSocketUrl(baseURL, specificAddress) {
        try {
            //Raises a TypeError exception if baseURL is not a valid URL
            var baseURLObj = new URL(baseURL);
            baseURLObj.protocol = (baseURLObj.protocol == "https:") ? "wss:" : "ws:";
            var specificURLObj = new URL(specificAddress, baseURLObj.origin)
            return specificURLObj.href;
        } catch (e) {
            return '';
    ...
    function _isCrossDomainRequest(address) {
        var crossDomain = _verifyDomainRequest(address);
        if (address.endsWith('Session') && crossDomain) {
            return true;
        }
        return false;
    ...
    function _verifyDomainRequest(address) {
        var currentURL = new URL(window.location.href);
        var specificURL = new URL(address);
        if (currentURL.origin.toString() === specificURL.origin.toString()) {
            return false;
        }
        return true;
    }
    ...
    function init(token, configEndpoint, appName, options) {
        ...
        if (!_self.isInitialized) {
            ...
            if ((!configEndpoint) || (configEndpoint === '')) {
                ...
                if (!_isAbsoluteURL(configEndpoint)) {
                    var currentURL = new URL(window.location.href);
                    var configEndpointObj = new URL(configEndpoint, currentURL.origin)
                    configEndpoint = configEndpointObj.href;
                ...
                setAuthCookie(token, configEndpoint).then(function () {
                    ...
                    getMetadataEndpoint(configEndpoint).then(function (signalMetadataEndpoint) {
                        signalMetadataEndpoint = _constructURL(configEndpoint, signalMetadataEndpoint);
                        // Get the signal metadata
                        var header = [{ name: 'Authorization', value: token }];
                        getMainAndStreamEndpoints(signalMetadataEndpoint, header).then(function (endpoints) {
                            _self.mainAddress = _constructWebSocketUrl(configEndpoint, endpoints.mainAddress);
                            open().then(function () {
