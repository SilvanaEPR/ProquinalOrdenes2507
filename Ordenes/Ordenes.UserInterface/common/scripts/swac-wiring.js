...
SWAC.Wire = function (name, inputDescription, outputDescription, converter, parameter, options, store) {
  ...
  var _that = this,
      ...
      _ignoreTransaction = (typeof options.joinTransaction !== 'undefined') ? !options.joinTransaction : true,

      _input = Array.isArray(inputDescription) ? inputDescription : [inputDescription],
      ...
      _wire = function () {
        ...
        try {
          ...
          for (i = 0; i < _input.length; i++) {
            entry = _input[i];
            if (entry.component) {
              comp = SWAC.Container._internal.getCmpList({ name: entry.component });
              if (entry.event) {
          ...
          try {
            if (_output.component && _output.method) {
              if (_output.method.indexOf('.') === -1) {
                apiout = SWAC.Container._internal.getCmpList({ name: _output.component });
              ...
              dpcout = SWAC.Container._internal.getCmpList({ name: _output.component });
              if (dpcout.dpc) {
                _setOutput = function (value) {
                  dpcout.dpc.open(_output.dpc).beginSet(value, _ignoreTransaction).then(function (data) {
              ...
              if (_output.method) {
                ...
                _setOutput = function (value) {
                  target.dpc.open(_output.dpc).beginSet(value, _ignoreTransaction).then(function (data) {
                    _onOutputChanged.notify(data.data);
                  });
                };
              }
  ...
  var
      ...
      _findWiresIO = function (wireList, entityValue, context, direction) {
        ...
        for (wire in wireList) {
          if (wireList[wire].description[direction]) {
            if (Array.isArray(wireList[wire].description[direction])) {
      ...
      */
      ...
      _get = function (filter) {
        ...
        if (filter.inputComponents) {
          if (Array.isArray(filter.inputComponents)) {
        ...
        if (filter.outputComponents) {
          if (Array.isArray(filter.outputComponents)) {
      ...
      _checkForWireMembers = function (arg) {
        ...
        argInput = Array.isArray(arg.input) ? arg.input : [arg.input];
   ...
   _onCreated = new SWAC.Eventing.Publisher('onCreated'),
      ...
      _observeDpc = function (comp) {
        var cur = SWAC.Container._internal.getCmpList({ name: comp }),
            createHandler = function (add) {
              return add ? function (event) {
  ...
  _checkOutputWirePreconditions = function (wire) {
    ...
    if (outgoing) {
      if (outgoing.component) {
        target = SWAC.Container._internal.getCmpList({ name: outgoing.component });
        if (!target) {
  ...
  try {
    if ((typeof module !== 'undefined') && (typeof module.exports !== 'undefined')) {
      ...
      define('@swac/wiring', ['@swac/container'], function (base) {
        if (base && typeof base.prototype.Wiring !== 'object') {
