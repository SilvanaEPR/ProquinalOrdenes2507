...
SWAC.Component = function (name, iframe, type, settings, lastUrl) {
  ...
  */
	  _dpcChanged = function (key, type, content) {
	    ...
	    if (((type === 'set')) && (key === 'private.isDirty') ) {
	      ...
	      _rootnode.access().open(key).beginGet().then(succCallback(key));
	    }
	  },
  ...
  */
  _subscribeEvent = function (callback, filters) {
    ...
    // event management not available in Workers
    if (typeof Worker !== 'undefined' && _that._internal.iframe.getIFrame() instanceof Worker) {
      SWAC.Logging.log('Event management not supported for \'' + _that.name() + '\' Worker component', SWAC.Logging.Level.External);
      return false;
    }
...
SWAC.ComponentBox = function (arg) {
  var self = this,
    ...
    mode = (settings.flavor.toLowerCase() === 'webworker') ? 'worker' : 'iframe',
    loadCallbck = null;
...
*/
  var _removeComponent = function (name, iframe, reason) {
    ...
    if (cmp) {
      ...
      _onRemoved.notify({ name: shortName, reason: reason });
    }
  };
 ...
 */
  ...
  */
  ...
  _get = function (filter) {
      ...
      if (typeof filter !== 'object') {
        ...
        for (filterProp in filter) {
          if (filter.hasOwnProperty(filterProp)) {
            if (filterProp !== 'name' && filterProp !== 'type' && filterProp !== 'dirty' && filterProp !== 'attributes') {
              return [];
            }
          }
        }
    ...
    return (filter && filter.name && !filter.type && !filter.dirty && !filter.attributes) ? found[0] : found;
  },
	      ...
	      if (!isSingleton) {
	        ...
	        if (_cache[realCmpName][name]) {
              ...
              (fullName.length + 1 === prealCmpName.length)) {
	            ...
	            _onServiceCreated.notify({ component: SWAC._internal.Utils.getRealComponentName(prealCmpName), service: name, instance: expose });
            }
	          else {
	            _onServiceCreated.notify({ component: JSON.stringify(_convertNames(prealCmpName)), service: name, instance: expose, delegated: true });
	          }
	        }
	      }
	      ...
	      if (cs) {
	        ...
	        for (i = 0; i < serviceinstances.length; i++) {
              ...
              (fullName.length + 1 === prealCmpName.length)) {
	            ...
	            prealCmpName = prealCmpName.toString().substring(fullName.toString().length + 1, prealCmpName.toString().length);
	            _onServiceRemoved.notify({ component: SWAC._internal.Utils.getRealComponentName(prealCmpName), service: serviceinstances[i] });
	          }
	          else {
	            _onServiceRemoved.notify({ component: JSON.stringify(_convertNames(prealCmpName)), service: serviceinstances[i], delegated: true });
	          }
