...
SWAC.Config = (function () {
  ...
  return {
    ...
    */
    ...
    TimeOuts: {
      Enabled: (function () {
        var _timeoutEnabledObj = SWAC._internal.Utils.getItemFromSessionStorage('swac.timeouts.enabled');
        if (_timeoutEnabledObj.found) {
          return (_timeoutEnabledObj.type === 'boolean') ? _timeoutEnabledObj.value : true;
        }
        else {
          return true;
        }
...
SWAC.Communication = (function () {
  ...
  var
      ...
      _isSourceComponent = function (source) {
        if (_checkWorker(source).result) {
          return _checkWorker(source).container;
        }
            ...
            if ((typeof (event.data) === 'string') && (event.data.length > 0)) {
              ...
              switch (data.t) {
                case 'boot': // INITIAL HAND-SHAKE
                  _handleBootMessage(_checkWorker(this).result ? this : event.source, data.c, event.origin);
                  break;
                case 'msg': // INTERNAL MESSAGES
                  _handleInternalMessage(_checkWorker(this).result ? this : event.source, data.c, event.origin);
                  break;
              }
            }
            else {
        ...
        return function (key, type, content) {
          ...
          if (type === 'addQueue') {
            ...
            if (content.flags !== SWAC.DPC._internal.Flags.Stub) {  //Stub + 0 (permission.None) -> node is not created proxy side
              if (_that.transaction.getId() !== null) {
      ...
      */
      _setDpcOnTransaction = function (dpc) {
        return new Promise(function (resolve, reject) {
          _privateDpc.root.open('public').beginSet(dpc.key, dpc.value, dpc.type, dpc.flags).then(
            function (value) {
  ...
  */
  ...
  /**
	...
	_beginExpose = function (final) {
	  ...
	  SWAC.Communication._internal.beginCall(null, (_finalFn ? null : SWAC.Config.TimeOuts.Internal), 'beginExpose', [expObj.api, expObj.events, hasPublicDpc, interfaces, SWAC.version], true).then(
						function (value) {
      ...
      */
       ...
       */
       _fireOnValueChanged = function (value, oldValue) {
         var retParamObj = { key: _key, relativeKey: _key, value: value, oldValue: oldValue },
           publicNode;
    ...
    if (_mine(uuid) || ((_getflags() & SWAC.DPC.Permissions.Write) === SWAC.DPC.Permissions.Write)) {
      ...
      accessor.beginSet = function (value, ignoreTransaction) {
        if (_dirty()) {
          ...
          return _beginSet(uuid, value, ignoreTransaction);
        }
      };
    }
      ...
      _set = function (uuid, key, value, type, flags) {
        ...
        if ((_mine(uuid)) || ((_getflags() & SWAC.DPC.Permissions.Write) === SWAC.DPC.Permissions.Write)) {
          ...
          if (key.indexOf('.') > -1) {
            ...
            if (current instanceof SWAC.DPC.Node) {
              return current.set(uuid, namespaces[namespaces.length - 1], value, type, flags);
            } else {
              throw new SWAC.Exception(1010);
            }
          }
  ...
  this.access = function (uuid) {
    ...
    if (_mine(uuid) || ((_getflags() & SWAC.DPC.Permissions.Write) === SWAC.DPC.Permissions.Write)) {
      accessor.set = function (key, value, type, flags) {
        var r = {};
        _isDirty();
        r = _set(uuid, key, value, type, flags);
...
SWAC.DPC.Transaction = function (parent) {
  ...
  var
    ...
    */
    _addInQueue = function (key, content) {
      _actionsQueue.push({ key: key.substr(key.indexOf('.') + 1), value: content.value, type: content.type, flags: content.flags, promise: content.promise });
    },
    ...
    * @param {Object} dpcObjList list of dpc and related new values
		...
		*/
        ...
        _errorCount = 0,
		  ...
		  if (!_isSupported()) {
		    return Promise.reject(new SWAC.Reason(17014));
		  }

		  if (!_parentCall && (_id !== null)) {
		    return Promise.reject(new SWAC.Reason(18003));
		  }

		  if (!Array.isArray(dpcObjList)) {
		    return Promise.reject(new SWAC.Reason(18002));
		  }
    ...
    /**
		...
		*/
        ...
        bulkQueue = [];
		  ...
		  if (!_id) {
		    return Promise.reject(new SWAC.Reason(17008));
		  }

		  if (_actionsQueue.length === 0) {
		    return Promise.reject(new SWAC.Reason(17009));
		  }
		  ...
		  _actionsQueue.forEach(function (item) {
		    ...
		    if (_dpc) {
		      ...
		      bulkQueue.push({ key: item.key, value: item.value, type: item.type, flags: item.flags });
		    }
		  });
  ...
  return {
    ...
    if (typeof Promise === 'undefined') {
      throw new Error('SWAC won�t run in this environment, Promise is not defined');
    }
  ...
  window.SWAC._internal.getBase = function () {
    return 'data:text/javascript;base64,' + window.btoa('(' + unescape(encodeURIComponent(baseFn.toString())) + ')();');
  };
})();