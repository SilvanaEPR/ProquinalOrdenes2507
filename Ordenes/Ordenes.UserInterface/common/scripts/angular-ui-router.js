...
function $StateProvider(   $urlRouterProvider,   $urlMatcherFactory) {
  ...
  function $get(   $rootScope,   $q,   $view,   $injector,   $resolve,   $stateParams,   $urlRouter,   $location,   $urlMatcherFactory) {
    ...
    $state.go = function go(to, params, options) {
        return $state.transitionTo(to, params, extend({ inherit: true, relative: $state.$current, isBackNavigation: false }, options));
    };
    ...
    $state.transitionTo = function transitionTo(to, toParams, options) {
      ...
      if (options.notify) {
         ...
         * </pre>
         */
        if ($rootScope.$broadcast('$stateChangeStart', to.self, toParams, from.self, fromParams, options, to).defaultPrevented) {
